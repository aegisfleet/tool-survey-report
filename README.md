...
サイトの品質向上のため、以下の継続的改善プロセスを実施しています：

```mermaid
flowchart TD
    A[AIエージェントがタスクを実行] --> B[間違った修正を実行]
    B --> C[AIエージェントに問題の原因を問う]
    C --> D[AIエージェント自身で改善策を検討]
    D --> E[AIエージェントがAGENTS.mdを更新]
    E --> A
```

AIエージェントは以下のように継続的に学習・改善します。

1. **原因分析**: なぜ間違いが発生したかをAIエージェント自身が分析
2. **改善策立案**: 同じ間違いを繰り返さないための具体的な対策を検討
3. **指示書更新**: `AGENTS.md`や関連タスク指示書を更新
4. **学習の蓄積**: 新しい知識とルールを次回のタスクに活用
