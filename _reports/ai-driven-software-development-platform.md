...
* **アーキテクチャ**: 複数の特化型AIエージェントが協調して動作するマルチエージェントシステム。独自LLM「Takane」を中核とし、エンタープライズ向けのセキュアな環境にデプロイされる。
* **主要コンポーネントとデータフロー**:
  * **要件定義・解析エージェント**: 法令文書や業務要件を入力とし、既存システムの仕様書やコードと突合して改修箇所を特定する。
  * **実装エージェント**: 解析結果に基づき、設計書やソースコードの修正を自律的に行う。
  * **テスト・品質保証エージェント**: 「Multi-layer Quality Control」により、実装されたコードに対してテストシナリオを自動生成し、結合テストを実行・監査する。
  * **データ連携**: これらのエージェントは、アセット化された「AI-Ready」なドキュメント・コードベースを介して情報を共有・更新する。

```mermaid
graph TD
    A[ユーザー/システム管理者] -->|要件・法令文書等を入力| B(要件定義・解析エージェント)
    B -->|改修要件の抽出・特定| C(実装エージェント)
    C -->|コード修正・設計書更新| D(テスト・品質保証エージェント)
    D -->|品質監査・テスト実行| E{要件適合?}
    E -->|No| C
    E -->|Yes| F[完了・出力]

    subgraph AI-Ready Engineering基盤
    G[(既存資産: コード, 仕様書)] -.->|解析・再構築| H[(AI可読アセット)]
    H -.-> B
    H -.-> C
    H -.-> D
    end
...
* **特筆すべき要素技術**:
  * **Multi-layer Quality Control**: AI自身が品質基準に基づき、客観的かつ複数層にわたってコードや設計を監査・修正する自己修復メカニズム。
  * **AI-Ready Engineering**: 暗黙知を含む既存のレガシー資産を、LLMが理解しやすいMarkdown等の形式知へと自動・半自動で再構築する技術。
...
|:---:|:---|:---:|:---:|:---:|:---:|
| **基本機能** | 自律的な全工程実行 | ◎<br><small>要件定義からテストまで協調実行</small> | ◎<br><small>計画からデプロイまで自律実行</small> | △<br><small>Agent機能はあるが支援寄り</small> | ◎<br><small>Agent Platformによる自律実行</small> |
...
| **非機能要件** | 日本語・ドメイン知識特化 | ◎<br><small>国産LLM「Takane」の活用</small> | △<br><small>基本英語</small> | ◎<br><small>対応済み</small> | ◯<br><small>標準的な対応</small> |
...
| **Devin** | 完全自律型のAIソフトウェアエンジニア | 計画からデプロイまで一貫して実行可能。独自の統合開発環境を持つ。 | 複雑なタスクの成功率が低い。コストが高め。日本語対応に課題がある。 | 明確に定義されたプロトタイプ開発や自動化タスクを委任したい場合。 |
| **GitHub Copilot** | GitHubエコシステム全体をカバーする業界標準のコーディング支援AIアシスタント | 圧倒的なモデル選択肢(GPT-5, Claude 4.5等)、IDE統合が強力、無料プランあり。 | タスクの計画や自律的なデバッグ能力はDevin等に比べ限定的。 | 開発者がコーディング作業の生産性を向上させたい場合。最先端のモデルを使いたい場合。 |
| **GitLab Duo** | GitLabのDevSecOpsプラットフォームに統合されたAI | CI/CDやセキュリティを含むライフサイクル全体をカバー。Agent Platformによる自律化とオープンソースモデルのセルフホスト対応を追加。 | GitLab環境への依存。 | GitLabをメインのDevOps基盤として利用している場合。セキュリティ重視。 |
