...
tags:
  ...
  - 開発者ツール
  - DevOps
description: GitHubやJira、AI開発ツールと連携し、「Speed・Quality・Control」の3軸で開発資本を可視化・分析するプラットフォーム
quick_summary:
  has_free_plan: false
  is_oss: false
  ...
  target_users:
    ...
    - エンジニアリングマネージャー
    - 開発チームリーダー
  latest_highlight: 2026年7月1日に新機能「開発資本スコア（α版）」を提供開始
  update_frequency: 高
evaluation:
  score: 82
  base_score: 70
  plus_points:
    - point: 5
      reason: 開発資本（Engineering Capital）の概念に基づくSpeed・Quality・Controlの3軸による可視化
    - point: 5
      reason: AI利用状況やAI効果レポートなど、AI駆動開発に対する強力な分析機能を搭載
    - point: 5
      reason: 国内発のサービスであり、日本語のUI・サポートが完全にネイティブ
  minus_points:
    - point: -3
      reason: 料金プランが公開されておらず、問い合わせが必要
  summary: 日本のエンジニア組織に特化した強力な分析ツール。AI時代の開発資本の可視化から改善までをワンストップで支援する。
links:
...
* **カテゴリ**: 開発者生産性分析
* **概要**: GitHub、GitLab、Jiraなどの開発ツールやAI支援ツールと連携し、エンジニアリング組織の生産性やパフォーマンスを自動で可視化・分析するAI時代の開発資本プラットフォーム。「Four Keys」や「SPACE」フレームワーク、「開発資本（Speed・Quality・Controlの3軸）」といった指標を用い、チームの振り返りやプロセス改善を支援する。
...
* **開発資本スコア**: 組織の状態をSpeed・Quality・Controlの3軸でスコア化し、開発組織の強みと課題を一目で把握可能。
* **AI利用レポート**: チームやメンバーごとのAI利用率やAIを活用したプルリクエストの割合を可視化。
* **AI効果レポート**: AIツールの利用有無やツール別のサイクルタイム、マージされたPR数を比較し、AI投資の意思決定を支援。
* **GitHub Copilot レポート**: GitHub Copilotの利用状況（使用頻度など）を分析し、AI支援開発の効果を測定。

## **4. 動作原理・システム構成**

* **アーキテクチャ**: SaaS型クラウドサービス（API連携によるデータ収集・分析プラットフォーム）
* **主要コンポーネントとデータフロー**:
  * GitHub、GitLab、Jiraなどの外部ツールやAI開発支援ツールからWebhookまたはAPI経由でイベントデータ（コミット、PR、レビュー、イシュー状態変更など）を定期的に収集。
  * 収集したデータをFindy Team+のバックエンドで正規化・集計し、「Speed・Quality・Control」の3軸で開発組織の状態を定量化。
  * ダッシュボードフロントエンドがAPIを介して集計データを取得し、視覚化する。
...
* **特筆すべき要素技術**:
  * **OAuth/アクセストークン**: 外部ツールとのセキュアな連携。
  * **データマスキング**: ソースコードの本文は取得せず、メタデータのみを扱うことでセキュリティリスクを低減。
...
* **外部サービス連携**:
  * **AI開発支援ツール**: GitHub Copilot, Claude Code, Codex, Amazon Q, Cursor, Cline, Devin, Gemini CLI, Antigravity, Kiro, Jules, Windsurf, Junie など
  * **ソースコード管理**: GitHub (Cloud/Self-hosted), GitLab (Cloud/Self-managed), Bitbucket Cloud, Backlog, Azure DevOps Repos
  * **プロジェクト管理**: Jira (Cloud/Data Center), Redmine, GitHub Projects, Backlog, Notion, Asana
  * **コミュニケーション**: Slack
  * **カレンダー**: Google Calendar (ミーティング分析用)
  * **クラウドマーケットプレイス**: AWS Marketplaceからの調達にも対応
...
* **特徴的なユースケース**:
  * レトロスペクティブでダッシュボードを画面共有し、事実（データ）に基づいて議論する利用や、1on1での客観的なフィードバックとしての活用。

## **16. 直近半年のアップデート情報**

* **2026-07-13**: プロデリア・パートナーズと協業し、「Findy Insights」と実践型人材育成プログラムの一体提供を通じ、エンタープライズ企業のプロダクトグロース支援を開始。
* **2026-07-01**: AI時代の開発力をSpeed・Quality・Controlの3指標で測る新機能「開発資本スコア（α版）」を提供開始。
* **2026-06-23**: 「AWS Marketplace」で「Findy Team+」の提供を開始。AWSとの連携強化により、エンタープライズ企業への導入拡大とグローバル展開を加速。
...
(出典: [ファインディ株式会社 ニュース](https://findy.co.jp/news/))

## **17. 類似ツールとの比較**

### **17.1 機能比較表 (星取表)**

| 機能カテゴリ | 機能項目 | 本ツール | LinearB | Sleuth | Jellyfish |
|:---:|:---|:---:|:---:|:---:|:---:|
| **分析指標** | 開発資本スコア / AIメトリクス | ◎<br><small>Speed/Quality/Controlの3軸スコア</small> | ◯<br><small>生成AI影響の分析あり</small> | △<br><small>DORA特化</small> | ◯<br><small>対応可能</small> |
...
|---------|------|------|------|------------------|
| **本ツール** | AI時代の開発資本プラットフォーム | 完全日本語対応、豊富な国内事例、AWS Marketplaceでの提供 | 海外製ツールに比べるとグローバルな連携エコシステムが限定的な場合がある | - |
