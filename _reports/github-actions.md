...
quick_summary:
  ...
  target_users:
    ...
    - DevOpsエンジニア
    - スタートアップ
  latest_highlight: 2026年3月に Scheduled Workflows のタイムゾーン指定に対応
  update_frequency: 高
evaluation:
  score: 75
  base_score: 70
...
* **関連リンク**:
  ...
  * レビューサイト: [ITreview](https://www.itreview.jp/products/github-actions/reviews)
* **カテゴリ**: CI/CD / CD, 自動化
...
* **前提条件**:
  * GitHubアカウントと対象となるリポジトリ（パブリックまたはプライベート）
  * 特別なランタイムは不要（各ジョブの実行環境でセットアップ可能）
* **インストール/導入**:

  ```bash
  # リポジトリのルートディレクトリにワークフロー定義ファイルを作成する
...
* **調査対象**: Google検索結果 (G2, DevOpsSchool, 技術ブログ等からの引用、2025年時点の評価)
* **総合評価**: 4.7/5.0 (G2より引用) - 非常に高い顧客満足度を維持。
* **ポジティブな評価**:
...
* **2026-05-XX**: **イメージ移行と同時実行キュー拡張**: Concurrency groups（同時実行グループ）がより大規模なキューに対応し、大容量ワークフローでの待ち時間が改善。また、ホステッドランナー画像の定期マイグレーションがアナウンス。
* **2026-04-XX**: **サービスコンテナのオーバーライド**: ワークフロー内で定義するサービスコンテナの `entrypoint` や `command` を、Docker Composeと同様の構文でオーバーライド可能に。
* **2026-04-XX**: **OIDCカスタムクレーム対応**: OIDCトークン内にリポジトリのカスタムプロパティをクレームとして含めることができるようになり、より厳格で細粒度の信頼ポリシーをクラウド連携で適用可能に。
* **2026-04-XX**: **再実行回数の上限設定**: Actions ワークフローの再実行（Rerun）の回数制限が最大50回に設定。
* **2026-03-XX**: **スケジュール実行（Cron）のタイムゾーンサポート**: `schedule` トリガーにおいて、これまでのUTC限定からIANAタイムゾーン（例: `Asia/Tokyo`）の直接指定に対応。
* **2026-03-XX**: **Environmentsの柔軟な利用**: デプロイメント履歴を自動作成することなく、シークレットや変数の管理目的だけでワークフロー環境（Environments）を利用可能に。
* **2026-02-XX**: **スタンドアロン型自動スケーリング (Public Preview)**: Kubernetes環境に依存せずカスタムランナーを自動スケールさせる、Go言語ベース of 「Runner Scale Set Client」がパブリックプレビューに。
* **2026-01-XX**: **ワークフロー構文の強化 (`case` 関数の導入)**: 条件分岐などのロジックをよりシンプルかつ記述しやすくするための `case` 関数が式評価エンジンに追加。
* **2026-01-01**: **ホステッドランナーの料金改定**: GitHubホステッドランナーの料金体系が改定され、最大39%の大幅な値下げが実施。

(出典: [GitHub Changelog](https://github.blog/changelog/label/actions/))
