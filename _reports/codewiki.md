...
* **調査対象**: InfoQ, Reddit, YouTubeコメント（G2, Capterra, ITreviewには2026年5月現在レビュー登録なし）
...
* **ポジティブな評価**:
  ...
  * 「Googleの強力なインフラを背景にしているため、大規模なコードベースでも高速に処理される。」
* **ネガティブな評価 / 改善要望**:
  * 「CLIがファイルを編集する前に読み込むなら、わざわざ外部ツールを使う意味があるのか？（エディタ統合してほしい）」
  * 「生成されたドキュメントをPDFやMarkdownとしてエクスポートしたい。」
  * 「プライベートリポジトリの対応がまだ十分ではなく、企業での本格的な導入にはセキュリティ上の懸念がある。」
* **特徴的なユースケース**:
  * 大規模なレガシーコードのオンボーディングにおいて、新入社員がシステムの全体像を迅速に把握するための学習ツールとしての利用。
