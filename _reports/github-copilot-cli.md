...
quick_summary:
  ...
  target_users:
    - 開発者
    - DevOpsエンジニア
  latest_highlight: Claude Fable 5モデルのサポートやUI/UXの改善など多数のアップデートを実施
  update_frequency: 高
evaluation:
  score: 82
  base_score: 70
...
* **モデル選択**: Claude 4.5 Opus/Sonnet や Claude Fable 5 など、最新のモデルをサポートしています。
* **プラグイン拡張**: プラグインが拡張機能を同梱できるようになり、マーケットプレイスから直接インストール可能になりました。
* **高度なコマンドとUI**: 自然言語での音声入力（`/voice`）や、アプリ起動（`/app`）に対応。Diffビューでの検索・ハイライト、インタラクティブな設定画面（`/settings`）も提供されています。
...
* **2026-06-13**: (v1.0.62) プラグインの拡張機能サポート、`/app` コマンドの追加、Diffビューでの検索・ハイライト機能が導入されました。
* **2026-06-09**: (v1.0.61) インタラクティブな設定ダイアログ (`/settings`) の追加、`Claude Fable 5` モデルのサポート、MCPサーバーの自動ロード機能が追加されました。
* **2026-06-05**: (v1.0.60) Anthropicモデルの reasoning effort レベルの追加や、UI/UXの改善が行われました。
* **2026-06-02**: (v1.0.59 / v1.0.58) 音声入力コマンド (`/voice`) の追加、および Rubber Duck（ラバーダック）エージェントがデフォルトで有効になりました。
...
|:---:|:---|:---:|:---:|:---:|:---:|
| **基本機能** | コマンド生成・実行 | ◎<br><small>対話式で安全に実行確認可能</small> | ◎<br><small>Claude 4.5を使用し自律的に実行</small> | ◎<br><small>ターミナル一体型でシームレス</small> | ◯<br><small>CLI上でのインライン補完</small> |
...
| **拡張性** | MCP対応 | ◎<br><small>標準対応・自動ロード機能等</small> | ◎<br><small>標準対応</small> | ×<br><small>非対応</small> | ×<br><small>非対応</small> |
...
| **Claude Code** | Anthropicが提供する自律型CLIコーディングエージェント。 | Claude 4.5モデルの高度な推論力と自律性。 | 実行ごとにAPIコストが発生する。 | より自律的に大規模なリファクタリングや機能追加を任せたい場合。 |
