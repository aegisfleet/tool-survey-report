...
Cucumberはコミュニティ主導となってからも、各言語実装（`cucumber-js`, `cucumber-jvm`など）で活発な開発が継続されています。以下は、直近の主なアップデートです（2026年1月時点）。

*   **全般的な傾向**:
    *   内部実装が、よりメッセージベースのアーキテクチャに移行しており、フォーマッターやレポーティング機能の柔軟性と拡張性が向上しています。
    *   依存ライブラリは継続的に最新版に更新され、セキュリティとパフォーマンスの向上が図られています。

*   **Cucumber-JVM (v7.31.0 - v7.33.0)**:
    *   **機能追加**: `Scenario.getLanguage()`が追加され、現在のシナリオの言語設定を取得できるようになりました(`v7.33.0`)。また、`UsageJsonFormatter`が追加されました(`v7.31.0`)。
    *   **レポート機能の改善**: Cucumber ReportsがGzipエンコーディングでアップロードされるようになり、効率化が図られました。また、JSONレポートに空のシナリオや背景が含まれるように修正されました(`v7.33.0`)。
    *   **依存関係の更新**: Gherkinパーサーや各種フォーマッター（HTML, JUnit-XMLなど）の依存関係が定期的に更新されています(`v7.32.0`)。
