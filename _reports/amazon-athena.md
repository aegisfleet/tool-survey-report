...
quick_summary:
  ...
  target_users:
    ...
    - データアナリスト
    - 開発者
  latest_highlight: 2026年4月に12のデータソースへのマネージドコネクタを提供開始し、より手軽なフェデレーテッドクエリを実現
  update_frequency: 中
evaluation:
  score: 85
  base_score: 70
...
* **マネージドコネクタ (フェデレーテッドクエリ)**: DynamoDBやSnowflakeなど主要な外部データソースに対して、LambdaやVPCなどのインフラ構築・管理不要で即座に連携できるマネージドコネクタが利用可能。

## **4. 動作原理・システム構成**

* **アーキテクチャ**: クラウド完結型のサーバーレスアーキテクチャ
* **主要コンポーネントとデータフロー**:
  * ユーザーがマネジメントコンソールやBIツールなどからSQLクエリを送信する。
  * AthenaはAWS Glue Data Catalogを参照してテーブルのメタデータやスキーマ情報を取得する。
  * サーバーレスな分散クエリエンジン（Presto/Trinoベース）がS3やフェデレーテッドソースから必要なデータを透過的にスキャン・集計する。
  * 実行結果は指定したS3バケットに出力され、ユーザーに返される。

```mermaid
flowchart LR
    User[ユーザー / BIツール] -->|SQLクエリ| Athena(Amazon Athena)
    Athena -->|メタデータ参照| Glue[(AWS Glue Data Catalog)]
    Athena <-->|データスキャン| S3[(Amazon S3 データレイク)]
    Athena <-->|Lambda経由| DB[(外部データソース)]
    Athena -->|結果保存| S3Out[(S3 結果バケット)]
```

* **特筆すべき要素技術**:
  * **Presto/Trino**: 大規模データを高速に分散処理するためのオープンソースSQLクエリエンジンを採用。
  * **AWS Lambda**: フェデレーテッドクエリではLambda関数をコネクタとして利用し、多様なデータソースにアクセスする。
...
| **プロビジョンドキャパシティ** | $0.30/DPU時間 | 必要なコンピュートリソース（DPU）を予約。最小4 DPU、最短1分からの細かな設定が可能となり、ワークロードに応じた柔軟なコスト最適化ができる。 |
...
* **認証**: AWS Identity and Access Management (IAM) ポリシーを使用して、Athenaへのアクセスおよび基盤となるS3バケット内のデータへのアクセスを詳細に制御可能。Amazon S3 TablesやApache Icebergマテリアライズドビューに対するIAMベースの認可もサポート。
...
* **2026-07-06**: ODBCドライバー v2.2.0.1 をリリース。
* **2026-07-01**: Amazon SageMaker Unified Studioから共有されたテーブルクエリに関するバグ修正。
* **2026-06-03**: JDBCドライバー v3.8.0 をリリース。
* **2026-05-12**: JDBC 2.x ドライバーにおけるサードパーティライブラリおよびセキュリティのアップデート。
* **2026-04-21**: Amazon DynamoDB、PostgreSQL、MySQL、Snowflakeなど12のデータソースに対して、インフラ管理不要の**マネージドコネクタ**を提供開始。
* **2026-03-27**: Capacity Reservationsがさらに多くのリージョン（東京リージョン含む多数）で利用可能に。
* **2026-03-20**: 認証フローやクエリ処理のセキュリティを向上させたODBCドライバー v2.1.0.0 をリリース。
* **2026-03-17**: Amazon S3 TablesとApache Icebergマテリアライズドビューに対するIAMベースの認可をサポート。
* **2026-02-10**: 最小4 DPU、最短1分から設定可能な Capacity Reservations をサポート。より柔軟なコスト管理とクエリ優先度の制御が可能に。
* **2026-01-12**: アジアパシフィック（ニュージーランド）リージョンでの提供開始。
* **2024-11-XX**: 次世代のAmazon SageMaker (Amazon SageMaker Unified Studio) との統合が発表され、データレイクなどの接続されたソースからのデータをより直感的なクエリエディタで分析可能になった。
* **2023-11-XX**: Capacity Reservationsが一般利用可能になり、プロビジョンドキャパシティ（DPU時間ベース）での料金プランが選択可能になった。

(出典: [Amazon Athena Release Notes](https://docs.aws.amazon.com/athena/latest/ug/release-notes.html))
