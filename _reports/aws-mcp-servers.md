...
quick_summary:
  ...
  target_users:
    ...
    - 開発者
    - AIエンジニア
  latest_highlight: CloudFormation, Bedrockに加え、Billing/Cost ManagementやAurora DSQLサポートなど新機能が多数追加
  update_frequency: 高
evaluation:
  score: 83
  base_score: 70
...
* **AWS Billing and Cost Management MCP Server**: AWSの請求・コスト情報の取得、Compute OptimizerやSavings Plansの推奨事項の取得。
* **Aurora DSQL MCP Server**: DSQLクラスターの管理と対話的なSQLクエリの実行。

## **4. 動作原理・システム構成**

* **アーキテクチャ**: ローカルファーストのクライアント・サーバー構成（MCPプロトコルを利用）。
* **主要コンポーネントとデータフロー**:
  * クライアント（Claude Desktop、Cursorなど）がユーザーの自然言語指示を受け取り、MCPプロトコルを通じてローカルのAWS MCP Serversにリクエストを送信する。
  * サーバーはAWS SDKを利用し、環境変数や`~/.aws/credentials`に設定された認証情報を用いて対象のAWSサービス（Bedrock, CloudFormationなど）APIを呼び出す。
* **特筆すべき要素技術**:
  * MCP (Model Context Protocol) を用いた標準化されたAI・ツール間通信。
  * Human-in-the-loop: デプロイやリソース変更などの副作用がある操作は、クライアント側でユーザーの承認（Approve）を求めることで安全性を担保。

```mermaid
graph LR
    A[AI Client<br>Claude/Cursor] <-->|MCP Protocol| B(AWS MCP Servers)
    B <-->|AWS SDK| C((AWS Cloud))
    C --- D[Amazon Bedrock]
    C --- E[AWS CloudFormation]
...
* **2026-09-08**: Billing/Cost ManagementサーバーでRI/SPツールの修正、Aurora DSQLでの外部キーサポートなどを追加。
* **2026-09-01**: MCP SDK v2への移行、Billing/Cost ManagementにCompute Optimizer推奨事項取得を追加。
* **2026-08-31**: Postgres/MySQLサーバーのセキュリティ強化、Timestream InfluxDB v2書き込み対応などを追加。
...
| 機能カテゴリ | 機能項目 | 本ツール | AWS CLI | AWS CloudFormation |
|:---:|:---|:---:|:---:|:---:|
| **インターフェース** | 自然言語操作 | ◎<br><small>AI経由で直感的に操作可能</small> | ×<br><small>コマンドの記憶が必要</small> | ×<br><small>YAML/JSONの記述が必要</small> |
| **情報取得** | ドキュメント統合 | ◎<br><small>即座に検索しコンテキスト化</small> | ×<br><small>対応していない</small> | ×<br><small>ドキュメント統合はなし</small> |
| **自動化** | 複数手順の実行 | ◎<br><small>AIが推論して一括実行</small> | ◯<br><small>シェルスクリプト化が必要</small> | ◯<br><small>スタック単位で自動デプロイ</small> |
...
| **AWS CloudFormation** | Infrastructure as Code | 宣言的なテンプレートでインフラを再現可能。 | テンプレート作成の学習コストが高い。 | 大規模なインフラをコードで厳密に管理・運用する場合。 |
