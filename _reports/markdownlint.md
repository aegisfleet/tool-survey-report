...
quick_summary:
  ...
  target_users:
    - 開発者
    - テクニカルライター
  latest_highlight: v0.41.0でのルール改善やNode 20サポート終了などの継続的な機能改善
  update_frequency: 中
evaluation:
  score: 85
  base_score: 70
...
* **アーキテクチャ**: ローカル実行型のCLI / Node.jsライブラリ
* **主要コンポーネントとデータフロー**:
  * Markdown入力 -> micromarkパーサー -> 構文木解析・ルール検証 (markdownlint) -> 警告/エラー出力・自動修正
* **特筆すべき要素技術**:
  * **micromark**: Markdownを解析するための軽量で高速なパーサー。
  * **CommonMark / GFM**: 標準のCommonMark仕様に加え、GitHub Flavored Markdown (GFM) のテーブルや自動リンク等にも対応。

```mermaid
flowchart LR
  A[Markdownファイル] --> B(micromarkパーサー)
  B --> C{ルール検証}
  C -->|違反あり| D[警告・エラー表示]
  C -->|Auto-fix| E[自動修正反映]
```
...
* **2026-07-13**: v0.41.1リリース。MD029ルールの改善やwebpack環境でのモジュール解決の問題を修正。
* **2026-06-10**: v0.41.0リリース。非推奨API（resultVersion, toString）の削除、Node.js 20のサポート終了、MD022などのルール改善。
...
| 機能カテゴリ | 機能項目 | markdownlint (v0.41.1) | Prettier (v3.9.x) | remark-lint (v4/5) |
|:---:|:---|:---:|:---:|:---:|
