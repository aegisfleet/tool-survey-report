...
quick_summary:
  ...
  target_users:
    ...
    - OSINTリサーチャー
    - ペネトレーションテスター
  latest_highlight: 2026年5月のアップデートで脆弱性の修正や新規サイトの追加を実施
  update_frequency: 中
evaluation:
  score: 83
  base_score: 70
...
* **アーキテクチャ**: ローカルファーストのCLI（クライアント）ツール。ユーザーのマシンから各プラットフォームの公開エンドポイントへ直接リクエストを送信する。
* **主要コンポーネントとデータフロー**:
  * ユーザーがCLIで指定したユーザー名を受け取り、内部の巨大なサイト設定リスト（JSON形式等）に基づいて検索タスクを生成。
  * `asyncio` や `concurrent.futures` などを活用し、非同期で各サイトのプロフィールURL（例: `https://twitter.com/[ユーザー名]`）に対してHTTPリクエストを発行する。
  * HTTPステータスコード（200 OK や 404 Not Found など）やページ内の特定文字列を解析し、アカウントの存在有無を判定する。
  * 結果をターミナルに表示するとともに、指定フォーマット（CSV等）でローカルファイルに保存する。
* **特筆すべき要素技術**:
  * **非同期処理**: 大量のリクエストを効率的に処理し、全体の検索時間を短縮。
  * **Torネットワーク連携**: 匿名性確保やIPブロック回避のため、Tor経由でリクエストをルーティングする機能（現在はオプショナル）。
...
* **2026-05-05**: Cracked Forum等の誤検知（False Positives）の修正および新規サイトの追加。
* **2026-05-02**: 脆弱性の修正（fix-vuln）とバージョンの更新。
...
(出典: [GitHub Commits](https://github.com/sherlock-project/sherlock/commits/master))
