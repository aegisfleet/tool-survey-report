...
quick_summary:
  ...
  target_users:
    - 開発者
    - セキュリティエンジニア
  latest_highlight: 共有物理ページの同時マッピング対応や、LVBS/HEKIの検証の強化などが行われました
  update_frequency: 高
evaluation:
  score: 80
  base_score: 70
...
* **アーキテクチャ**: ホストと分離されたサンドボックス型のライブラリOS。ホストOS（WindowsやLinuxなど）の上で、ゲストアプリケーションの実行環境として動作。
* **主要コンポーネントとデータフロー**:
  * North/Southインターフェースを通じてゲストとホストの通信を仲介。
* **特筆すべき要素技術**:
  * メモリ安全性を担保するためのRust言語での実装。
  * `vmap`等を用いた共有物理ページの分離・同時マッピング（2026年7月追加）など高度なページ分離機能。
...
* **2026-07-17**: 共有物理ページの分離・同時マッピングへの対応（`vmap`の利用）により、同時マッピング拒否の課題を解決。
* **2026-07-17**: LVBS/HEKIのモジュールおよびkexec検証を、TOCTOUや混乱した代理人攻撃（Confused Deputy）に対して保護を強化。
* **2026-02-05**: MicrosoftがLiteBoxをオープンソース（MITライセンス）としてGitHubに公開。

(出典: [LiteBox リポジトリ更新履歴](https://github.com/microsoft/litebox/commits/main/))
