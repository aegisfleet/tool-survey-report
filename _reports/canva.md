...
* **前提条件**:
  * Webブラウザ（Chrome, Safari, Edgeなど）またはデスクトップ/モバイルアプリ。
  * アカウント作成（Google, Facebook, Email連携可）。
* **インストール/導入**:
  SaaS型のためインストール不要（デスクトップ版を利用する場合は公式サイトからDL）。
  ```bash
  # 特になし（ブラウザで https://www.canva.com/ にアクセス）
  ```
* **初期設定**:
  * アカウント作成後、利用目的（個人、小規模ビジネス、大企業、教育など）を選択すると最適なテンプレートが提案される。
  * Pro/Teamsユーザーは「ブランドキット」にロゴ、フォント、カラーパレットを登録。
* **クイックスタート**:
  * 「デザインを作成」ボタンをクリックし、作りたい形式（Instagram投稿、プレゼンテーションなど）を選択。
  * テンプレートを選んでテキストや画像を差し替えるだけで完成。
