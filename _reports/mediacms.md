...
quick_summary:
  ...
  target_users:
    ...
    - 企業内ポータル
    - メディア組織
  latest_highlight: 2026年8月にv8.4.0をリリース。字幕エラーの修正（v8.3.5）などを実施。
  update_frequency: 中
evaluation:
  score: 82
  base_score: 70
...
MediaCMSは、バックエンドにDjango、フロントエンドにReactを採用したクライアント・サーバー型の動画CMSです。

* **アーキテクチャ**: クライアント・サーバー型 (Django + React)
* **主要コンポーネントとデータフロー**:
  * **フロントエンド**: Reactで構築されたSPAがユーザーの操作を受け付けます。
  * **バックエンド**: Django (Python) がAPIを提供し、データの保存や認証を行います。
  * **動画配信**: HLS (HTTP Live Streaming) を使用して動画をストリーミング配信します。
* **特筆すべき要素技術**:
  * **HLS**: 環境に応じたアダプティブストリーミングを実現します。

```mermaid
graph TD
    Client[クライアント/ブラウザ] -->|APIリクエスト| React[React フロントエンド]
    React -->|データ操作| Django[Django バックエンド]
    Django -->|動画メタデータ| DB[(データベース)]
    Django -->|ストリーミング| HLS[HLS動画配信]
    HLS -->|動画再生| Client
```
...
* **2026-08-25 (v8.4.0)**: リリーストリガーの追加（機能強化）。
* **2026-08-25 (v8.3.5)**: 字幕（subtitles）に関するエラーを修正。
* **2026-07-15 (v8.3.4)**: `shutil`の使用に関するバグ修正。
* **2026-07-08 (v8.3.3)**: プライベートメディアおよびプレイリストに関するバグを修正。
...
(出典: [CHANGELOG](https://github.com/mediacms-io/mediacms/blob/main/CHANGELOG.md))

## **17. 類似ツールとの比較**

### **17.1 機能比較表 (星取表)**

| 機能カテゴリ | 機能項目 | MediaCMS | WordPress |
|:---:|:---|:---:|:---:|
| **基本機能** | 動画配信・再生 | ◎<br><small>動画に特化、HLS対応</small> | ◯<br><small>標準で動画アップロード可、専用プラグインで強化</small> |
| **アーキテクチャ** | 構成 | 集中型<br><small>Django+React</small> | 集中型<br><small>PHP+MySQL</small> |
| **エンタープライズ** | 権限管理 (RBAC) | ◎<br><small>詳細な権限設定可</small> | ◯<br><small>標準の権限管理＋プラグイン</small> |
| **コスト** | ライセンス/料金 | ◎<br><small>無料 (OSS)</small> | ◎<br><small>無料 (OSS)</small> |
| **非機能要件** | データ主権 | ◎<br><small>完全自社管理</small> | ◎<br><small>完全自社管理</small> |

### **17.2 詳細比較**

| ツール名 | 特徴 | 強み | 弱み | 選択肢となるケース |
|---------|------|------|------|------------------|
| **MediaCMS** | モダンなWeb技術(Django/React)で作られた動画特化のCMS。 | UIがYouTubeライクで親しみやすく、動画専用の機能が豊富。 | 大規模配信にはインフラの増強・チューニングが必要。 | **動画ポータルサイトを独立して構築し、安全に管理したい場合。** |
| **WordPress** | 世界トップシェアの汎用CMS。 | エコシステムが巨大で、記事と組み合わせたメディア構築に強い。 | 大量・大容量の動画配信には外部ストレージ等の工夫が必要。 | 記事コンテンツを中心に、動画も一部扱いたい場合。 |
...
* **選択時のポイント**:
  * **WordPressとの比較**: 動画のみのポータルならMediaCMS。テキスト記事と併用するメディア構築ならWordPressが適しています。
