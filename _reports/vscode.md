...
VS Codeは毎月定期的にアップデートがリリースされており、常に新機能の追加や改善が行われています。直近のアップデートでは、特にAI機能の強化と、法人利用におけるセキュリティ管理機能の向上が中心となっています。

* **2025年12月 (v1.108)**
  * **VS Code Insiders Podcast**: VS Codeの舞台裏や新機能、開発者に焦点を当てた公式ポッドキャストが開始されました。

* **2025年11月 (v1.107)**
  * **Private Marketplace**: GitHub Enterpriseユーザー向けに、組織内で承認された拡張機能のみを配布・管理できるプライベートなマーケットプレイス機能が追加されました。これにより、セキュリティとコンプライアンスを強化できます。
  * **AIエージェントの統合体験**: サイドバーに「Agent Sessions」ビューが新設され、ローカル・クラウド上で実行されている複数のAIコーディングエージェント（GitHub Copilot, OpenAI Codex (CLI)など）の状態を一元的に管理・操作できるようになりました。
  * **Planning Agent**: 曖昧な指示からタスクの実行計画を対話形式で具体化してくれる「Plan」エージェントが組み込まれました。

* **2025年10月 (v1.106)**
  * **モデル選択の拡張 (BYOK)**: `Language Model Chat Provider API` が公開され、拡張機能を通じてサードパーティ製の言語モデル（Hugging Face, Cerebras提供のモデルなど）をVS Codeのチャット機能で利用できるようになりました。APIキーを設定するだけで、より多くのモデルを試せます。
...
* **2025年8月 (v1.104)**
  * **Auto Model Selection**: ユーザーのCopilotプランに応じて、最適なパフォーマンスを発揮するモデルを自動で選択する機能がプレビューとして導入されました。
