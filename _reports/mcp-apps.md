...
quick_summary:
  ...
  target_users:
    - 開発者
    - AIエンジニア
  latest_highlight: 'v1.7.0リリースによるホストへのツール公開機能と厳格なCSP（JITレス）対応の追加'
  update_frequency: 高
evaluation:
  score: 85
  base_score: 70
...
* **ホストへのツール公開**: アプリ (View) 側からホスト（クライアント）に対してツールを公開し、呼び出してもらう機能（`App.registerTool()`）をサポート。
...
* **実行環境の保護**: デフォルトで厳密なCSP (Content Security Policy) が適用され、`unsafe-eval` を許可しない（JITレス）環境で実行されるため、安全性が高い（`AppOptions.allowUnsafeEval`でオプトアウト可能）。
...
* **効果的な活用法 (Modern Practices)**:
  ...
  * **ハンドシェイクの順序**: ホストメソッドの呼び出しは、`connect()` が完了してから行うこと。また、イベントハンドラーも接続前に登録しておくのが安全である（`AppOptions.strict` で順序違反をエラーにできる）。
* **陥りやすい罠 (Antipatterns)**:
...
* **2026-06-05**: v1.7.4リリース。セキュリティパッチ適用（vulnerabilities 0件化）と、lazy-auth-server サンプルの改善（PKCE必須化等）。
* **2026-05-15**: v1.7.2リリース。pdf-server サンプルの依存関係（@cantoo/pdf-lib等）をメンテナンス版へ移行。
* **2026-04-27**: v1.7.1リリース。pdf-server の遅延フォーム抽出やキャッシュ共有機能の強化。
* **2026-04-21**: v1.7.0リリース。アプリからホストへのツール公開 (`App.registerTool()`)、デフォルトでの厳格なCSP（JITレス）適用、ハンドシェイク順序の厳密化ガード追加など、多数の機能追加と安定性向上。
* **2024-12-18**: MCP Appsの仕様公開とSDKのリリース。
* **2024-12-18**: Claude DesktopにおけるMCP Appsサポートのプレビュー開始。
* **2024-12-18**: GitHubリポジトリにてReact, Vue, Svelteなどのサンプルコード拡充。

(出典: [GitHub Releases](https://github.com/modelcontextprotocol/ext-apps/releases), [Model Context Protocol Blog](https://blog.modelcontextprotocol.io/))
...
| **ChatGPT (Canvas)** | OpenAIのチャット画面横に表示されるエディタ/キャンバス | GPT-5.5/GPT-5.4 Thinking等の最新モデルによる、ライティングやコーディングに特化したシームレスな体験。 | 開発者が自由にUIを拡張・自作することはできない。 | ChatGPTの標準機能として文章作成やコーディング支援を受けたい場合。 |
