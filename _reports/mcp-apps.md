...
quick_summary:
  ...
  target_users:
    - 開発者
    - AIエンジニア
  latest_highlight: サンプリング機能の追加やホストへのツール提供など、SDK APIが継続的に拡張
  update_frequency: 高
evaluation:
  score: 85
  base_score: 70
...
* **ホストへのツール提供**: Viewsからホスト側で実行可能なツール（WebMCPスタイル）を公開し、ホストに呼び出させることができる。
* **サンプリング機能の利用**: `App.createSamplingMessage()` を利用することで、SDKのネイティブなサンプリング機能を実行可能。
...
* **実行環境の制限 (CSP)**: デフォルトで `unsafe-eval` を無効化した厳格なCSP（コンテンツセキュリティポリシー）下で実行され、JITによるコード実行を防ぐ。
...
* **陥りやすい罠 (Antipatterns)**:
  ...
  * **JIT実行への依存**: デフォルト設定では `unsafe-eval` が無効化されているため、実行時にコードを評価するライブラリの使用は避け、ビルド時に処理する。
...
* **2026-06-05**: バージョン1.7.4リリース。lazy-auth-serverへのPKCE対応追加やセキュリティ更新（依存パッケージの脆弱性対応）を実施。
* **2026-05-15**: バージョン1.7.2リリース。PDFサーバーの依存ライブラリを保守中のフォークへ切り替え。
* **2026-04-21**: バージョン1.7.0リリース。`App.registerTool()`によるホストへのツール公開機能、`App.createSamplingMessage()`によるサンプリング機能の追加、およびデフォルトでの `unsafe-eval` を無効化する厳格なCSP設定が導入された。
* **2026-04-14**: バージョン1.6.0リリース。ツール実行におけるプログレスベースのタイムアウトリセットなどに対応。
* **2026-04-02**: バージョン1.5.0 / 1.4.0リリース。PDFサーバーでの注釈読み込み、フルスクリーン時のピンチトゥズーム対応、およびDOMモデルのイベントリスナーAPI拡充。
