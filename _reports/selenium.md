...
quick_summary:
  ...
  target_users:
    - 開発者
    - QAエンジニア
  latest_highlight: 2026年8月にSelenium 4.47.0をリリースし、BiDi APIの機能強化などを実施
  update_frequency: 高
evaluation:
  score: 85
  base_score: 70
...
* **アーキテクチャ**: クライアント・サーバー構成（WebDriverアーキテクチャ）
* **主要コンポーネントとデータフロー**:
  * クライアント側（言語バインディング）からテストスクリプトを実行し、W3C WebDriverプロトコルを介して各ブラウザ固有のドライバ（ChromeDriver, GeckoDriver等）へコマンドを送信する。ドライバがブラウザを直接操作し、結果をスクリプトに返す。
* **特筆すべき要素技術**:
  * **W3C WebDriverプロトコル**: ブラウザ操作の標準プロトコル。
  * **WebDriver BiDi**: WebSocketを用いた双方向通信プロトコル。従来の一方向HTTPコマンドとは異なり、ブラウザからの非同期イベント（コンソールログやネットワークエラーなど）を即座に受信できる。
...
* **2026-08-10**: **Selenium 4.47.0 リリース**
  * 生成されたBiDiモジュールで使用されるトランスポート層の追加、BiDiでのイベント購読パラメータのサポート、およびPython APIのHTMLドキュメント生成の改善などを実施。
* **2026-07-11**: **Selenium 4.46.0 リリース**
  * すべての現在のBiDi関連クラスをベータ版としてマーク。バインディング中立なBiDiスキーマの追加、およびシステムプロキシ設定のSelenium Managerへの対応などを実施。
* **2026-06-16**: **Selenium 4.45.0 リリース**
  * 内部アーキテクチャの継続的な更新と安定性の向上を実施。
...
(出典: [Selenium Releases](https://github.com/SeleniumHQ/selenium/releases))
