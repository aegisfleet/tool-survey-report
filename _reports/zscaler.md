...
quick_summary:
  ...
  target_users:
    ...
    - 中規模企業
    - IT・セキュリティ管理者
  latest_highlight: 2026年1月にAI Security Suite（AI資産管理、MCPゲートウェイ等）を発表
  update_frequency: 高
evaluation:
  score: 88
  base_score: 70
...
* **AI Security Suite**: AIアプリケーションへの安全なアクセスの確保、インテリジェントなAI資産管理（AI Asset Management）、プロンプトの保護、自動化されたAIレッドチーミングなどを統合的に提供。AI DeceptionやMCPゲートウェイによるLLM・エージェント通信の保護も含む。
* **Zero Trust Cloud / Branch**: クラウドワークロードの保護、マイクロセグメンテーション、ゼロトラストSD-WAN（Zero Trust Branch）により、データセンターからクラウド、ブランチ（支社）までの通信を保護。
* **Zscaler Cellular**: IoTデバイスやモバイルエンドポイントなど、セルラー通信を利用するデバイス向けに、セキュアでスケーラブルな接続を提供するサービス。

## **4. 動作原理・システム構成**

* **アーキテクチャ**: グローバルに分散されたデータセンター（Edge）を介して通信を仲介する、クラウドネイティブなインライン型プロキシアーキテクチャ（SSE / SASE）。
* **主要コンポーネントとデータフロー**:
  * エンドポイント（Zscaler Client Connector）から送信されるすべてのトラフィックは、最寄りのZscalerデータセンター（Zero Trust Exchange）に転送される。
  * Zero Trust Exchange内で、SSLインスペクション、マルウェア検査、DLPスキャン、アクセス制御などのセキュリティポリシーが一元的に適用される。
  * 認証・認可された通信のみが、インターネット、SaaS、またはプライベートアプリケーションへとルーティングされる。
* **特筆すべき要素技術**:
  * **Zero Trust Exchange**: VPNのようにネットワークレベルでの接続を許可するのではなく、アプリケーションごとの論理的なトンネルを確立するため、ラテラルムーブメント（横展開）を防止。
...
flowchart LR
    ...
    subgraph ZTE ["Zscaler Zero Trust Exchange (クラウド)"]
        direction TB
        Auth["アクセス制御 / ZTNA"]
        Scan["SSL可視化 / 脅威防御\n(ZIA/ZPA/AI Security)"]
        DLP["データ漏洩防止 (DLP)"]
    ...
    Internet["インターネット / SaaS"]
    PrivateApp["プライベートアプリ\n(データセンター/クラウド)"]

    User -- "SAML認証" --> IdP
    User -- "トラフィック転送" --> ZTE
    ZTE -- "安全な接続" --> Internet
    ZTE -- "ZPAトンネル" --> PrivateApp
```
...
| **OpenAI / Anthropic** | ◎ | 主要なLLMプロバイダーと連携。安全なAI利用を支援し、シャドーAIの検知や情報漏洩を防止するコントロールを提供。 | 特になし。 |
...
* **2026-01-27**: 「Zscaler AI Security Suite」のイノベーションを発表。AIの資産管理、シャドーAIの検知、MCPゲートウェイによるエージェント通信の保護、AIに対するDeception（おとり）機能などを追加し、エンタープライズの安全なAI導入を支援。
* **2026-01-27**: 「ThreatLabz 2026 AI Threat Report」を公開。AIアクティビティが前年比83%増加していることや、シャドーAIによるデータ流出リスク、AIシステムへの攻撃の自動化に関する洞察を提供。
* **2025-11-05**: 「Zscaler ThreatLabz 2025 Mobile, IoT, and OT Threat Report」を公開。Androidマルウェアの67%増加やIoTを狙う攻撃の急増を報告。あわせて「Zscaler Zero Trust Branch」や「Zscaler Cellular」によるエッジデバイス保護の重要性を強調。
...
| **AIセキュリティ** | AI資産管理・シャドーAI制御 | ◎<br><small>AI Security Suiteで包括的な可視化・プロンプト保護を提供。MCPゲートウェイ対応</small> | ◯<br><small>AI Access Securityで可視化・制御を提供</small> | ◯<br><small>SkopeAIによるデータ保護とLLM制御</small> | △<br><small>AI利用の可視化は一部対応</small> |
