...
* **2026-03-09**: バージョン `2.12.0` をリリースし、バグ修正を実施。
* **2026-02-23**: バージョン `2.10.0` をリリース。外部シークレットプロバイダーへの複数接続サポートを追加し、大規模なワークフローと認証情報のボリュームに対するパフォーマンス改善（ロード時間を30%〜80%短縮）を実施。
* **2026-02-09**: バージョン `2.8.0` をリリース。外部シークレットのバリデーション強化（Enterprise）と、プロジェクトメンバーや認証情報をAPI経由で取得しやすくするAPI監査機能の改善を実施。
* **2026-01-26**: バージョン `2.6.0` をリリース。AIエージェントが特定のツールを実行する前に人間の明示的な承認を要求できる「Human-in-the-loop for AI tool calls」機能を追加。
* **2026-01-20**: バージョン `2.5.0` をリリース。Chatノードに「Send a message」と「Send a message and wait for response」の2つのHuman-in-the-loopアクションを追加。
* **2026-01-09**: バージョン `2.0.0` (n8n 2.0) をリリース。次期メジャーバージョンとして、UI/UXの改善や安定性の向上を実施。
