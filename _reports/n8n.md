...
quick_summary:
  ...
  target_users:
    ...
    - IT部門
    - 技術系スタートアップ
  latest_highlight: 2026年5月にMCPサーバーへの接続設定を簡略化する機能をリリース
  update_frequency: 高
evaluation:
  score: 84
  base_score: 70
...
* **Microsoft Agent 365 連携**: n8nのAIエージェントをMicrosoft 365アプリ内でチームメンバーとして直接統合し、会話や連携を行うことができる機能。
* **OpenTelemetryサポート**: ワークフローの実行ログやエラーをOpenTelemetry対応の監視ツールに直接出力し、一元的な監視を実現。
...
* **外部シークレットプロバイダー**: HashiCorp Vault, AWS Secrets Manager, Azure Key Vault, GCP Secret Managerに加え、新たに「1Password」とのシームレスな連携に対応（Enterprise版）。
* **MCP (Model Context Protocol)**: Agent向けの機能として、MCPサーバー（Apify, Linear, Notion等）への接続設定を簡素化し、AIエージェントへの統合を強化している。
...
* **2026-06-23**: バージョン `2.28.0` をリリース。
* **2026-05-19**: バージョン `2.22.0` をリリース。一部のMCPサーバー（Apify, Notion, PostHog等）への接続がより簡単に行える機能を搭載。
* **2026-05-05**: バージョン `2.20.0` をリリース。「Microsoft Agent 365 Trigger node」を追加し、Microsoft 365アプリ内で動作するAIエージェントの構築をサポート。
* **2026-04-28**: バージョン `2.19.0` をリリース。Enterprise版向けに、IdPとn8n内のロールマッピング機能、インスタンスの自動初期設定機能を追加。
* **2026-04-07**: バージョン `2.16.0` をリリース。Enterprise版向けにトークンエクスチェンジ認証や実行データの秘匿化（Redaction）機能を追加。
* **2026-03-30**: バージョン `2.15.0` をリリース。ワークフローのOpenTelemetryサポートを追加。
* **2026-03-16**: バージョン `2.13.0` をリリース。バージョン履歴の差分を視覚的に確認できる「Visual diff」を追加。
* **2026-03-09**: バージョン `2.12.0` をリリース。外部シークレットプロバイダーとして新たに「1Password」をサポート（Enterprise版向け）。
* **2026-02-23**: バージョン `2.10.0` をリリース。外部シークレットプロバイダーへの複数接続サポートを追加し、大規模なワークフローと認証情報のボリュームに対するパフォーマンス改善（ロード時間を30%〜80%短縮）を実施。
* **2026-01-26**: バージョン `2.6.0` をリリース。AIエージェントが特定のツールを実行する前に人間の明示的な承認を要求できる「Human-in-the-loop for AI tool calls」機能を追加。
...
|:---:|:---|:---:|:---:|:---:|:---:|
| **基本機能** | 連携アプリ数 | ◯<br><small>350+</small> | ◎<br><small>8500+</small> | ◯<br><small>1600+</small> | ◯<br><small>1000+</small> |
| **カテゴリ特定** | 複雑なロジック | ◎<br><small>コード記述可</small> | △<br><small>直線的</small> | ◎<br><small>視覚的で柔軟</small> | ◎<br><small>高度な分岐・ループ</small> |
| **AI機能** | AIエージェント構築 | ◎<br><small>高度なHITL・MCP対応</small> | ◎<br><small>Zapier AI Agents搭載</small> | ◎<br><small>Make AI Agents搭載</small> | ◯<br><small>Agent Studio機能</small> |
...
|---------|------|------|------|------------------|
| **本ツール** | オープンソースでセルフホスト可能。コード記述による高い拡張性を持つ。 | ・無料から利用可能(OSS)<br>・データガバナンス<br>・高度なAI連携(HITL, MCP等)とカスタマイズ性 | ・学習コストが高い<br>・セルフホストの運用負荷 | 高度なカスタマイズやデータガバナンスが要件となる場合。エンジニアが主体となって自動化を推進するチーム。 |
| **Zapier** | 業界最大級の連携アプリ数を誇る。UIが非常にシンプル。 | ・導入のしやすさ<br>・圧倒的な連携先(8500+)<br>・AI機能の充実 | ・コストが高くなりがち<br>・複雑な処理は苦手 | シンプルな定型業務を素早く自動化したい場合。専門的な技術者がいないチーム。 |
| **Make** | 視覚的なUIで複雑なワークフローの構築が得意。 | ・複雑なロジックを視覚的に構築可能<br>・コストパフォーマンスの高さ | ・操作の習熟にやや時間がかかる<br>・連携アプリ数はZapierに劣る | データ加工や条件分岐など、複雑なロジックを要する自動化を行いたい場合。コストを抑えたい場合。 |
| **Workato** | エンタープライズ向けの統合iPaaS。セキュリティ重視。 | ・高度なセキュリティとガバナンス<br>・ITとビジネスの協調機能 | ・導入コストが非常に高い<br>・オーバースペックの可能性 | 全社規模でセキュリティを重視した自動化基盤を構築する場合。 |
