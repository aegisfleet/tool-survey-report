...
quick_summary:
  ...
  target_users:
    ...
    - AIエンジニア
    - Python開発者
  latest_highlight: 2026年8月にv0.24.0をリリースし、Hugging Face Hub統合やエクスポートダイアログを強化
  update_frequency: 高
evaluation:
  score: 87
  base_score: 70
...
* **アーキテクチャ**: ローカルファーストのクライアント・サーバー構成（クラウド版 `molab` も提供）および Pyodide を活用した WASM (ブラウザ完結) 実行環境。
* **主要コンポーネントとデータフロー**:
  * **AST解析とDAG構築**: セル内のPythonコードは抽象構文木（AST）にパースされ、変数や関数の定義・参照関係が解析されます。これにより、セル間の依存関係を示す有向非巡回グラフ（DAG）が自動構築されます。
  * **リアクティブ実行エンジン**: DAGに基づいて、あるセルが実行・更新されると、その出力に依存する後続のすべてのセルが決定論的な順序で自動再実行されます。
  * **クライアント・サーバー通信**: ブラウザ上のUI（フロントエンド）とPythonカーネル（バックエンド）間はWebSocketsで通信し、UIコンポーネントの操作がリアルタイムにバックエンドの変数状態に反映されます。

```mermaid
graph TD
    A[ブラウザ UI] -->|WebSockets| B(marimo サーバー / カーネル)
    B --> C{AST解析 & DAG構築}
    C --> D[セル実行]
    D -->|状態更新| C
    D -->|結果・描画| B
...
* **特筆すべき要素技術**:
  * **Pyodide**: ブラウザ内で直接Pythonを実行するためのWASMポート。これによりバックエンドなしでの静的ホスティング（GitHub Pages等）を可能にします。
  * **AST (Abstract Syntax Tree) 解析**: 独自の静的解析により実行順序を決定し、Jupyterのような「隠し状態」の発生を根本的に防ぎます。
...
* **外部サービス連携**:
  * **データソース**: Hugging Face Hub (ネイティブ統合), DuckDB, SQLite, Postgres, MySQL, Google Cloud BigQuery, Google Sheets, MotherDuck など。
...
* **2026-08-17**: v0.24.0リリース - Hugging Face Hubのネイティブ統合（データソースから直接参照可能に）、統合エクスポートダイアログの追加など。
* **2026-07-31**: v0.23.16リリース - UIとエディタ体験の各種改善。
...
| 機能カテゴリ | 機能項目 | marimo | Visual Studio Code | uv |
|:---:|:---|:---:|:---:|:---:|
| **実行モデル** | リアクティブ実行 | ◎<br><small>標準で依存関係に基づき自動実行</small> | ×<br><small>手動での順次実行（Jupyter拡張）</small> | -<br><small>パッケージマネージャのため対象外</small> |
| **バージョン管理** | Gitフレンドリー | ◎<br><small>純粋な`.py`ファイル</small> | ◎<br><small>強力なGit統合</small> | ◎<br><small>プロジェクト設定の版管理</small> |
| **アプリ化** | Webアプリデプロイ | ◯<br><small>CLIから簡単デプロイ</small> | ×<br><small>エディタのため対象外</small> | -<br><small>パッケージマネージャのため対象外</small> |
| **コラボレーション** | リアルタイム共同編集 | △<br><small>ローカル中心、molabで一部可能</small> | ◎<br><small>Live Shareで強力に対応</small> | -<br><small>対象外</small> |
| **環境構築** | パッケージ管理統合 | ◎<br><small>ノートブック内に依存関係を記述</small> | ◯<br><small>拡張機能で対応</small> | ◎<br><small>超高速な仮想環境・パッケージ管理</small> |
...
| **Visual Studio Code** | 汎用的なコードエディタ（IDE） | 豊富な拡張機能、Jupyterサポート、強力なデバッグ機能とGit統合。 | ノートブック機能自体は標準的であり、隠し状態の問題は残る。 | 既存のJupyterノートブックを扱いながら、大規模なPythonプロジェクトを開発する場合。 |
| **uv** | 超高速なPythonパッケージ・プロジェクトマネージャ | Rust製の圧倒的な速度と、一貫した仮想環境・依存関係の管理。 | ノートブックとしてのデータ分析や可視化の機能は持たない。 | marimoと併用し、基盤となるPython環境や依存関係を高速・堅牢に管理したい場合（補完関係）。 |
