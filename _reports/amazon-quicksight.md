...
* **主要コンポーネントとデータフロー**:
  * S3, Athena, Redshift等のデータソースからデータを抽出し、インメモリエンジンのSPICEに取り込むか、Direct Queryで直接アクセスする。
  * 処理されたデータは、ダッシュボードレイヤーに渡されインタラクティブに可視化される。
* **特筆すべき要素技術**:
  * SPICE (Super-fast, Parallel, In-memory Calculation Engine)
  * Amazon Q（生成AIアシスタント）による自然言語処理
...
* **2026-05-08**: Amazon QuickSight が Amazon Athena データソースのクロスアカウントアクセスのサポートを開始。
* **2026-05-04**: Amazon QuickSight で自然言語プロンプトからダッシュボードを生成する機能が追加。
* **2026-04-15**: Amazon QuickSight がリッチでコンテキストに沿ったデータ探索を可能にするシートツールヒントを導入。
* **2026-02-18**: Amazon Q in QuickSightで、ダッシュボードのデータに対するカスタムQ&A機能が強化され、より精度の高い回答が得られるように。
* **2026-01-12**: SPICEエンジンのパフォーマンスが向上し、10億行以上の大規模データセットの取り込みとクエリがさらに高速化。
* **2025-10-15**: Googleスプレッドシートへの直接接続をサポート。認証設定により安全にシートデータを読み込み可能に。

(出典: [製品アップデート情報](https://aws.amazon.com/about-aws/whats-new/analytics/))
