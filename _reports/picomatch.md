...
quick_summary:
  ...
  latest_highlight: バージョン4.0.7をリリース
  update_frequency: 低
evaluation:
  score: 82
  base_score: 70
...
picomatchは、複雑なglobパターンを解析（パース）し、それをネイティブなJavaScript正規表現（RegExp）にコンパイルすることで超高速なマッチングを実現している。

* **アーキテクチャ**: 依存関係を持たない単一のライブラリ（モジュール）構成。
* **主要コンポーネントとデータフロー**:
  * パターンが入力されると、まず字句解析（スキャン）によりAST（抽象構文木）またはトークンの配列に変換される。
  * 構文木から不要な展開や冗長な部分を最適化しつつ、等価な正規表現文字列を生成する。
  * 最終的にJavaScriptの `RegExp` オブジェクトとしてコンパイルされ、ファイルパスや文字列とのマッチングに利用される。

```mermaid
graph TD
    A[入力されたglobパターン] --> B[字句解析/スキャナ]
    B --> C[AST / トークンツリー]
    C --> D[正規表現文字列の生成]
    D --> E[RegExpオブジェクトのコンパイル]
    E --> F[ファイルパスのマッチング判定]
```

* **特筆すべき要素技術**:
  * **POSIXブラケット/extglobsの実装**: 標準的な正規表現にはないBash特有の記法を、自前のパーサーで標準の正規表現パターンへ巧みに変換している。
  * **結果のキャッシュ（メモ化）**: 同一パターンの再パースを防ぐための内部キャッシュ機構により、反復的な呼び出しを高速化。
...
* **2026-08-24**: バージョン4.0.7リリース - 括弧で囲まれたパターン内の末尾のglobstarsの処理を修正。
* **2026-08-24**: バージョン4.0.6リリース - トークン要求時にパターン全体をスキャンするよう修正。
* **2026-07-02**: バージョン4.0.5リリース - riskyな反復extglobsを書き換える際にすべてのブランチを保持するよう修正。
