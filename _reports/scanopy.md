...
quick_summary:
  ...
  target_users:
    ...
    - MSP（マネージドサービスプロバイダー）
    - ホームラボ愛好家
  latest_highlight: 2026年8月にv0.17.9リリース。大規模グラフの描画速度の大幅向上や単一ホストの再スキャン機能が追加された
  update_frequency: 高
evaluation:
  score: 78
  base_score: 70
  plus_points:
    ...
    - point: 2
      reason: セルフホスト（Community版）なら無料で1ネットワークを管理可能
    - point: 2
...
* **自動ディスカバリー**: ネットワークをスキャンし、ホスト、サービス、およびそれらの関係性を自動的に特定する。単一ホストの再スキャン機能も追加され柔軟なディスカバリーが可能。
* **200種類以上のサービス検出**: Docker、Podman、PostgreSQL、nginx、各種ルーターなど、200以上のサービスを自動で検出・識別する。
* **インタラクティブなトポロジー図**: スキャン結果から動的なネットワーク図を自動生成し、ブラウザ上で閲覧・操作できる。
* **インベントリのテーブルビュー**: タブごとに折りたたみ可能なセクションやタグ形式の値を持つテーブルビューを備え、リストとして一元管理・閲覧できる。
* **分散スキャン**: 軽量なデーモンを各ネットワークセグメントにデプロイすることで、複雑なネットワーク全体をマッピング可能。
* **スケジュールスキャン**: 定期的な自動スキャンにより、構成図を常に最新の状態に保つ。
* **スナップショット機能（履歴保持）**: 過去のネットワーク状態をスナップショットとして保存・固定し、最新の状態と比較することが可能。
...
* **Docker等の詳細なサービス検出**: 単なるPingやポートスキャンだけでなく、Docker/Podmanコンテナや具体的なサービス名（DB、Webサーバー等）まで詳細に検出できる点が他のスキャナーより優れている。
* **定額制の料金体系**: デバイス単位ではなくネットワーク・シート単位の課金のため、大規模ネットワークでもコストが予測しやすい。
* **セルフホスト可能**: AGPL-3.0ライセンスのオープンソースとして提供されており、自社インフラで無料でホストすることも可能（1シート・1ネットワークの制限あり）。

## **6. 弱み・注意点 (Cons)**

* **日本語非対応**: 現在のところ、公式サイト、ドキュメント、UIなどは日本語に対応していない（Weblateでの翻訳プロジェクトは存在）。
* **IPv6非対応**: 現状はIPv4のみのサポートとなっている。
* **無料セルフホスト版の制限**: Community Editionは1シート・1ネットワークの制限が設けられており、複数ネットワークを管理するには商用ライセンスが必要。
...
| プラン名 | 料金 | 主な特徴 |
|---------|------|---------|
| **Starter (Cloud)** | $11.99/月 | 1シート、1ネットワーク、ホスト無制限。スケジュールスキャン、SVGエクスポート、共有リンク。 |
| **Pro (Cloud)** | $39.99/月 | 1シート、3ネットワーク（追加+$8/月）。APIアクセス、Mermaidエクスポート、図の埋め込み。 |
| **Business (Cloud)** | $79.99/月 | 5シート（追加+$8/月）、15ネットワーク（追加+$6/月）。Confluenceエクスポート、優先サポート。MSP向け。 |
| **Enterprise (Cloud)** | 個別見積り | カスタムシート/ネットワーク。SSO、専用サポートなど。 |
| **Community (Self-Hosted)** | 無料 | AGPL-3.0ライセンス。自社インフラで運用。1シート、1ネットワーク。ホスト数は無制限。 |
| **Standard/Plus (Self-Hosted)** | $3,000/年〜 | 商用ライセンス。複数ネットワーク・シート管理、高度な機能サポート。 |
...
* **導入企業**: 公開事例なし。ただし、IT部門やホームラボ愛好家の間での利用が多数報告されている。
* **導入事例**:
...
|:---|:---:|:---|:---|
| **Docker / Podman** | ◎ | DockerおよびPodman環境の検出にネイティブ対応しており、コンテナ間の関係性を可視化できる。 | 特になし |
...
* **2026-08-06**: v0.17.9 リリース (インベントリのテーブルビュー追加、大規模トポロジの描画パフォーマンス改善)
* **2026-07-29**: v0.17.7 リリース (単一ホストの再スキャン機能追加、UniFiコントローラー連携(Beta)の追加)
* **2026-07-01**: v0.17.2 リリース (Podmanサポートの追加)
* **2026-06-23**: v0.17.0 リリース (トポロジのバージョン管理をスナップショットモデルに刷新、SNMPv1/v3対応強化)
* **2026-03-26**: v0.15.3 リリース
