...
quick_summary:
  ...
  target_users:
    ...
    - MSP（マネージドサービスプロバイダー）
    - ホームラボ愛好家
  latest_highlight: 2026年8月にv0.17.9リリース（インベントリのテーブルビュー追加、大規模構成図の描画パフォーマンス大幅改善など）
  update_frequency: 高
evaluation:
  score: 79
  base_score: 70
...
* **200種類以上のサービス検出**: DockerやPodman、PostgreSQL、nginx、各種ルーターなど、200以上のサービスを自動で検出・識別する。
* **単一ホストの再スキャン**: ネットワーク全体ではなく、個別のデバイスに対して再スキャンを実行できる。
...
* **アーキテクチャ**: クライアント・サーバー構成（クラウド版とセルフホスト版）とエージェントレススキャン。
* **主要コンポーネントとデータフロー**:
  * ネットワーク内に軽量なデーモンを配置し、サーバーへアウトバウンド接続のみを行う（DaemonPollモード）。
  * デーモンがネットワークセグメントをスキャンし、ホストやサービス、コンテナ間の関係性データを収集してサーバーに送信。
  * サーバー側で収集データを統合し、Svelteで構築されたフロントエンド上で動的なトポロジー図をレンダリングする。
* **特筆すべき要素技術**:
  * **Docker API連携**: Dockerソケットを通じてコンテナの内部情報を取得し、サービスや仮想ネットワークを詳細に検出。
  * **SNMP/ARPスキャン**: L2/L3レイヤーでの詳細なデバイス探索。
...
| **Self-Hosted Standard/Plus** | 公式サイト参照 | オンプレミス展開向けの商用プラン。テナント分離などの強化機能を提供。 |
...
| **UniFi Controller** | ◯ | UniFiで管理されているデバイスを可視化し、ネットワークの洞察を活用できる（現在Beta版）。 | Beta版のため一部制限がある可能性あり |
...
* **2026-08-06**: v0.17.9 リリース (インベントリのテーブルビュー追加、大規模ネットワーク描画のパフォーマンス向上)
* **2026-07-29**: v0.17.7 リリース (単一ホストの再スキャン機能、UniFiコントローラー連携(Beta)追加)
* **2026-07-07**: v0.17.4 リリース (Self-Hosted Standard/Plus プランの追加、テナント分離の強化)
* **2026-07-01**: v0.17.2 リリース (Podman コンテナ連携追加、アプリケーションビューのレイアウト最適化)
...
| **詳細検出** | サービス/Docker検出 | ◎<br><small>200種以上対応、Podmanもサポート</small> | - | - | ◯<br><small>ポートベースでの推測</small> |
