...
quick_summary:
  ...
  target_users:
    ...
    - DevOpsエンジニア
    - GitHub管理者
  latest_highlight: 2026年9月にv0.6.1がリリースされ、GitHub Enterprise Server対応や認証強化、Copilot Budget Detectionなどが追加された
  update_frequency: 高
evaluation:
  score: 85
  base_score: 70
...
* **Copilotポリシー分析**: 組織単位でのシート使用状況、コンテンツ除外設定、ポリシー構成、MCP設定に加え、Copilot Budgetの利用状況やアラート設定をスキャンする。
...
* **GHES（GitHub Enterprise Server）サポート**: Data Residency（GHE.com）やセルフホスト型のGitHub Enterprise Serverに対するスキャンにもネイティブ対応する。
* **MCP（Model Context Protocol）サーバー機能**: AIアシスタント（VS Code/GitHub Copilot等）がghqrの機能と対話するためのサーバーとして動作する。
* **高度なレポート機能**: JSONファイルからのエンリッチメント再実行、経営層向けのMarkdownエグゼクティブサマリー、詳細なExcel出力など多様なフォーマットをサポートする。

## **4. 動作原理・システム構成**

* **アーキテクチャ**: GitHub API（REST/GraphQL）に依存するクライアントサイドのCLIツールであり、ローカルまたはCI/CD環境で実行される。内蔵するMCPサーバーを介してAIアシスタントから対話的に呼び出すことも可能。
* **主要コンポーネントとデータフロー**:
  * **データ収集フェーズ**: 指定されたスコープ（Enterprise/Organization/Repository）に基づいてGitHub APIを呼び出し、設定情報を取得する。レート制限に対する自動バックオフ機能や、PAT（Personal Access Token）およびGitHub Appインストーレーショントークンの認証チェーンを備える。
  * **分析・評価フェーズ**: 取得した情報を内蔵のベストプラクティス・ルールセットと照合し、推奨設定からの逸脱を評価する。
  * **出力フェーズ**: 分析結果をJSONにダンプし、さらにExcelやMarkdownのレポート形式に変換（エンリッチメント）して出力する。過去のJSON結果からの再出力（Replaying enrichment）もサポートしている。

```mermaid
flowchart TD
    User([ユーザー / CIパイプライン]) --> |コマンド実行 / トークン付与| CLI[ghqr CLI]
    AI([AIアシスタント]) --> |MCPプロトコル| MCPServer[ghqr MCP サーバー]
    MCPServer --> CLI

    CLI --> |認証チェーン<br>PAT / GitHub App| GitHubAPI[(GitHub API<br>REST / GraphQL)]
    GitHubAPI --> |設定データ・ポリシー情報| CLI

    CLI --> |ルール照合| Analyzer[分析・評価エンジン]
    Analyzer --> |JSON出力| JSONOut[(生データ .json)]
    JSONOut --> |エンリッチメント| ReportGen[レポート生成器]
    ReportGen --> |Markdown / Excel| ReportOut[(各種レポート)]
```

* **特筆すべき要素技術**:
  * **MCP (Model Context Protocol)**: ClaudeやGitHub Copilotなどと連携し、AIが直接セキュリティ状態を監査できる仕組みを提供。
  * **APIページネーションとレート制限回避**: 大規模エンタープライズの数千リポジトリ・数万アラート（セキュリティアラートなど）をスキャンする際も、100件キャップの回避や効率的なページネーション、バックオフを実装してAPI利用枠枯渇を防ぐ。
...
* **2026-09-07**: v0.6.1 リリース。依存関係のアップデートなど。
* **2026-07-31**: v0.6.0 リリース。GitHub認証用のクレデンシャルチェーンが実装され、柔軟な認証をサポート。
* **2026-07-21**: v0.5.1 リリース。Excelレポートの不整合（ルールセット、Enterprise Budget、GHASなど）を修正。
* **2026-06-24**: v0.5.0 リリース。Organizationセキュリティアラート集約の100件キャップをページネーションで解除、GitHub Appインストーレーショントークンのサポート、Markdownレポートの経営層向けフォーマット追加。
* **2026-06-05**: v0.4.3 リリース。ドキュメントの強化とCopilot Budgetの推奨事項を追加。
* **2026-05-30**: v0.4.2 リリース。MCPツールにおけるGHESスキャン失敗時のエラー表示を改善。
* **2026-04-22**: v0.2.0 リリース。リポジトリスキャン機能（`--repository`）の追加、GHE.com（Data Residency）対応、ルールセットエンリッチメントの進捗ログ出力などを追加。
* **2026-03-16**: v0.1.3 リリース。
* **2026-03-11**: v0.1.2 リリース。EnterpriseDiscoveryStageにおけるスキップロジックの改善など。
* **2026-03-02**: v0.1.1 リリース。各種依存関係やアクションのアップデート。
* **2026-02-24**: v0.1.0 リリース（初期リリースに近いバージョン）。
...
| 機能カテゴリ | 機能項目 | GitHub Quick Review | Trivy | SonarQube | Semgrep |
|:---:|:---|:---:|:---:|:---:|:---:|
| **基本機能** | SCM設定・ポリシー監査 | ◎<br><small>GitHub特化</small> | △<br><small>IaC/シークレット中心</small> | ×<br><small>対象外</small> | ×<br><small>対象外</small> |
| **セキュリティ** | 静的コード解析(SAST) | ×<br><small>設定確認のみ</small> | ◯<br><small>脆弱性スキャン全般</small> | ◎<br><small>詳細なコード解析</small> | ◎<br><small>高速なSAST</small> |
| **プラットフォーム** | GitHub連携 | ◎<br><small>専用ツール</small> | ◎<br><small>GitHub Actions対応</small> | ◯<br><small>複数対応</small> | ◯<br><small>複数対応</small> |
...
| **Trivy** | 包括的な脆弱性スキャナー | コンテナ、IaC、依存関係など幅広くスキャン可能 | SCM基盤の詳細な設定監査には特化していない | リポジトリ内のコードやコンテナの脆弱性を包括的にスキャンしたい場合 |
...
* **選択時のポイント**:
  ソースコードの脆弱性（SAST）や依存関係の脆弱性（SCA）を見つけるSonarQube、Semgrep、Trivyなどのツールと**競合するものではなく、補完関係にある**。SCM基盤（GitHub自体）のセキュリティ設定の網羅的チェックを行いたい場合は、ghqrの導入が最適解となる。コンテナやIaCの脆弱性も検知したい場合はTrivyとの併用が有効である。
