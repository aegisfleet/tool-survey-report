...
quick_summary:
  ...
  target_users:
    ...
    - データサイエンティスト
    - 一般ユーザー
  latest_highlight: 2026年6月に強力な推論機能とマルチモーダル能力を備えたQwen3.6-35B-A3Bのパブリックプレビューを開始
  update_frequency: 高
evaluation:
  score: 88
  base_score: 70
...
* **アーキテクチャ**: クラウド完結型SaaS（WebUI）およびAPIベースのクライアント・サーバー構成
* **主要コンポーネントとデータフロー**:
  * ユーザーのプロンプト（テキスト・画像・音声）はWebUIまたはAPI Gatewayを通じて送信される。
  * Qwenの基盤モデル（Qwen-Max, Qwen-Plus, Qwen VLo等）がリクエストを処理し、テキスト生成、画像分析、コード生成を行う。
  * エージェント機能（Deep Research, Web Dev）が有効な場合、自律的に外部のWeb検索や処理タスクを反復実行し、最終的な結果をユーザーに返す。
* **特筆すべき要素技術**:
  * **Hybrid Thinking**: タスクの難易度に応じて通常の推論とCoT（Chain-of-Thought）推論を動的に切り替える仕組み。
  * **MTP (Multi-token prediction)**: 推論速度を大幅に向上させる複数トークンの同時予測技術。

```mermaid
graph TD
    User["ユーザー"] -->|Web UI / API| Gateway["API Gateway"]
    Gateway --> ModelStudio["Model Studio (DashScope)"]

    subgraph "Qwen Platform"
        ModelStudio --> CoreModel["Qwen Core Models
(Max / Plus / Turbo)"]
        ModelStudio --> Multimodal["Qwen VLo
(画像・音声・動画処理)"]
        ModelStudio --> Agents["Agentic Framework
(Deep Research / Web Dev)"]

        Agents -.->|Web Search| Internet["インターネット情報"]
        Agents -.-> CoreModel
    end

    CoreModel -->|生成結果| ModelStudio
    Multimodal -->|生成結果| ModelStudio
    Agents -->|最終レポート/コード| ModelStudio
    ModelStudio -->|レスポンス| Gateway
    Gateway -->|レスポンス| User
```
...
* **2026-06-04**: **Qwen3.6-35B-A3B**: 強力なマルチモーダル能力を備えたモデルのパブリックプレビューを開始。
...
|:---:|:---|:---:|:---:|:---:|:---:|
| **モデル性能** | 推論能力 (CoT) | ◎<br><small>Hybrid Thinking</small> | ◎<br><small>GPT-5.4</small> | ◎<br><small>Opus 4.8</small> | ◯<br><small>3.5 Flash</small> |
