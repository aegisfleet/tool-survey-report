...
quick_summary:
  ...
  target_users:
    ...
    - バックエンドエンジニア
    - AIエージェント開発者
  latest_highlight: 2026年9月にv1.4.2をリリース。Rustへの書き換えが完了。
  update_frequency: 高
evaluation:
  score: 85
  base_score: 70
...
* **アーキテクチャ**: ローカルファーストの実行環境（ランタイム）であり、Zig、C++、Rust（v1.4以降）で記述された高速なコアと、JavaScriptCoreエンジンを組み合わせた構造。
* **主要コンポーネントとデータフロー**:
  * **JavaScriptCore**: Safariでも使用されている高速なJavaScriptエンジン。V8ではなくJSCを採用することで起動時間を短縮している。
  * **Bun.js**: C++、Zig、およびRustで書かれたBunのコア。HTTPサーバー、ファイルシステム操作などのシステムコールを高速に処理する。
  * **バンドラ/トランスパイラ**: 内部でTSやJSXをネイティブに解釈し、即座に実行可能なコードに変換する。
* **特筆すべき要素技術**:
  * **mimalloc**: 高度なメモリ割り当てアロケータ。
  * **Web標準API**: `fetch`, `URL`, `WebSocket` などを内部でネイティブに実装。
  * **FFI (Foreign Function Interface)**: C言語などのライブラリを非常に低オーバーヘッドで呼び出せる機能。
  * **SQLite内蔵**: アプリケーションから直接データベースを操作できる `bun:sqlite`。

```mermaid
graph TD
    A[ユーザーコード TS/JS/JSX] --> B(Bun Transpiler/Bundler)
    B --> C{JavaScriptCore Engine}
    C --> D[Bun Core C++/Zig/Rust]
    D --> E[System APIs FS, Network, OS]
    D --> F[Built-in Tools SQLite, FFI, HTTP]
```
...
* **2026-09-05**: Bun v1.4.2リリース。バグ修正と安定性向上。
* **2026-09-04**: Bun v1.4.1リリース。バグ修正とパフォーマンス改善。
* **2026-08-20**: Bun v1.4リリース。Rustへの書き換え完了、Node.jsテストスイート互換性大幅向上(+1,517 tests)、`Bun.Image`、`Bun.WebView` などの新機能追加。
* **2026-05-13**: Bun v1.3.14リリース。FreeBSDネイティブビルド対応、HTTP/3対応などの機能強化。
* **2026-04-20**: Bun v1.3.13リリース。`bun test --parallel` / `--isolate` / `--shard`、`fetch()` のHTTP/2・HTTP/3対応。
* **2026-04-10**: Bun v1.3.12リリース。`Bun.WebView`（ヘッドレスブラウザ）、`bun ./README.md` のMarkdown描画など。
* **2026-03-18**: Bun v1.3.11リリース。`Bun.cron()` の追加、`bun test --path-ignore-patterns` などの強化。

(出典: [Bun Releases](https://github.com/oven-sh/bun/releases))
...
| **互換性** | npm対応 | ◯<br><small>高い互換性を持つ</small> | ◎<br><small>本家 (v25.9 / v24.14 LTS)</small> | ◯<br><small>互換機能あり (v2.7)</small> | ◎<br><small>公式標準 (v12)</small> |
