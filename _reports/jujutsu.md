...
quick_summary:
  ...
  target_users:
    ...
    - Gitユーザー
    - 大規模プロジェクト開発者
  latest_highlight: 2026年7月にv0.43.0をリリースし、jj runによる一括コマンド実行をサポート
  update_frequency: 高
evaluation:
  score: 82
  base_score: 70
...
* **一括コマンド実行と自動伝播**: `jj run` コマンドにより、複数の変更セットそれぞれに対して指定したコマンド（フォーマッタやテストなど）を実行し、その結果の変更やコンフリクトを自動的に伝播させることができる。
* **ブランチレスな操作**: 名前付きブランチを作成せずに、無名のリビジョンだけで作業を進めることが可能（必要に応じてブックマークを使用）。
* **強力なリビジョン選択**: 柔軟なクエリ言語（Revsets）を用いて、操作対象のコミットを柔軟に指定できる。

## **4. 動作原理・システム構成**

* **アーキテクチャ**: ローカルファーストの分散型バージョン管理システム。Gitとの双方向互換性を持つ。
* **主要コンポーネントとデータフロー**:
  * データはローカルの `.jj` ディレクトリと `.git` ディレクトリに保存される。
  * 操作はOperation Logとして記録され、仮想的な状態を安全に管理・巻き戻すことができる。
* **特筆すべき要素技術**:
  * Git互換バックエンド
  * Rustによる高速でメモリ安全な実装
...
* **効果的な活用法 (Modern Practices)**:
  ...
  * **`jj run` による一括フォーマット・テスト**: 複数のコミットにまたがる変更に対して `jj run` を用いてコマンドを実行し、安全にコードの品質を保つ。
* **陥りやすい罠 (Antipatterns)**:
...
* **2026-07-02 (v0.43.0)**: 変更セットごとにコマンドを実行し結果を伝播させる `jj run` コマンドを追加。
* **2026-06-04 (v0.42.0)**: マルチスレッドパフォーマンス向上のためのmimallocの採用。
* **2026-05-07 (v0.41.0)**: `jj fix` での特定行フォーマット対応や `--no-integrate-operation` フラグの追加。
* **2026-04-02 (v0.40.0)**: diffの片側のみ一致するコンテンツをマッチさせる `diff_lines_added()` / `diff_lines_removed()` revset関数を追加。
