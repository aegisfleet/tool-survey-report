...
evaluation:
  ...
  plus_points:
    ...
    - point: 5
      reason: OpenAI・Anthropic互換APIを提供しており、既存システムからの移行が容易
    - point: 5
      reason: 円建ての料金体系で為替リスクがなく、コストパフォーマンスが高い
  score: 82
  summary: データの安全性とコストを重視する国内プロジェクトに最適。モデルの多様性も着実に拡充している。
...
quick_summary:
  has_free_plan: true
  is_oss: false
  latest_highlight: 2026年8月にPLaMo 3.0 Primeが提供開始されるなどモデルラインナップの拡充が進行中
  starting_price: 無料枠あり
  target_users:
    - 開発者
    - 企業
    - 官公庁
...
* **さくらのAI Engine**: OpenAIおよびAnthropic互換のAPIを提供する推論基盤。テキスト生成、音声認識、音声合成、埋め込みなどの機能を従量課金で利用可能。
* **さくらのAIソリューション**: 企業向けのパッケージ型ソリューション。閉域網接続や専用インスタンスにより、高度なセキュリティ環境を提供。
* **Playground**: コーディング不要でAIモデルの性能を試せるWeb UI。手軽な認証手続きで無料利用可能（RAG機能も統合）。
* **マルチモデル対応**: 国産モデル（NEC「cotomi v3」、PFN「PLaMo 3.0 Prime」）や、主要なオープンソース・外部モデル（Kimi, Gemma, Qwen3など）を選択して利用できる。
* **RAG API**: ドキュメント（PDF, txt等）をアップロードし、ベクトル検索と生成を単一のAPIコールで実行できる機能。チャンクサイズ指定やユークリッド距離の指定にも対応。
* **音声認識 (Audio transcription)**: Whisper等のモデルを利用した高精度な音声文字起こし機能。
* **音声合成 (Text-to-Speech)**: テキストを自然な音声（VOICEVOX等）として出力する機能。
...
* **アーキテクチャ**: クラウド完結型のフルマネージドサービス (国内データセンター完結)
* **システム構成図 (Mermaid)**:

```mermaid
graph TD
    Client["クライアント (Webアプリ/SDK)"] -->|OpenAI / Anthropic互換 API| Gateway["APIゲートウェイ\n(https://api.ai.sakura.ad.jp)"]
    Gateway --> Auth["認証・ルーティング"]
    Auth --> LLM["推論エンジン\n(PLaMo, Kimi, Gemma, Qwen等)"]
    Auth --> RAG["RAG エンジン\n(ドキュメント解析/検索)"]
    Auth --> Audio["音声処理\n(文字起こし, 音声合成)"]
    RAG -->|ベクトル検索| VectorDB[("ベクトルDB\n(チャンク化ドキュメント)")]

    subgraph "さくらのクラウド (国内データセンター完結)"
        Gateway
        Auth
        LLM
        RAG
...
* **主要コンポーネントとデータフロー**:
  * 開発者はOpenAIまたはAnthropic互換のAPIエンドポイントにリクエストを送信する。
  * データは日本国内のさくらインターネットのデータセンター（さくらのクラウド・高火力基盤）で処理され、顧客のデータが推論モデルの学習に使われたり、外部のモデル提供元へ送信されることはない。
* **特筆すべき要素技術**:
  * **OpenAI / Anthropic互換レイヤー**: 既存のSDKやライブラリ（LangChain, Dify等）からエンドポイントを変更するだけで利用可能にする仕組み。
  * **RAG API**: チャンク分割、埋め込み(Embeddings)、ベクトル検索、LLMによる回答生成までの複雑なパイプラインを1つのAPIで完結させるマネージド機能。
...
* **APIの互換性**: 業界標準のOpenAIおよびAnthropicのAPI仕様と互換性があり、既存のアプリケーションやツールから容易に移行できる。
* **コストパフォーマンス**: 為替の影響を受けない円建て料金と自社インフラによる低価格設定。初期費用不要で、無料プランも提供されている。
* **多様なモデルの提供**: 特定ベンダーにロックインされず、PFNの「PLaMo」のような国産フラッグシップモデルから、最新のOSSモデル、Kimiなどの強力なモデルを切り替えて利用できる。
...
| プラン名 | 料金 | 主な特徴 |
|---------|------|---------|
| **基盤モデル無償プラン** | 無料 | ・`gpt-oss-120b`等の主要モデルを月3,000リクエストまで無料で利用可能<br>・リクエスト上限超過後はレート制限がかかる |
| **従量課金プラン** | モデル毎のトークン課金 | ・無償プランの利用枠を超えて利用可能<br>・例: `gpt-oss-120b` (Input: 0.15円/1万トークン)<br>・音声認識: 0.5円/60秒 |
| **クローズドモデル** | 個別見積もり/要申請 | ・PFN「PLaMo 3.0 Prime」、NEC「cotomi v3」など<br>・無償枠の対象外。法人のみ利用申請が可能で、審査がある |

* **課金体系**: 主にトークン単位（Input/Output）の従量課金。音声認識は秒単位、音声合成(TTS)はモーラ単位(3円/10000モーラ)、RAGはチャンク単位（3円/100チャンク）の課金。パブリックプレビューモデル（Kimi, Gemma等）も無償枠・従量課金が設定されている。
* **無料トライアル**: 基盤モデル無償プランに加え、Playgroundにて手軽な認証手続きのみで無料試用が可能。
...
* **API**:
  * **OpenAI互換 Chat Completions API**: 標準的なテキスト推論。
  * **OpenAI互換 Responses API**: エージェント用途・ツール連携向けの高度な制御。
  * **Anthropic互換 Messages API**: Claude互換API。「Claude Code」等のツールから直接利用可能（一部モデル対応）。
* **外部サービス連携**:
  * **LangChain / LlamaIndex**: OpenAI互換APIを通じてシームレスに連携可能。
  * **Claude Code**: Anthropic互換APIを用いて、Kimiモデル等を直接指定して連携。
...
* **2026-08-13**: PLaMo 2.0-31B Instructedの提供終了を発表（10月12日終了予定）。
* **2026-08-04**: PFNの純国産基盤モデル「PLaMo 3.0 Prime」を提供開始。
* **2026-07-28**: マルチモーダルモデル「Kimi-K2.7-Code」のパブリックプレビューを提供開始。
* **2026-06-30**: 「gemma-4-31B-it」のパブリックプレビューを提供開始。
* **2026-06-04**: マルチモーダルモデル「Qwen3.6-35B-A3B」のパブリックプレビューを提供開始。
* **2026-02-26**: テキストを自然な音声として生成する「音声合成（TTS）API」を提供開始。
...
(出典: [さくらのクラウド ニュース（さくらのAI Engine）](https://cloud.sakura.ad.jp/news/tag/ai-engine/))
...
|:---:|:---|:---:|:---:|:---:|:---:|
| **モデル** | 多様性 | ◯<br><small>PLaMo, Kimi等拡充中</small> | ◎<br><small>Claude 3.7等主要数社網羅</small> | ◯<br><small>Rakuten AI 2.0 (日本語特化)</small> | ◎<br><small>Gemini 3.1 + OSS</small> |
| **インフラ** | 国内DC | ◎<br><small>完全国内完結</small> | ◯<br><small>東京リージョン</small> | ◯<br><small>国内基盤</small> | ◯<br><small>東京リージョン</small> |
| **コスト** | 円建て | ◎<br><small>為替リスクなし</small> | △<br><small>ドル建て</small> | ◎<br><small>円建て(OSSは無料)</small> | △<br><small>ドル建て</small> |
...
|---------|------|------|------|------------------|
| **さくらのAI** | データ国内完結型のLLMプラットフォーム | ・データ主権の確保<br>・円建ての安定した価格<br>・OpenAI/Anthropic互換API | ・エコシステムの発展性 | データセキュリティやコスト安定性を最優先する場合。手軽にOpenAI/Claude互換APIを試したい場合。 |
| **Amazon Bedrock** | AWS統合型のAIプラットフォーム | ・豊富なモデル選択肢 (Claude 3.7等)<br>・AWSサービスとの強力な連携 | ・複雑な料金体系<br>・ドル建ての価格変動 | 既存のAWSインフラを最大限に活用し、複数モデルを利用したい場合。 |
| **Rakuten AI** | 楽天経済圏特化のAIプラットフォーム | ・日本語特化OSSモデル (Rakuten AI 2.0)<br>・楽天エコシステムとの連携 | ・外部サービス連携が限定的 | 日本語LLMを自社運用したい場合や、楽天のサービスを活用したビジネスを展開する場合。 |
| **Vertex AI Studio** | Google CloudのAIプラットフォーム | ・Gemini 3.1など最新モデルへのアクセス<br>・エンドツーエンドのMLOps連携 | ・学習コストが高い<br>・複雑な料金体系 | 高度なAIアプリ開発や、Google Cloud環境でプロトタイピングから運用まで一気通貫で行う場合。 |
