...
quick_summary:
  ...
  target_users:
    ...
    - DevOpsエンジニア
    - エンタープライズ
  latest_highlight: 2026年6月・7月にJFrog CLIの継続的なアップデート（Poetry対応強化、Alpine APK CI修正など）を実施
  update_frequency: 高
evaluation:
  score: 83
  base_score: 70
...
* **ユニバーサルパッケージ管理**: Maven, Gradle, npm, NuGet, PyPI, Docker, Helm, Conan, Go, Poetryなど、主要な全てのパッケージマネージャーに対応。
...
* **アーキテクチャ**: クラウド完結型SaaS、またはオンプレミスのクライアント・サーバー構成。
* **主要コンポーネントとデータフロー**:
  * ユーザーやCI/CDパイプラインから送信されたアーティファクトは、チェックサムベースのストレージに保存され、メタデータはデータベースで管理される。
  * JFrog Xrayと連携し、保存されたコンポーネントの脆弱性やライセンスをスキャンする。
* **特筆すべき要素技術**:
  * チェックサムベースのストレージ機能（重複排除）。
  * 高可用性を実現するマルチサイトレプリケーション機能。
...
| **Python / npm / Poetry** | ◯ | プロキシリポジトリとしてキャッシュ機能が優秀。 | Cloud版またはPro版以上が必要。 |
...
* **2026-07-08**: **JFrog CLI v2.113.0リリース**。Alpine APK CIにおけるGoバージョン不一致の修正などを実施。
* **2026-07-01**: **JFrog CLI v2.112.0リリース**。Poetryのフォールバックロジック追加などを実施。
* **2026-06-26**: **JFrog CLI v2.110.0リリース**。PoetryにおけるPost Failure Runサポート追加などを実施。
* **2026-06-17**: **JFrog CLI v2.109.0リリース**。mcpおよびuvコマンドに対するAIエージェント向けヘルプテキストの追加などを実施。
...
| 機能カテゴリ | 機能項目 | 本ツール | Nexus Repository | GitHub Packages | GitLab |
|:---:|:---|:---:|:---:|:---:|:---:|
| **基本機能** | 対応形式 | ◎<br><small>30以上 (Max)</small> | ◎<br><small>非常に広範</small> | △<br><small>主要のみ</small> | ◯<br><small>主要のみ</small> |
| **セキュリティ** | 脆弱性スキャン | ◎<br><small>Xray (強力)</small> | ◯<br><small>IQ Server</small> | ◯<br><small>Dependabot</small> | ◎<br><small>DevSecOps統合</small> |
| **拡張機能** | AI/ML対応 | ◎<br><small>MLOps統合</small> | △<br><small>限定的</small> | △<br><small>レジストリのみ</small> | ◯<br><small>Duo Agent</small> |
| **非機能要件** | コスト | △<br><small>高価</small> | ◎<br><small>OSS版あり</small> | ◯<br><small>従量/枠内</small> | ◎<br><small>OSS/SaaS</small> |
