...
evaluation:
  ...
  plus_points:
    ...
    - point: 5
      reason: GitHub Actions特有の式({% raw %}${{ }}{% endraw %})に対する強力な型チェックやコンテキスト検証が充実している
    - point: 4
...
* **式の強力な型チェック**: `{% raw %}${{ }}{% endraw %}` 内の式におけるセマンティクスや型チェック。存在しないプロパティの参照や型不一致を検出する。
...
* **セキュリティチェック**: 信頼できない入力（例: `{% raw %}${{ github.event.pull_request.title }}{% endraw %}` をそのままスクリプト展開する）によるインジェクションの脆弱性を検出する。
...
* **2026-02-14**: GitHub Actionsに新たに追加された `{% raw %}${{ case() }}{% endraw %}` 関数のサポートを追加。リリースバイナリに対するArtifact attestationsの検証機能への対応などを追加。（v1.7.11）
...
| **octoscan** | セキュリティ特化のスキャナ | `{% raw %}${{ github.event.* }}{% endraw %}`のような未検証入力のインジェクションを高精度に検出できる。 | 構文エラーや型チェックなどは検出しない。 | サードパーティやOSSからのPRなど、外部入力を受け取るワークフローのセキュリティを担保したい場合。 |
