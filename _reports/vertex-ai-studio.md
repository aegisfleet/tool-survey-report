...
quick_summary:
  has_free_plan: true
  is_oss: false
  latest_highlight: 2026年4月にRAG Cross Corpus Retrievalのプレビュー開始、Gemini 3.1 Pro/Flash-Lite等の新モデルに対応
...
* **カテゴリ**: LLMプラットフォーム
* **概要**: Google Cloud上で生成AIモデルのプロトタイピング、カスタマイズ、本番環境への統合を迅速に行うための統合開発環境。開発者はコーディング不要のUIを通じて、Gemini 3.1をはじめとする最新の基盤モデルを試し、自社データでチューニングし、アプリケーションに組み込むまでの一連のワークフローを加速できる。
...
* **Model Garden**: Gemini, Imagen, VeoといったGoogleの最新モデルに加え、Llama, Claude, Gemmaなどのパートナー製モデルやオープンソースモデルを含む100以上の基盤モデルにアクセスし、デプロイできる。
* **Agent Engine**: コーディング不要で、会話型AIエージェントを構築、テスト、デプロイするためのツールセット。Memory Bank、Sessions、Code Executionなどの機能で外部APIとの連携（Tool use）や自律的タスクを高度にサポート。
* **RAG Engine**: Vector SearchやPineconeなどを活用し、Serverless modeによるデータベースプロビジョニングの抽象化と合わせ、効率的にRAGパイプラインを構築・管理できる。
* **MLOps連携**: Vertex AI Pipelines, Model Registry, MonitoringといったMLOpsツール群とシームレスに連携し、モデルのバージョン管理、CI/CD、本番環境でのパフォーマンス監視までを一貫して行える。

## **4. 動作原理・システム構成**

* **アーキテクチャ**: クラウド完結型マネージドプラットフォーム。
* **主要コンポーネントとデータフロー**: ユーザーはWebブラウザまたはAPI経由でGoogle Cloudのインフラストラクチャ上のVertex AIの各サービス（モデル利用、チューニング、エージェント構築など）にアクセスする。チューニングデータや生成されたプロンプト等のデータはGoogle Cloud上のVPC内に安全に保存される。
* **特筆すべき要素技術**:
  * Serverless RAG Engine
  * GoogleのTPUおよび最先端のマルチモーダル基盤モデル群
...
* **最先端モデルへの迅速なアクセス**: Gemini 3.1 Pro/Flashなど、Googleが開発した最新・最大規模のマルチモーダルモデルをいち早く利用でき、競合優位性を確保できる。
...
* **2026-04-17**: **RAG Cross Corpus Retrieval** がパブリックプレビューで利用可能に。複数のRAGコーパスから同時に関連するコンテキストを検索し、回答を生成することが可能になった。
* **2026-04-14**: Anthropic社の **Claude Opus 4.7** がModel Gardenで利用可能に。
* **2026-04-03**: **Gemma 4 26B A4B IT** がModel Gardenで試験的に提供開始。また、フルマネージドデータベースである **Vertex AI RAG Engine Serverless mode** がパブリックプレビューとなった。
* **2026-03-25**: 音楽生成モデル **Lyria 3** のパブリックプレビューが開始。
* **2026-03-03**: 高速でコスト効率の高い **Gemini 3.1 Flash-Lite** がパブリックプレビューとして利用可能に。
* **2026-02-19**: 高度な推論能力を持つ **Gemini 3.1 Pro** がパブリックプレビューで利用可能に。
* **2026-02-17**: Anthropic社の **Claude Sonnet 4.6** がModel Gardenで利用可能に。
...
(出典: [製品アップデート情報](https://cloud.google.com/vertex-ai/generative-ai/docs/release-notes))
...
|:---:|:---|:---:|:---:|:---:|:---:|
| **モデル** | 最新モデル | ◎<br><small>Gemini 3.1</small> | ◎<br><small>GPT-4o</small> | ◯<br><small>Claude 3.7</small> | ◎<br><small>GPT-4o</small> |
...
|---|---|---|---|---|
| **Vertex AI Studio** | Google Cloudに完全統合されたエンドツーエンドのAI開発プラットフォーム。 | Googleの最新モデル(Gemini 3.1等)への最速アクセス。プロトタイピングからMLOpsまで一気通貫でカバー。 | 多機能ゆえの複雑さと学習コスト。Google Cloudへの強い依存。 | Google Cloudを主要インフラとしており、本格的なAIアプリ開発・運用までを見据える場合。 |
...
| **Amazon Bedrock** | 複数のAI企業の基盤モデル（Anthropic, Cohere等）を単一APIで利用できるAWSのマネージドサービス。 | マルチプロバイダーのモデルを選択できる柔軟性。AWSエコシステムとの緊密な連携。 | Googleの最新モデル（Gemini 3.1等）は利用不可。UIはAWSコンソールに準拠。 | 特定のプロバイダーに依存せず、複数の主要モデルをAWS上で比較・利用したい場合。 |
