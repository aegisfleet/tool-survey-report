...
quick_summary:
  ...
  target_users:
    ...
    - バックエンドエンジニア
    - 大規模プロジェクト・モノレポ開発者
  latest_highlight: Yarn 4.18.0 リリース、およびセキュリティ向上のための enableScripts のデフォルト false 化
  update_frequency: 中
evaluation:
  score: 85
  base_score: 70
...
* **アーキテクチャ**: ローカルファーストのパッケージマネージャーであり、Yarn Berry (v2以降) はコアが拡張可能なプラグインアーキテクチャを採用しています。
* **主要コンポーネントとデータフロー**:
  * PnP (Plug'n'Play)を利用する場合、巨大な `node_modules` フォルダを生成せず、代わりに `.pnp.cjs` ファイルで依存関係ツリーと実際のファイルパスのマッピングを管理します。
  * ダウンロードされたパッケージのzipファイル群はローカルの `.yarn/cache` フォルダに保存され、Node.jsのモジュール解決時にインターセプトされて直接読み込まれます。
* **特筆すべき要素技術**:
  * **Zero-Installs**: キャッシュされた依存関係（`.yarn/cache`）や設定ファイルをそのままGitリポジトリにコミットすることで、CI/CD環境や新規開発者の環境構築時に `yarn install` を実行する時間すら不要にする革新的な仕組みです。

```mermaid
graph TD
    subgraph プロジェクトリポジトリ
        A[package.json]
        B[yarn.lock]
        C[.pnp.cjs]
        D[.yarn/cache]
...
* **準拠規格**: Yarn自体はOSSのツールですが、セキュリティ監査コマンド（`yarn npm audit`等）を備えており、サプライチェーンセキュリティに寄与します。また、近年のアップデート（v4.14.0等）により、`enableScripts: false` や `npmMinimalAgeGate: 1d` などをデフォルトとすることで、悪意のあるパッケージのインストール後スクリプト実行や公開直後の不審なパッケージから環境を保護するセキュアな設計を推し進めています。
...
* **2026-07-29**: Yarn 4.18.0 リリース。依存関係解決のパフォーマンス向上（`npm` プラグインにおける安定版がない場合の事前リリース版解決の修正等）や `js-yaml` v4 への移行など。
* **2026-06-02**: Yarn 4.16.0 リリース。Node 24.16.0以降のEBADF回避ワークアラウンドの停止や `yarn npm stage` コマンドの実装。
* **2026-05-19**: Yarn 4.15.0 リリース。`npmMinimalAgeGate: 1d` をデフォルトとし、公開から1日以内のパッケージに対する警告・ブロックを強化。
* **2026-04-16**: Yarn 4.14.0 リリース。`enableScripts: false` をデフォルト化し、インストール後の自動スクリプト実行を無効化（セキュリティの向上）。

(出典: [Yarn Releases](https://github.com/yarnpkg/berry/releases))
