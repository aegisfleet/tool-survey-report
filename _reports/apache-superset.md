...
* **グローバルタスクフレームワーク**: バージョン6.1.0で追加された機能。タスクの実行をグローバルに管理可能。
* **動的通貨（Dynamic currency）**: データセットごとに動的に通貨を設定可能。
...
* **アーキテクチャ**: クラウド完結型SaaS（Presetなどのマネージド利用時）、またはローカル/オンプレミスで構築可能なクライアント・サーバー構成。
* **主要コンポーネントとデータフロー**:
  * フロントエンド（React）がユーザーの操作を受け付け、バックエンド（Python/Flask）のAPIを呼び出す。
  * バックエンドはSQLAlchemyを介して各種データベース（PostgreSQL, BigQuery, Snowflake等）にSQLクエリを発行し、結果を取得。
  * 取得したデータはフロントエンドに返され、ECharts等を用いて可視化される。
* **特筆すべき要素技術**:
  * Flask-AppBuilderによる堅牢な認証・認可システム。
  * Celeryを用いた非同期クエリ実行（長時間実行されるクエリ用）。
  * RedisやMemcached等を用いた強力なキャッシュ機構（パフォーマンス向上のため）。
...
* **2026-05-13**: バージョン 6.1.0 をリリース。グローバルタスクフレームワークの追加、動的通貨（Dynamic currency）対応の追加、MCPチャート更新時のプレビューフロー追加など、多数の機能追加と修正を実施。
* **2025-12-18**: メジャーバージョン 6.0.0 をリリース（多数の新機能追加とバグ修正）。
* **2025-12-04**: バージョン 6.0.0 のリリース候補版 (6.0.0rc4) を公開。
* **2025-11-20**: バージョン 6.0.0 のリリース候補版 (6.0.0rc3) を公開。
