...
* **関連リンク**:
  * GitHub: [https://github.com/agentsmd/agents.md](https://github.com/agentsmd/agents.md)
  * ドキュメント: [https://agents.md/](https://agents.md/)
* **カテゴリ**: AIエージェント基盤
...
* **アーキテクチャ**: ファイルベースのコンテキスト共有アーキテクチャ（特定のサーバーやシステムを持たない静的ファイル群）。
* **主要コンポーネントとデータフロー**:
  * 開発者がプロジェクトのルート（または各サブディレクトリ）に `AGENTS.md` を配置する。
  * 対応するAIエージェント（Clineなど）が起動時やタスク実行時にリポジトリ内の `AGENTS.md` を読み込む。
  * ファイルに記述された指示やコンテキストをプロンプトに統合し、生成や操作の精度を向上させる。
* **特筆すべき要素技術**:
  * Markdown形式（シンプルでパースが容易）
  * ディレクトリツリーに基づくネスト構造（サブディレクトリごとの個別ルールの適用）

```mermaid
flowchart TD
    Dev[開発者] -->|作成・更新| File["AGENTS.md
(リポジトリ内)"]
    Agent["AIコーディングエージェント
(Cline, Aider等)"] -->|自動読み込み| File
    File -->|コンテキスト・指示の提供| Agent
    Agent -->|文脈に沿った実行| Target[ソースコード・コマンド実行]
```
...
* **調査対象**: 公式サイト、技術ブログ
* **総合評価**: G2、Capterra、ITreviewにレビューの登録なし。コンセプトへの賛同は多い。
* **ポジティブな評価**:
...
* **2026-09-10**: プロジェクトの運営方針を定めるTechnical Charterがリポジトリに追加された。
* **2026-03-10**: Augment Code、Junieエージェントへの対応およびGemini CLIに関するドキュメントが追加・更新された。
* **2025-12-08**: Cognition社のWindsurfへの対応が追加された。
* **2025-12-05**: Block社のオープンソースAIエージェントGooseへの対応が追加された。
* **2025-11-13**: Cline (v3.37.0) が `AGENTS.md` のサポートを追加。
* **2025-10-XX**: GitHub Copilot、Devin、Ona、UiPathなど多数のエージェントの互換性リストが拡充された。
* **2025-09-09**: VS Codeのサポートに関する情報が追加された。

(出典: [AGENTS.md GitHub Releases / Commits](https://github.com/agentsmd/agents.md/commits/main))
