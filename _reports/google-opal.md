...
* **グローバル展開の開始**: 2025年11月、日本を含む15カ国で新たに利用可能になりました。これにより、これまで米国内に限定されていた利用地域が大幅に拡大しました。(出典: [Google公式ブログ](https://blog.google/technology/google-labs/opal-expansion/))
* **デバッグ機能の強化**: ワークフローをステップごとに実行し、エラー箇所をリアルタイムで特定できるノーコードのデバッグ機能が追加されました。
* **パフォーマンスの向上**: アプリ作成時の応答速度が向上し、複数のステップを同時に実行できる並列処理が導入されたことで、複雑なワークフローの待機時間が短縮されました。
...
* **調査担当者としての総合的な評価**:
  ...
  * 2025年11月には日本を含むグローバル展開が開始され、これまで課題であった利用地域の制限が解消されたことで、より多くのユーザーにとって実用的な選択肢となりました。
  * デバッグ機能の強化やパフォーマンス改善も行われており、単なる実験的ツールから、より信頼性の高いプラットフォームへと進化しています。今後、さらに多くの機能追加や改善が期待される、動向を注視すべきツールです。
