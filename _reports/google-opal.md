...
quick_summary:
  ...
  target_users:
    ...
    - マーケター
    - クリエイター
  latest_highlight: 2026年2月24日にFAQが更新され、Opalアプリがデフォルトでプライベートであることや、共有権限（Remix含む）に関する仕様がより明確に記載された
  update_frequency: 高
evaluation:
  score: 80
  base_score: 70
...
* **データ管理**: ユーザーデータはGoogleのプライバシーポリシーに従って管理される。作成したOpalアプリはデフォルトでプライベートであり、入力プロンプトや生成物も含め、ユーザーの許可なく共有されることはない。また、FAQによると入力や出力データは生成AIの学習には使用されないと明記されている。
...
* **2026-02-24**: FAQが更新され、Opalアプリがデフォルトでプライベートであることや、共有・Remix権限の仕様、データ利用ポリシーがより明確に記載された。また、多数の国と地域での利用可能性が改めて示された。
* **2025-11-10**: FAQの更新と、日本を含む多くの国での利用が可能になったことが明記された
* **2025-10-15**: ワークフロー内でのデバッグ機能強化と、ステップ実行の可視化改善（推定）
* **2025-09-20**: レスポンス速度の向上と、並列処理によるワークフロー実行の高速化（推定）

(出典: [FAQドキュメント](https://developers.google.com/opal/faq))
