...
* **Managed Data Stores**: フルマネージドなPostgreSQLとRedis（Key Value）を提供。バックアップやポイントインタイムリカバリ（PITR）にも対応。PostgreSQLはコネクションプーリング(PgBouncer)に対応し、多数のクライアント接続を効率的に処理可能。
...
* **Jules & MCP Integration**: Google LabsのAI「Jules」と統合された自律的デバッグ機能に加え、Render MCPサーバーによりClaude Code、Codex、CursorなどのAIツールからのリソース管理・デプロイ操作（OAuth認証対応）が可能。
* **Managed OIDC Authentication**: AWS IAMだけでなくAnthropicやOpenAIへのOIDC認証をサポート。APIキーをハードコードすることなくセキュアにAIプロバイダと連携。

## **4. 動作原理・システム構成**

* **アーキテクチャ**: クラウド完結型PaaS
* **主要コンポーネントとデータフロー**:
  * GitHub/GitLabなどのGitリポジトリを起点とし、コードのプッシュをトリガーにRenderのビルドサーバーがコンテナをビルド。
  * 成果物は自動的にデプロイされ、Cloudflareをベースとしたエッジネットワーク経由で配信される。
  * Webサービスとデータベース間はプライベートネットワークで接続。
* **特筆すべき要素技術**:
  * コンテナランタイムベースのアーキテクチャにより、Dockerfileを用いた任意の言語環境の実行が可能。
  * **PgBouncer**: 多数のDB接続を効率的に管理するコネクションプーリング。
  * **Blueprints (IaC)**: `render.yaml` によるインフラ構成のコード管理。
...
* **認証**: GitHub/GitLab/Google/Emailでのログイン。2要素認証（2FA）対応。SSOはOrganizationプラン以上。また、Managed OIDCを通じてAWS IAM、Anthropic、OpenAIへのシームレスでセキュアな一時的認証情報（トークン）を発行・ローテーションすることが可能。
...
* **2026-08-07**: **Reduced Build Times**: ビルドノードのアップグレードにより、全ランタイムの平均ビルド時間を40%削減。
* **2026-07-24**: **Managed OIDC for AI**: Managed OIDCがAnthropicおよびOpenAIをサポート開始。
* **2026-07-22**: **Render MCP OAuth**: Render MCPサーバーがClaude Code、Codex、Cursor向けのOAuth認証をサポート。
* **2026-07-15**: **Render-to-AWS OIDC**: AWS IAMに対するOIDC認証がProプラン以上で一般提供（GA）開始。
* **2026-07-01**: **Postgres Connection Pooling**: 有料のPostgresインスタンスでPgBouncerによるコネクションプーリングが利用可能に。
...
| **AI・セキュリティ連携** | AIモデル連携/OIDC認証 | ◎<br><small>Anthropic, OpenAI等へのOIDC対応、MCP対応</small> | ◎<br><small>AI SDK, AI Gatewayが強力</small> | △<br><small>アドオン依存</small> | ◯<br><small>Bedrock連携可能</small> |
