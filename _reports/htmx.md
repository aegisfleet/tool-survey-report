...
quick_summary:
  ...
  target_users:
    - バックエンドエンジニア
    - フロントエンドエンジニア
  latest_highlight: v4.0.0の安定版がリリースされ、v2系もマイナーアップデートが継続中。
  update_frequency: 高
evaluation:
  score: 88
  base_score: 70
...
* **アーキテクチャ**: クライアント・サーバー型（Hypermedia-Driven Application）
* **主要コンポーネントとデータフロー**:
  * クライアント側: `htmx` がHTML属性（`hx-get`, `hx-post`等）を解釈し、指定されたトリガーでサーバーへAJAXリクエストを送信する。
  * サーバー側: JSONではなく、HTML片（フラグメント）を生成してレスポンスとして返す。
  * DOM更新: `htmx` がレスポンスのHTMLを受け取り、`hx-swap` の指定に従って現在のDOMの一部を直接置換する。

```mermaid
sequenceDiagram
    participant Browser as ブラウザ (htmx)
    participant Server as サーバー (Django, Rails, Go等)

    Browser->>Server: HTTPリクエスト (AJAX)
    Note over Browser,Server: HX-Request: true などの専用ヘッダーが付与される
    Server-->>Browser: HTMLフラグメント (レスポンス)
    Note over Browser: DOMの一部を新しいHTMLで置換
```

* **特筆すべき要素技術**:
  * AJAX、CSS Transitions、WebSockets、Server-Sent Events をHTMLの拡張として直接扱える。
...
* **2026-09-06**: v2.0.10 リリース（TypeScript定義ファイルの復元、エスケープ処理の改善など）、v2.0.8 リリース（Document.parseHTMLUnsafe()の利用等）
* **2026-08-28**: v4.0.0 リリース（メジャーアップデート、内部アーキテクチャの刷新など）
* **2026-04-15**: v2.0.9 リリース（HX-Locationの修正、パス解決のバグ修正など）
...
| **アーキテクチャ** | サーバー主導 | ◎<br><small>HTMLを返す</small> | ◎<br><small>標準機能化</small> | △<br><small>Nuxt等で対応</small> | ◎<br><small>HTMLを返す</small> |
...
| **React** | 世界で最も使われているUIライブラリ。RSCによるフルスタック化。 | 圧倒的なエコシステム、CompilerによるDX向上、求人の多さ。 | 学習コストが高く、ビルド環境の構築が必要。 | 大規模で複雑なSPAの開発、クロスプラットフォーム展開を視野に入れる場合。 |
