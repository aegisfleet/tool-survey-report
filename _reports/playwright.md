...
* **UIの日本語化**: Playwrightのツール（Inspector, Trace Viewer 등）のUIは英語のみです。
* **ドキュメントの日本語化**: 公式ドキュメントは英語のみで、日本語版は提供されていません。しかし、APIが直感的であるため、日本の開発者コミュニティでもブログ記事や勉強会などを通じて情報共有が活発に行われています。

## **10. 連携機能 (API・インテグレーション)**
...
Playwrightは月に1〜2回の頻度で頻繁にアップデートされており、継続的に機能改善や新機能の追加が行われています。直近の主要なアップデートは以下の通りです。

* **v1.56 (2025年10月)**:
  * **Playwright Agents**: LLM（大規模言語モデル）を活用してテストの計画・生成・修正を支援するエージェント機能を導入。
  * **新API追加**: `page.consoleMessages()`, `page.pageErrors()` など、ページの状態を取得するAPIを拡充。
* **v1.55 (2025年8月)**:
  * **Codegenの機能強化**: UI操作の記録時に `toBeVisible()` アサーションを自動で生成するオプションを追加。
* **v1.54 (2025年7月)**:
  * **Node.js 16のサポート終了** および **Node.js 18の非推奨化**。
  * CHIPS (Cookies Having Independent Partitioned State) をサポートするための新しいCookieプロパティ `partitionKey` を追加。
* **v1.53 (2025年6月)**:
  * **Trace Viewer UIの改善**: テスト内のステップがより詳細に表示されるようになり、デバッグ体験が向上。
