...
quick_summary:
  ...
  target_users:
    - Web開発者
    - QAエンジニア
  latest_highlight: 2026年7月にコンポーネントテストの新モデルとAbortSignalを導入したv1.62.0をリリース
  update_frequency: 高
evaluation:
  score: 92
  base_score: 70
...
* **コンポーネントテストの新モデル**: StoryとGalleryを用いたモデルを採用し、`fixtures.mount()` を用いて特定のシナリオでのコンポーネントのテストと型チェックが可能（v1.62〜）。
* **WebAuthn パスキー対応**: ハードウェアキー不要で、仮想オーセンティケーターを用いてパスキー認証のシナリオをテスト可能（v1.61〜）。
* **Screencast API**: アクションのアノテーションやリアルタイムのフレームキャプチャ、動画レシート等を統一されたインターフェースで記録可能（v1.59〜）。
...
* **アーキテクチャ**: ローカル環境で実行されるNode.jsクライアントと、ブラウザを操作するサーバープロセスのクライアント・サーバー構成（WebSocket通信）。
* **主要コンポーネントとデータフロー**:
  * Playwright Testランナーがテストスクリプトを解析し、ワーカープロセスを起動します。
  * 各ワーカーは独自のブラウザインスタンスを起動し、テストごとに独立したブラウザコンテキスト（インコグニートウィンドウに相当）を生成してテストを実行します。
  * クライアントからのコマンドはWebSocketプロトコルを通じてブラウザプロセスに送信され、結果が返されます。
* **特筆すべき要素技術**:
  * **ブラウザコンテキスト**: OSレベルの分離を必要とせず、数ミリ秒でクリーンなテスト環境を提供する技術。
  * **Chrome DevTools Protocol (CDP)**: Chromiumの操作にCDPを拡張して利用。Firefox/WebKitに対しても独自のパッチを適用したバイナリを使用して同等の制御を実現。

```mermaid
graph LR
    A[Playwright Test Runner] -->|起動・管理| B(Worker Process 1)
    A -->|起動・管理| C(Worker Process N)
    B -->|WebSocket| D[Browser Chromium/Firefox/WebKit]
    D --> E[Browser Context 1]
    D --> F[Browser Context 2]
```
...
* **認証**: HTTP認証、OAuth、Cookieベースの認証に加え、仮想オーセンティケーターを用いたWebAuthnパスキー認証(v1.61~)など、Webアプリのあらゆる認証フローを自動化可能。認証状態を保存して再利用する機能もあります。
...
* **2026-07-24 (v1.62.0)**: コンポーネントテストの新モデル（Stories and Galleries）への移行と、`AbortSignal` を用いた操作のキャンセル機能の追加。
* **2026-06-15 (v1.61.0)**: 仮想オーセンティケーターを用いたWebAuthnパスキー認証のサポートを追加。
* **2026-05-11 (v1.60.0)**: `tracing.startHar()` によるHAR記録のネイティブサポートと、外部からのドラッグアンドドロップをシミュレートする `locator.drop()` の追加。
* **2026-04-01 (v1.59.0)**: 動画記録やアクションの注釈、リアルタイムキャプチャ機能を提供する新しい `page.screencast` APIを追加。
...
|:---:|:---|:---:|:---:|:---:|
| **基本性能** | 実行速度・安定性 | ◎<br><small>自動待機が強力</small> | ◯<br><small>標準的</small> | △<br><small>不安定になりがち</small> |
