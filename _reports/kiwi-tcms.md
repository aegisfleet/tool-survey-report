...
quick_summary:
  ...
  target_users:
    ...
    - 開発者
    - QAマネージャー
  latest_highlight: 2026年8月にバージョン16.3をリリース（セキュリティ強化・改善）
  update_frequency: 高
evaluation:
  score: 78
  base_score: 70
...
* **アーキテクチャ**: クライアント・サーバー型アーキテクチャ。オンプレミスまたはクラウドでのコンテナ（Docker）ベースの運用が標準。
* **主要コンポーネントとデータフロー**:
  * **フロントエンド**: HTML/CSS/JavaScriptベースのWebインターフェース（Patternfly採用）。
  * **バックエンド**: PythonおよびDjangoフレームワークで構築されたAPIおよびWebアプリケーションサーバー。
  * **データベース**: テストケースや実行結果を保存するためのリレーショナルデータベース（標準でPostgreSQL）。
  * **データフロー**:
    1. ユーザーまたはCI/CDツール（自動テスト）がAPI/Web画面経由でテスト結果を送信。
    2. バックエンドがデータを受信し、データベースに記録。
    3. テスト失敗時など、必要に応じてJira等のバグトラッカーとAPI連携してバグチケットを自動起票。
* **特筆すべき要素技術**:
  * **Dockerコンテナファースト**: 全てがコンテナ化されており、環境構築とアップデートが容易。
  * **RPC API**: JSON-RPC / XML-RPC APIを提供し、高度な自動化連携を実現。

```mermaid
graph TD
    subgraph クライアント
        User[ユーザー<br>QA/開発者]
        CICD[CI/CDパイプライン<br>自動テスト]
    end

    subgraph Kiwi TCMSサーバー
        WebUI[フロントエンド<br>Web UI]
        API[API<br>JSON-RPC/XML-RPC]
        Backend[バックエンド<br>Python/Django]
        DB[(PostgreSQL<br>データベース)]
    end

    subgraph 外部システム
        Tracker[バグトラッカー<br>Jira/GitHub/等]
        Auth[外部認証システム<br>LDAP/OAuth]
    end
...
* **2026-08-24**: **Kiwi TCMS 16.3** - 重要なセキュリティ関連の更新および複数の改善。
* **2026-07-22**: **Kiwi TCMS 16.2** - 機能強化、バグ修正および依存関係の更新。
* **2026-06-24**: **Kiwi TCMS 16.1** - パフォーマンス向上とシステムの安定性改善。
* **2026-06-05**: **Kiwi TCMS 16.0** - メジャーバージョンアップ、新機能追加およびUI改善。
...
|---------|------|------|------|------------------|
| **Kiwi TCMS** | オープンソース | コスト、カスタマイズ性、オンプレミス | 日本語対応なし、運用管理が必要 | コストを抑えたい、技術力がある、オンプレ必須の場合 |
| **TestRail** | 商用スタンダード | 使いやすさ、サポート、Jira連携、AI機能(TestRail 10.2) | コストが高い、SaaSメイン | 予算があり、標準的なサポートと使いやすさ、AIによるテスト生成を求める場合 |
| **Qase** | モダンSaaS | 洗練されたUI/UX、AIDEN(AI機能)、高速動作 | 日本語対応なし、歴史が浅い | 最新のUI体験と高度なAIアシスタント(AIDEN)機能を重視する場合 |
