...
quick_summary:
  ...
  target_users:
    ...
    - エンタープライズ開発チーム
    - OSSコントリビューター
  latest_highlight: 2026-06リリースでJava 26をサポート
  update_frequency: 高
evaluation:
  score: 80
  base_score: 70
...
* **アーキテクチャ**: ローカル環境で動作するプラグインベースの統合開発環境。
* **主要コンポーネントとデータフロー**:
  * **ワークスペース**: すべてのプロジェクト設定、ソースコード、メタデータが保存されるディレクトリ。Eclipseはこのディレクトリ内のファイルを監視・管理する。
  * **JDT (Java Development Tools)**: ユーザーがコードを編集・保存するたびに、バックグラウンドでインクリメンタルビルド（差分コンパイル）を実行し、即座にエラーの検出や補完候補の生成を行う。
* **特筆すべき要素技術**:
  * **OSGiフレームワーク**: Eclipseの核となるプラグインアーキテクチャ。各機能（JDT、CDT、PDE等）はすべてOSGiバンドルとして実装されており、疎結合かつ動的なモジュールの追加・削除が可能。

```mermaid
flowchart TD
    User([ユーザー]) -->|コード編集/保存| Editor[エディタ]

    subgraph Eclipse IDE
        Editor -->|変更通知| JDT[JDT<br>インクリメンタルビルド]
        JDT -->|コンパイル/解析| Workspace[(ワークスペース<br>ローカルファイル)]
        Workspace -->|クラスファイル/エラー情報| UI[UI/パースペクティブ]

        PluginManager[OSGi フレームワーク] -.->|モジュール管理| JDT
        PluginManager -.->|モジュール管理| OtherPlugins[その他のプラグイン<br>Git, Maven等]
    end
...
* **2026-06-01 (2026-06)**: Java 26サポートの追加、ステートメントレベルのステップフィルタリングやバイトコード命令ハイライトなどのデバッグ改善、プラグイン開発ツールの改善など。
* **2026-03-01 (2026-03)**: 2026年第1四半期の定期リリース（バグ修正およびコンポーネントアップデート）。
...
(出典: [Eclipse New and Noteworthy](https://eclipseide.org/release/noteworthy/))
