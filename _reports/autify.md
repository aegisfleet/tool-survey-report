...
* **Eメールテスト**: ユーザー登録通知や購入確認メールなど、メールの受信と内容の検証を自動化できます。
* **ビジュアルリグレッションテスト**: UIの差分を自動で検出し、意図しないデザイン崩れを防ぎます。
* **ステップグループ**: ログイン処理など、繰り返し利用する一連の操作をグループ化して再利用できます。
...
* **情報源**: G2.com, TrustRadiusなどの海外レビューサイトの情報を集約。
* **ポジティブな評価**:
  * **簡単な操作性**: 多くのユーザーが、プログラミング知識を必要としないノーコードのアプローチを高く評価。直感的なUIで学習コストが低く、QA担当者以外もテスト作成に参加しやすい点が強みとして挙げられている。
  * **AIによるメンテナンス**: UI変更に追従するAIの自動修復機能により、テストシナリオのメンテナンス負荷が大幅に削減されるという声が多い。
* **改善要望**:
  * (今回の調査でアクセスできた情報からは、具体的な改善要望や否定的な評価は見当たらなかった)

## **13. 直近半年のアップデート情報**

* **リリースノート**: 公式サイトのリリースノートはJavaScriptで動的にコンテンツを生成しているため、本調査のエージェントでは自動取得できませんでした。
* **開発動向**: 公式サイトの情報からは、近年は特にAIを活用した機能（Genesis 2.0, Nexus）の開発に注力していることが伺えます。テストの設計から作成、実行、メンテナンスまで、AIによる支援範囲を拡大する方向性が見られます。
