...
quick_summary:
  ...
  target_users:
    ...
    - SRE
    - 大規模チーム
  latest_highlight: v2.1.8リリース (2026-05-19)
  update_frequency: 高
evaluation:
  score: 85
  base_score: 70
...
* **非Gitフック対応**: Git環境以外でも動作するフックのインストールと実行をサポート（v2.1.0以降）。
* **タイムアウト制御**: 実行するコマンドやスクリプトに対してタイムアウト（timeout）を設定し、ハングアップを防止可能（v2.0.16以降）。
...
* **2026-05-19 (v2.1.8 / v2.1.7)**: バグ修正とパッケージングの改善。ローカルフックパスの警告修正や、ステージング変更の復元処理の改善など。
* **2026-04-16 (v2.1.6)**: Windows環境でのパス正規化や、PTY割り当てのスキップ処理などエッジケースの修正。
* **2026-02-03 (v2.1.0)**: 非Gitフック (non-git hooks) のインストールと実行をサポート。
* **2026-01-27 (v2.0.16)**: タイムアウト（timeout）引数の追加とパフォーマンスの最適化。
