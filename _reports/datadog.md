...
quick_summary:
  ...
  target_users:
    ...
    - SRE
    - 開発者
  latest_highlight: Datadog Agent 7.83.2 をリリースしAPM等の機能強化を実施 (2026年9月)
  update_frequency: 高
evaluation:
  score: 88
  base_score: 70
...
* **アーキテクチャ**: クラウド完結型SaaSと、各ホストやコンテナにデプロイされるDatadog Agent（クライアント）のハイブリッド構成。
* **主要コンポーネントとデータフロー**:
  * データ収集（エージェント）: 監視対象のサーバー、コンテナ（Kubernetes等）、クラウドプロバイダー（AWS、GCP、Azure）にDatadog Agentをインストールするか、クラウド側のAPI（例: CloudWatch Metric Streams）を通じてメトリクス、ログ、トレースを収集。
  * 処理と保存（バックエンド）: 収集されたデータはDatadogのSaaSプラットフォームへHTTPS経由で送信され、ストリーム処理による高速な評価（アラート評価等）と、長期保存用データストアへの格納が行われる。
  * 可視化（フロントエンド）: ユーザーはWebブラウザを通じてSaaSダッシュボードにアクセスし、統合されたデータから分析を行う。

```mermaid
flowchart TD
    subgraph ClientEnvironment["クライアント環境 (インフラ/アプリ)"]
        A[Datadog Agent]
        B[クラウドインテグレーションAPI]
        C["アプリケーション(APM/ログ)"]
    end

    subgraph DatadogSaaS["Datadog SaaSプラットフォーム"]
        D["データインジェスト & ストリーム処理"]
        E[("(時系列データベース)")]
        F[("(ログ/トレースストア)")]
        G["アラート・機械学習エンジン"]
    ...
    User[運用者 / 開発者] -- "ブラウザアクセス" --> H
```

* **特筆すべき要素技術**:
  * **Datadog Agent**: Go言語とPythonで記述された軽量なオープンソースのエージェント。数百種類のインテグレーション（チェック）をサポートする。
  * **Unified Service Tagging**: すべてのデータ（メトリクス、トレース、ログ）に一貫したタグ（例: `env`, `service`, `version`）を付与することで、シームレスな相関分析を実現。
...
* **調査対象**: Google検索結果からの引用（G2, Capterraのレビュー内容）
* **総合評価**: 4.4/5.0 (G2), 4.6/5.0 (Capterra) と非常に高い評価（Google検索結果より）。
* **ポジティブな評価**:
...
* **2026-09-16**: Datadog Agent 7.83.2 をリリース。
* **2026-09-09**: Datadog Agent 7.83.1 をリリース。
* **2026-08-05**: Datadog Cluster Agent 7.82.0 リリース。Prometheus HTTP Service Discoveryへの`exclude_filter`フィールド追加など。
* **2026-07-22**: Datadog Agent 7.81.2 リリース。メトリクス転送にデフォルトでDatadog v3 intake APIを使用するよう変更。
* **2026-04-23**: Datadog Agent 7.78.1 をリリース。Embedded PythonやGoのバージョンアップ、不具合修正を実施。
* **2026-04-15**: Datadog Agent 7.78.0 をリリース。APM OTLPの属性優先順位の挙動変更など。
...
(出典: [Datadog Agent Releases](https://github.com/DataDog/datadog-agent/releases) / [Datadog Blog](https://www.datadoghq.com/blog/))
...
| **AI/自動化** | AI監視 | ◎<br><small>LLM Obs/AI Guard</small> | ◎<br><small>New Relic AI</small> | ◯<br><small>AI Assistant</small> | ◯<br><small>Grafana Assistant (v13.0~)</small> |
| **ダッシュボード** | 共同作業 | ◎<br><small>リアルタイム編集・共有</small> | ◎<br><small>Notebooks (GA版)</small> | ◯<br><small>ダッシュボードスタジオ</small> | ◎<br><small>Git Sync対応</small> |
...
| **New Relic** | APMに強みを持つオブザーバビリティプラットフォーム | 開発者向けの機能が充実(Notebooks等)。100GB無料の寛大なプラン。従量課金がシンプル。 | 大規模データ時のコスト管理が必要。NRQL学習が必要。 | コストを抑えてフルスタック監視を始めたい、開発者主体の組織。 |
| **Splunk** | ログ管理とセキュリティ分析(SIEM)のパイオニア | Gartner SIEMの11回連続リーダー。膨大なログ分析やセキュリティ要件に強い。 | 非常に高価。学習コストが高い。 | ログ分析やSIEMが主目的で、コンプライアンス要件が厳しい場合。 |
| **Grafana** | 可視化特化のOSSプラットフォーム | v13.0からのGit Syncなど高度な可視化と運用性。データソースを選ばない柔軟性。 | データ保存機能はない（別途DBが必要）。構築・運用スキルが必要。 | コストを抑えたい、既存のOSSスタック（Prometheus等）を活用したい場合。 |
