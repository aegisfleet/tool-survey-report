...
* **豊富なプラグインエコシステム**: 2000を超えるプラグインが利用可能で、Git, Maven, Docker, Kubernetes, Slackなど、ほぼすべての開発・運用ツールと連携できる。
...
* **UI/UX**: 伝統的なWeb UIは、多機能である一方、情報量が多く、初見では少し分かりにくい部分もある。近年、Blue OceanというモダンなUIが導入され、パイプラインの視覚化や操作性が向上しているが、まだすべての機能が対応しているわけではない。
...
* **調査対象**: G2, Capterra, TrustRadiusなどの海外レビューサイトおよび国内技術ブログ記事の genelal な評価。
* **総合評価**: 5段階評価で4.0~4.5程度のスコアを獲得していることが多い。
* **ポジティブな評価**:
  * 「プラグインが豊富で、やりたいことはほぼ何でも実現できる」
  * 「無料で利用できるのが最大の魅力」
  * 「オンプレミスで構築できるため、セキュリティ面で安心」
  * 「パイプラインをコードで管理できる(Jenkinsfile)のが良い」
* **ネガティブな評価 / 改善要望**:
  * 「サーバーのメンテナンスやプラグインの管理が大変」
  * 「UIが古くて使いにくい」
  * 「初心者が使いこなすには学習コストが高い」
  * 「時々、プラグインの競合で不安定になることがある」
* **特徴的なユースケース**:
...
* **2024年10月 (2.462.3)**
  * Java 11をサポートする最後のLTSリリース。今後のLTSではJava 17または21が必須に。
  * 重要なセキュリティ修正。
* **2024年9月 (2.462.2)**
  * Spring FrameworkおよびSpring Securityのアップグレード。
  * SecretTextAreaフィールドのフォームバリデーションが機能するように修正。
* **2024年8月 (2.462.1, 2.452.4)**
  * 重要なセキュリティ修正。
  * Commons FileUploadライブラリのアップグレード。
  * 「New item」ページのUIをリフレッシュ。
* **2024年7月 (2.452.3)**
  * ユーザーがブラウザで代替言語オプションを定義している場合でも、ヘルプテキストが正しいロケールで表示されるように修正。
* **2024年6月 (2.452.2)**
  * クラウドプラグインの実装における競合を避けるため、CloudSetクエリパラメータの型名を変更。
* **2024年5月 (2.452.1)**
  * WebSocketなどを使用したインバウンドエージェントの再設定後の起動手順の不具合を修正。
* **アップデートの傾向**:
  * **UI/UXの近代化**: UIコンポーネントのリフレッシュや改善が継続的に行われている。
  * **依存ライブラリの更新**: Spring、Jetty、Commons FileUploadなど、主要な内部ライブラリが定期的に更新されている。
  * **セキュリティ**: 毎月のように重要なセキュリティ修正がリリースされており、安全性の維持に注力している。
  * **Javaバージョンの移行**: Java 17/21への移行が完了し、Java 11のサポートが終了した。
