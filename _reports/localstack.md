...
quick_summary:
  ...
  target_users:
    ...
    - バックエンド開発者
    - DevOpsエンジニア
  latest_highlight: 2026年2月にv4.14.0をリリース。リソースグループタグ付けAPIプロバイダーの追加（EC2、KMS、S3等に統合）、MSKでのKRaftクラスタサポートなどを追加。
  update_frequency: 高
evaluation:
  score: 88
  base_score: 70
...
* **Resource Groups Tagging APIプロバイダー**: EC2、KMS、S3、Lambda、Route 53などのサービスネイティブなタグ付けを統合。
* **Amazon MSK KRaftクラスタ**: MSKでのKRaftベースクラスタをサポートし、Kafkaのローカルエミュレーションを強化。

## **4. 動作原理・システム構成**

* **アーキテクチャ**: ローカルファーストのDockerベースクライアント・サーバー構成。
* **主要コンポーネントとデータフロー**:
  * LocalStackコンテナ内でAPI Gateway、S3、Lambdaなどのエミュレーションエンジンが稼働し、開発者のAWS CLI/SDKからのリクエストを処理。
* **特筆すべき要素技術**:
  * Pythonベース。一部サービスで実エンジン(Dockerコンテナ)やMotoライブラリを使用。
...
* **2026-02-26 (v4.14.0)**: 新しいResource Groups Tagging APIプロバイダーの追加（EC2、KMS、S3、Lambda等のタグ付けを統合）、Amazon MSKでのKRaftベースクラスタサポート、HiveのデフォルトメタストアとしてのGlue統合、EKSノードのIMDS統合（Kubernetes 1.34/1.35サポート）。CloudFormationのDeletionPolicy/UpdateReplacePolicy対応。
...
| 機能カテゴリ | 機能項目 | LocalStack | Moto | AWS Sandbox | fakecloud | Floci |
|:---:|:---|:---:|:---:|:---:|:---:|:---:|
| **エミュレーション** | 対応サービス数 | ◎<br><small>100+</small> | ◯<br><small>主要サービス</small> | ◎<br><small>全サービス(実物)</small> | △<br><small>主要22サービスをサポート</small> | ◯<br><small>主要47サービスをサポート</small> |
| **利用形態** | 実行環境 | ◯<br><small>Docker/単体</small> | ◯<br><small>Pythonライブラリ</small> | △<br><small>クラウド接続必須</small> | ◎<br><small>単一バイナリで動作（19MB）</small> | ◎<br><small>Docker連携（RDS等実エンジン）</small> |
| **コスト** | 利用料金 | ◯<br><small>無料〜有料</small> | ◎<br><small>完全無料(OSS)</small> | △<br><small>従量課金</small> | ◎<br><small>完全無料(OSS)</small> | ◎<br><small>完全無料(OSS)</small> |
| **開発効率** | デプロイ速度 | ◎<br><small>即時</small> | ◎<br><small>即時</small> | △<br><small>待ち時間あり</small> | ◎<br><small>即時</small> | ◎<br><small>即時（高速起動）</small> |

### **17.2 詳細比較**

| ツール名 | 特徴 | 強み | 弱み | 選択肢となるケース |
|---------|------|------|------|------------------|
| **LocalStack** | Dockerベースの包括的AWSエミュレータ | 対応サービス数が圧倒的に多い。IaC連携が強力。他言語からも利用容易。 | Community版の制限強化。リソース消費が大きい。 | 本格的なAWSアプリの統合テスト、広範なサービスを利用する場合。企業サポートが必要な場合。 |
...
| **fakecloud** | 完全無料・単一バイナリのAWSエミュレーター | アカウント不要、軽量で高速、高度な機能も無料 | 対応サービスが22種類に限定 | Dockerを使わずコストをかけずに主要AWSサービスのローカルテスト環境を構築したい場合 |
