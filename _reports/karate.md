...
quick_summary:
  ...
  target_users:
    - 開発者
    - QAエンジニア
  latest_highlight: v2.1.1にてJava 21+と仮想スレッドに対応 (2026年07月)
  update_frequency: 高
evaluation:
  score: 84
  base_score: 70
...
* **UI自動化 (Karate UI)**: Chrome, Firefox, Edge, Safariなどのブラウザを操作し、W3C WebDriverやCDPを使用した高度なWeb UIテストを実現します。
...
* **Java相互運用性と並列実行**: Java 21+の仮想スレッドを活用した高速な並列テスト実行をサポートし、既存のJavaクラスとも強力に連携できます。
* **内蔵JavaScriptエンジン**: 外部依存の少ない独自のJSエンジン（karate-js）を搭載し、高速な起動と柔軟なスクリプト記述が可能です。

## **4. 動作原理・システム構成**

* **アーキテクチャ**: ローカルファーストのクライアント・サーバー連携構成で、独自DSLとJSエンジンによるスクリプト実行フレームワークです。
* **主要コンポーネントとデータフロー**:
  * テストスクリプトは`.feature`ファイルで管理され、ローカルのファイルシステム上で実行されます。
  * **APIテスト**: HTTPクライアント（Java組み込みなど）を利用して、対象のWeb APIに直接リクエストを送信し、レスポンスを検証します。
  * **UIテスト**: W3C WebDriverまたはChrome DevTools Protocol (CDP) を通じてブラウザインスタンスを操作し、DOMの検証やユーザー操作をシミュレートします。
* **特筆すべき要素技術**:
  * **仮想スレッド**: Java 21+の仮想スレッド（Virtual Threads）を活用し、スレッドプールのチューニングなしで高並列なテスト実行を実現します。
  * **カスタムJSエンジン (`karate-js`)**: 組み込みの軽量なJavaScriptエンジンを使用し、テストスクリプト内のJS評価を高速化しています。

```mermaid
graph TD
    A[テストスクリプト <br> .featureファイル] --> B(Karate Runner)
    B --> C{テストタイプ}

    C -->|APIテスト| D[HTTPクライアント]
    D --> E[対象APIサーバー]

    C -->|UIテスト| F[ブラウザ操作]
    F -->|CDP / W3C WebDriver| G[ブラウザインスタンス <br> Chrome / Firefox 等]

    B -.-> H[内蔵JSエンジン <br> karate-js]
    B -.-> I[並列実行 <br> Java仮想スレッド]
```
...
* **特徴的なユースケース**:
  * APIテストの自動化と合わせて、テストシナリオをGatling経由で負荷テストにも再利用する高度な運用例が見られる。

## **16. 直近半年のアップデート情報**

* **2026-07-15**: v2.1.1リリース。HTMLレポートの大幅なパフォーマンス向上（メモリ使用量削減・高速化）、エラーログでの機能ファイル位置の特定機能強化、および変数スコープのバグ修正。
* **2026-06-17**: v2.1.0リリース。`karate-js`におけるES6クラスのサポート（継承、`super`など）、Ext SPIによるプラグイン拡張機能、ドライバーのOOPIF（Out-of-process iframe）サポートなど新機能追加。
* **2026-03-26**: v2.0.0 メジャーリリース。Java 21以上の仮想スレッドを活用した並列実行、軽量なカスタムJSエンジンの搭載（GraalJSからの移行）、W3C WebDriverのサポート追加など、根本的なアーキテクチャ刷新が行われた。
* **2025-01-06**: v1.5.2リリース。Java 21+環境での動作安定性を向上。Gatling統合時のレポート警告を修正。CVE対応のための依存ライブラリ（json-smart等）の更新。
* **2024-08-05**: v1.5.1リリース。CVE対応のための依存ライブラリ（netty等）の更新や、ClassgraphのアップデートによるQuarkus関連の問題を修正。
* **2024-07-26**: v1.5.0リリース。Java 17以上が必須要件に変更。Maven Group IDが `io.karatelabs` に変更され、Playwrightサポートが強化された。
...
| 機能カテゴリ | 機能項目 | Karate | Playwright | Cypress | Apache JMeter |
|:---:|:---|:---:|:---:|:---:|:---:|
| **テスト種類** | APIテスト | ◎<br><small>統合DSL</small> | ◯<br><small>補助機能</small> | △<br><small>可能だが主機能ではない</small> | ◯<br><small>プロトコル対応</small> |
| **テスト種類** | UIテスト | ◯<br><small>WebDriver/CDP対応</small> | ◎<br><small>全主要ブラウザ対応・高機能</small> | ◯<br><small>一部制限あり</small> | ×<br><small>非対応</small> |
| **非機能** | 負荷テスト | ◯<br><small>Gatling連携</small> | ×<br><small>専用ツール必要</small> | ×<br><small>専用ツール必要</small> | ◎<br><small>多機能・高拡張</small> |
| **開発体験** | 言語/DSL | ◎<br><small>Gherkin / JSエンジン内蔵</small> | ◯<br><small>TS/JS/Py/Java/C#</small> | ◯<br><small>JS/TS</small> | △<br><small>GUIベース中心</small> |

### **17.2 詳細比較**

| ツール名 | 特徴 | 強み | 弱み | 選択肢となるケース |
|---------|------|------|------|------------------|
| **Karate** | API/UI/負荷テスト統合型 | 単一ツールで広範囲をカバー、高い生産性、Java 21/仮想スレッドによる並列化 | IDE高度機能が有償、独自DSL | APIテストを中心にUI/負荷テストまで一元管理したい場合。 |
| **Playwright** | モダンE2Eテストツール | 高速で堅牢なブラウザ操作、デバッグ機能、AIエージェント対応 (v1.56+) | 負荷テスト機能なし | UIテスト（E2E）が主目的で、最新のWeb技術への対応を重視する場合。 |
| **Cypress** | E2Eテストツール | GUIデバッガの使いやすさ、自動待機機能 | Safariサポートが実験的、並列実行にコストがかかる | 開発プロセスに統合されたフロントエンドのE2Eテストを重視する場合。 |
| **Apache JMeter** | オープンソース負荷テストツール | 強力な負荷生成とプロトコル対応の幅広さ、GUI構築 | メモリ消費が大きくUIテスト機能を持たない | サーバーの限界を測定する大規模な負荷テストを行いたい場合。 |
