...
* **動画編集機能**:
  ...
  * **Storyboard**: タイムライン上で動画シーケンスを整理・編集。
  * **Loop**: シームレスなループ動画を作成。
  * **Blend**: 2つの動画を滑らかに結合。
* **SNSアプリ「Sora」 (iOS)**:
...
* **無料プラン**:
  * **Sora 2 (基本機能)**: 無料で利用可能。ただし、1日あたりの動画生成回数や生成される動画の長さに制限がある。個人利用や機能の試用向け。
  * **SNSアプリ「Sora」**: 無料で利用可能。動画の視聴、投稿、Cameo機能、リミックス機能が利用できる。将来的にサーバー負荷が高まった場合、追加の動画生成が有料になる可能性が示唆されている。
* **有料プラン**:
  * **ChatGPT Plus**: 月額20ドル。Soraの動画生成機能が含まれる。最大720pの画質で、最長10秒の動画を生成可能。生成される動画にはウォーターマークが表示される。
  * **Sora 2 Pro (ChatGPT Pro内)**: 月額200ドル。ChatGPT Proプランの一部として提供。高解像度（最大1080p）での生成、最長15〜20秒の動画生成に対応。ウォーターマークは表示されない。同時生成数もPlusプランより多い。
...
* **UI/UX**:
  * **Webブラウザ版**: プロンプト入力欄が中心のシンプルなインターフェース。生成された動画は一覧で確認でき、ダウンロードや削除といった基本的な操作が可能。
...
* **アップデートの傾向**: 単なる動画生成の品質向上だけでなく、「世界をシミュレートする」という大きな目標に向け、物理的な一貫性や音声を含むマルチモーダルな理解を深める方向に開発が進んでいる。また、SNSアプリの提供により、技術開発と並行してクリエイティブなコミュニティ形成と安全な利用方法の模索を重視している姿勢が見える。
