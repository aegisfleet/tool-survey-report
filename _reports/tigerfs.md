...
quick_summary:
  ...
  target_users:
    - 開発者
    - AIエージェント
  latest_highlight: v0.7.0で完全なトランザクションワークスペースとアンドゥ機能を追加
  update_frequency: 高
evaluation:
  score: 85
  base_score: 70
...
* **スキーマ管理**: `.create/`、`.modify/`、`.delete/`ディレクトリを利用してDDL変更をファイル操作として反映。`.tables/`ディレクトリでデータベース内のテーブル構造を閲覧可能。
* **トランザクションとアンドゥ (.undo/)**: すべての変更は`.log/`と`.savepoint/`に記録され、`.undo/`ディレクトリから一回のファイル操作で変更の取り消しが可能。
* **完全なPOSIX互換性**: ディレクトリ名・ファイル名の変更を伴うAtomic Rename（VimやClaude等のエディタが利用する置換）にネイティブ対応し、コンポジット主キーもサポート。

## **4. 動作原理・システム構成**

* **アーキテクチャ**: クライアント・サーバー型（ファイルシステム側とDBサーバー側）の構成。FUSE/NFSを用いてOSレベルでファイル操作をインターセプトし、SQLクエリに変換してPostgreSQLサーバーへ送信する。
* **主要コンポーネントとデータフロー**:
  * データはすべてバックエンドのPostgreSQLデータベース内に保存され、ファイルシステム上に実ファイルは存在しない。
  * ユーザーやAIエージェントのファイル操作（例：`cat`や`mv`）はTigerFSプロセスによって適切なSQLクエリ（`SELECT`や`UPDATE`等）に変換され、実行結果がファイル操作の結果として返却される。
* **特筆すべき要素技術**:
  * macOSでは依存関係のないNFSマウント、LinuxではFUSEを利用して仮想的なファイルシステムを構築。
  * バックエンドのPostgreSQLによる完全なACIDトランザクションにより、競合状態をデータベースレベルで解消。
...
* 開発の初期段階であり、今後のアップデートで仕様変更される可能性がある。
...
* **外部サービス連携**:
  ...
  * **AIエージェント統合**: Claude Code, Gemini CLI, Codexなどのコーディングエージェントがマウント時に検出されると、必要なエージェントスキルを自動的にインストールしてシームレスに連携。
...
* **2026-06-02**: v0.7.0リリース。ファイルシステムを完全なトランザクションワークスペースとして扱う機能（`.log/`, `.undo/`, `.savepoint/`）を追加。POSIXに準拠したAtomic Renameへの対応など安定性を向上。
* **2026-03-27**: v0.6.0リリース。専用の`tigerfs`スキーマへの分離、`tigerfs migrate`コマンドの追加、AIエージェントのスキル自動インストール機能を導入。
...
(出典: [製品アップデート情報](https://github.com/timescale/tigerfs/releases))
...
| **エンタープライズ** | データ復元・履歴・アンドゥ | ◎<br><small>全履歴自動保存、ワンタッチでアンドゥ可能</small> | ×<br><small>バックアップ依存</small> | ◎<br><small>コミット単位での履歴・復元</small> | △<br><small>別途ログや設定、複雑なクエリが必要</small> |
