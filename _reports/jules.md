...
* **Jules Tools (CLI)**: ターミナルから直接Julesを操作できるコマンドラインインターフェース。タスクの開始、停止、検証、複数タスクの並列実行などを実行でき、既存のCLIワークフローと統合可能。リポジトリの自動推論機能も備える。
...
* **強力なAIモデル**: 最新のGemini 3 Proを搭載しており、マルチステップの複雑なタスクにおいても、より一貫性のある計画と正確な実行が可能。マルチモーダル機能によるWebアプリのUI検証精度も向上している。
* **柔軟なタスク開始方法**: GitHubリポジトリに紐付いたタスクだけでなく、リポジトリ不要で即座にアイデアを試せる「Repolessモード」を提供し、プロトタイピングのハードルを大幅に下げている。
...
| プラン名 | 料金 | 主な特徴 |
|---|---|---|
| **Free/Intro** | 無料 | タスク数: 約15回/日, 同時実行数: 約3タスク |
| **Google AI Pro** | 約$19.99/月 | タスク数: 約100回/日, 同時実行数: 約15タスク |
| **Google AI Ultra** | 約$124.99/月 | タスク数: 約300回/日, 同時実行数: 約60タスク |
...
* **2025年11月20日**: **Repolessモードの導入**。リポジトリを選択せずにJulesのタスクを即座に開始できるようになった。これにより、アイデアの着想からコーディングまでの時間的オーバーヘッドを削減し、迅速なプロトタイピングを支援する。
* **2025年11月19日**: **Gemini 3 Proの搭載**。最新世代の「Gemini 3 Pro」モデルがJulesで利用可能に。より高度な推論能力、指示追従性、マルチモーダル機能の向上により、タスクの計画精度やWebアプリのUI検証能力が大幅に強化された。(Google AI Ultraユーザーから順次展開)
* **2025年11月10日**: **Jules Tools (CLI)の機能強化**。`--parallel`フラグによる複数タスクの並列実行、WSL/Arch Linux環境での認証問題の修正、リポジトリの自動推論機能、Side-by-Sideの差分表示機能など、多数の改善がリリースされた。
...
|---|---|---|---|---|
| **Google Jules** | **自律型エージェント**。タスクを指示し、完了後にPRを受け取る非同期ワークフロー。 | 複雑なタスクの自律実行、開発者のローカル環境非依存、非同期での並列作業。 | リアルタイムのコード補完や対話的な開発スタイルには不向き。 | 定型タスクの自動化や、複数タスクの並行処理を重視する場合。 |
| **GitHub Copilot** | **AIペアプログラマー**。エディタ内でリアルタイムにコード補完・生成を行う。 | リアルタイム性、対話的な開発スタイル、多くの言語・エディタに対応。 | 複数ファイルにまたがる複雑なタスクの自律実行はできない。 | コーディング中に常にAIの支援を受けながら、対話的に開発を進めたい場合。 |
| **Cursor** | **AI-firstコードエディタ**。コードベース全体を理解した上でのコード生成や編集、デバッグ支援。 | コードベース全体の深い理解、エディタに統合されたシームレスなAI体験。 | 特定のエディタ環境に縛られる。自律的なタスク実行機能はない。 | AIとの対話を前提とした新しい開発体験をエディタレベルで求める場合。 |
| **Amazon CodeWhisperer** | **AWS特化型AIコーダー**。AWSサービスに最適化されたコード補完・生成、セキュリティスキャン。 | AWSサービスとの親和性、セキュリティ脆弱性スキャン機能。 | AWS以外の環境での強みが少ない。自律的なタスク実行は不可。 | 主にAWSのエコシステム内で開発を行う場合。 |
