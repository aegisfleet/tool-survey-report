...
quick_summary:
  ...
  target_users:
    - 開発者
    - データサイエンティスト
  latest_highlight: v0.1.7リリースの公開
  update_frequency: 中
evaluation:
  score: 78
  base_score: 70
...
* **高度な抽出・変換処理**: v0.1.6より、埋め込み画像やPDFスキャンのためのOCRレイヤーサービスや、Azure Content Understandingコンバータのサポートが追加されました。


## **4. 動作原理・システム構成**

* **アーキテクチャ**: ローカル環境で動作するPythonライブラリ構成
* **主要コンポーネントとデータフロー**:
  * 各種ファイル（PDF、Word、Excel等）を入力として受け取り、内部のプラグインシステムまたはコンバータを通じてMarkdownテキストを出力します。
  * 外部API（OpenAIやAzureなど）を利用する拡張機能は、それぞれのサービスに対して必要な情報を送信し、解析結果を受け取って変換プロセスに統合します。

```mermaid
flowchart LR
    A[入力ファイル] --> B(MarkItDown Core)
    B --> C[Markdown出力]
    B -.-> D[OCR/Azure API等]
```
...
* **2026-07-29**: v0.1.7 リリース (PPTXグラフ変換の改善、LaTeXマクロ修正、SVG画像処理の修正など)
* **2026-05-26**: v0.1.6 リリース (埋め込み画像やPDFスキャン向けOCRレイヤー追加、Azure Content Understanding連携サポート、再帰的HTMLバグ修正など)
...
※注記: Unstructured および LlamaParse は本サイトに調査レポートが存在しないため、公式情報を基に記載しています。
