...
quick_summary:
  ...
  target_users:
    ...
    - データサイエンティスト
    - MLエンジニア
  latest_highlight: 2026年5月にVertex AI Extensionsが非推奨化され、当プラットフォームへの移行が推奨された
  update_frequency: 高
evaluation:
  score: 92
  base_score: 70
...
* **アーキテクチャ**: クラウドベースのフルマネージドAIエージェントプラットフォーム。
* **主要コンポーネントとデータフロー**:
  * Agent Studio、Model Gardenなどのフロントエンド/統合ツールから、基盤となるGeminiモデルやBigQuery等のデータソースへアクセスする。
  * ユーザーのプロンプトやタスク指示は、APIを通じてバックエンドのVertex AIインフラストラクチャ上で処理される。
* **特筆すべき要素技術**:
  * フルマネージドなMLOpsパイプライン。

```mermaid
graph TD
    A[ユーザー/開発者] -->|プロンプト/データ| B(Agent Studio)
    B --> C{Agent Platform}
    C --> D[Gemini 3 モデル]
    C --> E[Model Garden サードパーティモデル]
    C --> F[(BigQuery / Cloud Storage)]
    C --> G[MLOps ツールチェーン]
```
...
* **2026-05-26**: **Vertex AI Extensions** の非推奨化が発表（2026年11月26日提供終了予定）。代替としてAgent Platformへの移行が推奨される。
...
| 機能カテゴリ | 機能項目 | 本ツール (Agent Platform) | Amazon Bedrock | Vertex AI Studio |
|:---:|:---|:---:|:---:|:---:|
| **生成AI基盤** | マルチモーダル対応 | ◎<br><small>Gemini 3統合</small> | ◯<br><small>Claude Sonnet 5等連携</small> | ◎<br><small>Gemini・OSS等連携</small> |
| **開発環境** | MLOpsパイプライン | ◎<br><small>統合ツール完備</small> | ◯<br><small>SageMaker連携</small> | ◎<br><small>GCP完全統合</small> |
| **エコシステム** | モデルの多様性 | ◎<br><small>Model Garden</small> | ◎<br><small>多様な提供元</small> | ◯<br><small>OSSも豊富</small> |
| **エンタープライズ** | データ基盤連携 | ◎<br><small>BigQuery連携</small> | ◯<br><small>Redshift等連携</small> | ◎<br><small>BigQuery連携等</small> |
...
| **Amazon Bedrock** | AWSの生成AIマネージドサービス | Claude Sonnet 5など多様なモデルを単一APIで利用可能 | 料金体系が複雑 | AWS環境を主軸とし、特定ベンダーに依存したくない場合 |
| **Vertex AI Studio** | Google CloudのAI統合開発環境 | エンドツーエンドのMLOpsとGeminiモデルの迅速な利用 | 学習コストが高い | プロトタイピングから本番運用まで、Google Cloudのインフラストラクチャ内でシームレスに行いたい場合 |
