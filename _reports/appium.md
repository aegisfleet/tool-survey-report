...
quick_summary:
  ...
  target_users:
    ...
    - 開発者
    - テスト自動化チーム
  latest_highlight: 2026年8月にAppium 3.7.0をリリース
  update_frequency: 高
evaluation:
  score: 85
  base_score: 70
...
* **主要コンポーネントとデータフロー**:
  * **テストスクリプト (Client)**: ユーザーが好みのプログラミング言語で記述したテストコード。Appiumクライアントライブラリを使用して、HTTPまたはWebDriver BiDi経由でAppiumサーバーにコマンドを送信します。
  * **Appium Server**: Node.jsで動作するサーバー。クライアントからのコマンドを受け取り、対象のプラットフォーム（iOS/Android等）に適したネイティブな形式に変換してドライバに送ります。
  * **ドライバ (Driver)**: UIAutomator2 (Android)、XCUITest (iOS) などの各プラットフォーム固有の自動化フレームワークをラップしたもの。サーバーからの指示に従い、実際のデバイスやエミュレータ上でアプリを操作します。
* **特筆すべき要素技術**:
  * **WebDriverプロトコル**: W3Cで標準化された、ブラウザおよびプラットフォームを操作するためのHTTPベースのプロトコル。
  * **Appium 2.0アーキテクチャ**: ドライバとプラグインが独立したパッケージとなり、必要なコンポーネントのみをインストールして環境を最適化できるようになりました。

```mermaid
graph TD
    A[テストスクリプト<br>Java, Python, JS等] -->|HTTP / WebDriver BiDi| B(Appium Server)
    B -->|UIAutomator2 プロトコル| C[Android デバイス/エミュレータ]
    B -->|XCUITest プロトコル| D[iOS デバイス/シミュレータ]
    B -->|その他のプロトコル| E[Windows, Mac 等]
```
...
* **2026-08-24**: **Appium 3.7.0 リリース**
  * グローバル拡張ミドルウェア用の httpServer.frontRouter の追加、およびストレージプラグインでのルートエイリアス追加など。
* **2026-07-25**: **Appium 3.6.0 リリース**
  * 認識されないサーバーCLI引数を許容する `--allow-unknown-args` オプションの追加や、モジュールインポートの重複等のバグ修正。
* **2026-05-31**: **Appium 3.5.0 リリース**
  * ドライバとプラグインが通信するためのセッションごとのIPCチャネル追加。
* **2026-04-09**: **Appium 3.3.0 リリース**
  * モノレポパッケージ内の依存関係で厳密なバージョン指定を採用するように変更。
* **2026-01-26**: **Appium 3.2.0 リリース**
  * verbose extensions listにリポジトリリンクを追加。ドライバやプラグインのインストール全体におけるAppiumのシンボリックリンクの動作を修正。

(出典: [GitHub Releases](https://github.com/appium/appium/releases))
