...
quick_summary:
  ...
  target_users:
    - "AI開発者"
    - "組み込みエンジニア"
  latest_highlight: "Public Preview版として公開、Intel/QualcommのNPUアクセラレーションに対応"
  update_frequency: "高"
...
* **モデルのカスタマイズ**: 事前設定されたモデルの利用に加え、独自のカスタムモデルも使用可能。
* **シームレスな統合**: CLI、SDK、REST APIを通じて既存のアプリケーションやワークフローに統合。
* **ハイブリッド運用**: 必要に応じてMicrosoft Foundry (クラウド) へのスケールアップが可能な設計。

## **4. 特徴・強み (Pros)**

* **コスト効率**: 既存のハードウェアリソースを活用するため、クラウドの従量課金コストを削減できる。
* **プライバシー保護**: データがデバイス外部に送信されないため、セキュリティとコンプライアンス要件を満たしやすい。
* **低遅延**: ネットワークの往復時間が発生しないため、リアルタイムに近い応答性能を実現。
...
* **環境構築の複雑さ**: NPUドライバのインストールやバージョン管理など、ローカル環境のセットアップが必要。
...
* **課金体系**: なし（ローカル実行のため）。
* **無料トライアル**: 該当なし。
* **注意点**: モデルや実行プロバイダーの初回ダウンロード時にはインターネット接続が必要。また、ハイブリッド構成でMicrosoft Foundry（クラウド）を利用する場合は、別途Azureの利用料金が発生する。

## **7. 導入実績・事例**

* **導入企業**: Public Preview段階のため、具体的な企業導入事例はまだ少ない。Microsoftの「Copilot+ PC」イニシアティブに関連する開発者やISV（独立系ソフトウェアベンダー）での利用が想定されている。
* **導入事例**: 公式サイトでの公開事例はなし。
...
* **公式サポート**: Azureの有償サポートプラン契約者は、技術サポートの対象となる場合がある。

## **9. 連携機能 (API・インテグレーション)**

* **API**: REST APIが提供されており、HTTPリクエストを介して推論エンジンを操作できる。
* **外部サービス連携**:
...
* **データ管理**: データは処理から出力まで全てローカルデバイス上で完結し、外部に送信されない。
* **準拠規格**: 公式サイトで公開されていない。問い合わせが必要。

## **11. 操作性 (UI/UX) と学習コスト**

* **UI/UX**: CLIやAPIが中心のため、GUIを持たないCUIツール。操作にはターミナルの知識が必須。
* **学習コスト**: Docker、コマンドライン、AIモデルの基本知識があれば導入は比較的容易。ただし、NPUドライバの不整合など、ハードウェア固有のトラブルシューティングが必要になる場合がある。
...
* **2026-01-06**: Microsoft Learnの公式ドキュメントが更新。Public Preview版として、インストール手順やCLIリファレンスが提供されている。
* **継続的な開発**: 現在もPublic Previewとして活発に開発が進行中。対応モデルの追加や推論エンジンの最適化が継続的に行われている。
...
| **TensorRT-LLM** | NVIDIA GPUに特化した高速推論ライブラリ | NVIDIA環境での最高の推論パフォーマンス | NVIDIA製GPUが必須、セットアップが複雑 | NVIDIA GPU環境で、推論速度を極限まで追求したい場合 |
