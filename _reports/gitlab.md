...
* **GitLab Duo (AI機能群)**: 開発ライフサイクル全体を支援するAI機能。Premium/Ultimateプランには基本的なAI機能が含まれ、より高度な機能は有料アドオンとして提供される。
  * **コード提案 & AIチャット (Premium/Ultimate標準機能)**: IDE内で文脈に応じたコード補完や、実装方法に関する質問への回答を行う。
  * **テスト生成 (Duo Pro)**: マージリクエストの差分に基づき、単体テストや結合テストのコードを自動生成。
  * **脆弱性の説明と修正 (Duo Enterprise)**: スキャンで検出された脆弱性の内容を自然言語で解説し、修正コードを提案。
  * **根本原因分析 (Duo Enterprise)**: CI/CDパイプラインの失敗原因を特定し、解決策を提示。
  * **エピックとイシューの要約 (Duo Enterprise)**: 議論が長くなったイシューやエピックの内容をAIが要約。
...
* **ライフサイクル全体に統合されたAI**: GitLab Duoが、コーディング支援からテスト、セキュリティ、計画まで開発プロセスのあらゆる段階に深く統合されている。基本的なAI機能はPremiumプランから利用でき、必要に応じて高度な機能をアドオンで追加できるため、スモールスタートしやすい。
...
* **Premium**:
  ...
  * **主な機能**: Freeプランの全機能に加え、**基本的なAI機能 (AIチャット、コード提案)**、高度なCI/CD (**月10,000分**)、プロジェクト管理機能、優先サポートなど。
* **Ultimate**:
  * **価格**: 要問い合わせ
...
* **有料アドオン (Premium / Ultimate向け)**:
  * **GitLab Duo Pro**: $19 /ユーザー/月。コード生成・リファクタリング、テスト生成、GitLab UI全体でのAIチャットなど、より高度なAI機能を提供。
  * **GitLab Duo Enterprise**: 要問い合わせ。脆弱性の説明と解決、根本原因分析、コードレビュー支援など、ソフトウェア開発ライフサイクル全体にわたる最上位のAI機能を提供。
...
* **GitLab 18.5 (2025年10月〜11月)**:
  * **GitLab Duo Agentの導入 (ベータ版)**: 計画を担当する`Planner Agent`や、セキュリティ分析を行う`Security Analyst Agent`など、特定のタスクに特化した新しいAIエージェント群が導入され、より高度で自律的なタスク実行支援が可能に。
  * **AIモデルのアップデートと移行**: Anthropic社の`Claude Sonnet 3.7`のサポートを2026年1月に終了し、`Claude 4.0`以上への移行を推奨。これにより、より高性能な基盤モデルを利用可能に。
  * **パイプライン入力のセキュリティ強化**: CI/CDパイプラインにおいて、従来の変数（variables）に代わり、より厳密な型付けと検証が可能な入力（inputs）への移行を推奨し、セキュリティを向上。
  * **Maven仮想レジストリUI (ベータ版)**: 複数のMavenリポジトリを一つにまとめて扱えるUIが追加され、Java開発者の依存関係管理がより容易に。
  * **最新パッチリリース**: **18.5.2** (2025年11月12日) を含む、継続的なセキュリティ修正とバグフィックスを提供。
* **開発体験の向上**:
  * **グループとプロジェクトの削除フロー改善**: 誤操作を防ぐための保留期間(デフォルト7日)や、セルフサービスでの復元機能が導入され、管理性が向上。
