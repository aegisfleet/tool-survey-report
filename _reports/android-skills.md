...
quick_summary:
  ...
  target_users:
    - Android開発者
    - AIエージェント開発者
  latest_highlight: v1.0.11 リリース（2026年9月）
  update_frequency: 中
evaluation:
  score: 80
  base_score: 70
...
* **アーキテクチャ**: ローカル開発環境のファイルシステムに配置され、AIエージェントから参照されるマークダウンベースのナレッジリポジトリ
* **主要コンポーネントとデータフロー**:
  * スキル定義ファイル（`SKILL.md`）を中心に構成される
  * ユーザーがAndroid CLIなどを通じてローカルの`.skills/`ディレクトリ等にスキルをダウンロード・配置する
  * AIエージェント（Gemini, Antigravity等）がタスク実行時にローカルのスキルディレクトリを参照し、コンテキストとして読み込む
* **特筆すべき要素技術**:
  * プロンプトインジェクション/コンテキスト拡張：LLMのプロンプトに動的に専門知識を注入する仕組み
  * Agent Skillsフォーマット準拠

```mermaid
graph TD
    GitHub["Android Skills (GitHub)"] -->|ダウンロード| LocalDir["ローカルディレクトリ (.skills/)"]
    LocalDir -->|コンテキストとして参照| AIAgent["AIエージェント (Gemini, Antigravity等)"]
    AIAgent -->|指示・パターン適用| CodeGeneration["コード生成 / 修正提案"]
```
...
* **2026-09-07**: v1.0.11 リリース
* **2026-08-24**: v1.0.10 および v1.0.9 リリース
* **2026-08-17**: v1.0.8 リリース
* **2026-08-07**: v1.0.7 リリース
* **2026-07-29**: v1.0.6 リリース
* **2026-04-14**: v0.0.2 リリース。最新のAndroidスキルが追加。

(出典: [GitHub Releases](https://github.com/android/skills/releases))

## **17. 類似ツールとの比較**

### **17.1 機能比較表 (星取表)**

| 機能カテゴリ | 機能項目 | Android Skills | Android CLI | Agent Skills | Vercel Agent Skills |
|:---:|:---|:---:|:---:|:---:|:---:|
| **基本機能** | プラットフォーム特化知識 | ◎<br><small>Android公式のベストプラクティス</small> | ◯<br><small>CLI操作と連携</small> | ◯<br><small>汎用フォーマット</small> | ◎<br><small>Vercel/Reactエコシステム</small> |
| **カテゴリ特定** | ツール統合 | ◎<br><small>Android CLI等に統合</small> | ◎<br><small>標準でエージェント用スキルを提供</small> | ◯<br><small>各種対応ツール</small> | ◎<br><small>npxコマンドで即座にインストール可能</small> |
| **拡張機能** | ファイルバンドル | ◎<br><small>スクリプト等も同梱</small> | △<br><small>CLIが主体</small> | ◎<br><small>仕様としてサポート</small> | ◯<br><small>スクリプトや指示書の追加が容易</small> |
...
| **Vercel Agent Skills** | AIエージェントに特化したガイドラインやスクリプトの集合 | 導入が非常に簡単で、Vercel/Reactエコシステムの公式ベストプラクティスが利用可能。 | 複雑な双方向通信や永続的なプロセスには不向き。 | フロントエンド開発でAIエージェントを活用しており、最新のベストプラクティスを遵守させたい場合。 |
