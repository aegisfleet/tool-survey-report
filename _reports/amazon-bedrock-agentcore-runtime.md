...
quick_summary:
  ...
  target_users:
    ...
    - 開発者
    - エンタープライズ
  latest_highlight: 2026年5月にAgentCore Payments（少額決済機能）のプレビュー版を公開。
  update_frequency: 高
evaluation:
  score: 85
  base_score: 70
...
* **AgentCore Payments**: AIエージェントが自律的にAPIやWebコンテンツにマイクロペイメント（少額決済）を行うことが可能なウォレット機能。

## **4. 動作原理・システム構成**

* **アーキテクチャ**: クラウド完結型のサーバーレスエージェント基盤
* **主要コンポーネントとデータフロー**:
  * ユーザーのリクエストやエージェントの推論ループは、隔離されたMicroVM環境内で実行される。データは顧客のAWSアカウント環境内で処理され、モデルの学習には使用されない。
  * 外部API呼び出し等のI/O待ちが発生した際、CPUリソースは解放され、コストの最適化が行われる。
* **特筆すべき要素技術**:
  * **Firecracker MicroVM**: セッションごとに完全分離された軽量VMで高いセキュリティと高速な起動を両立。
  * **x402**: HTTPネイティブな決済プロトコル。HTTP 402（Payment Required）レスポンスを受け取った際、ステーブルコインを用いたマイクロペイメントを即座に実行する。
...
* **クイックスタート**:
  ...
  # AgentCore Runtimeクライアントの作成
  client = boto3.client('bedrock-agent-runtime')

  # エージェントの実行例 (APIがGA時に更新される可能性があります)
...
* **自律的な経済活動**: AgentCore Paymentsにより、エージェントが自律的に有料APIやコンテンツにアクセス可能になり、開発者の課金契約・クレデンシャル管理の負担が軽減される。
...
| **Payments** | 決済額に依存 | エージェントが自律的に実行したマイクロペイメントの額。上限設定によるガバナンスが可能。 |

* **課金体系**: セッション開始から終了までの間、アクティブなCPU処理時間と、確保されたメモリ量に応じて秒単位で課金。I/O待機中はCPU課金が発生しない。エージェントによる決済機能(AgentCore Payments)を利用した場合は、各有料リソースに応じた金額が課金される。
* **無料枠**: 新規AWSアカウント向けにAgentCore用の無料クレジット（$200相当）が提供される場合がある（AWS Free Tierの一部）。

## **9. 導入実績・事例**

* **導入企業**: Ericsson, Thomson Reuters, Cox Automotive, Amazon Devices, 住信 SBI ネット銀行
* **導入事例**:
  ...
  * **住信 SBI ネット銀行**: AgentCoreを中核としたAIエージェント機能を活用し、自然言語による対話を通じた銀行手続きを可能にするAI銀行サービス「NEOBANK ai」を構築。
...
* **外部サービス連携**: AgentCore Gatewayを通じて、Slack, Microsoft 365, Salesforce などのSaaSや、社内のプライベートAPIと安全に連携可能。CoinbaseやStripeと連携し、エージェントによる自律的な決済（ウォレット機能）も可能。
...
* **調査対象**: 公式事例、AWS Summit発表資料（G2、Capterra、ITreviewにレビューの登録なし）
* **総合評価**: レビューサイトの登録がないためスコアなし
* **ポジティブな評価**:
  * 「インフラ管理から解放され、ビジネスロジックの開発に集中できるようになった」（AWS公式事例より要約）
  * 「セキュリティ要件の厳しい金融・医療業界でも安心して使える分離レベルが魅力」（AWS公式事例より要約）
  * 「既存のLangGraphエージェントをそのままクラウドに移行できたのが便利」（AWS公式事例より要約）
* **ネガティブな評価 / 改善要望**:
  * 「新しいサービスのため、まだ一部のリージョンでしか利用できない場合がある」（ユーザーコミュニティのフィードバックより要約）
  * 「デバッグ時にローカル環境との差異に戸惑うことがある」（開発者のブログより要約）
  * 「料金計算が細かく、月額の正確な予算見積もりがやや難しい」（フォーラムより要約）
* **特徴的なユースケース**:
  * 長時間の待機が発生するマルチエージェントシステムのホスティングにおいて、待機中のコストを削減するユースケースで特に役立っている。

## **16. 直近半年のアップデート情報**

* **2026-05-07**: CoinbaseおよびStripeと提携し、AIエージェントに自律的な決済能力（ウォレット機能）を提供するAgentCore Paymentsのプレビュー版を公開。
* **2026-03-17**: AgentCoreによる、高度なネットワーク運用エージェントの構築事例やベストプラクティスが公開。
* **2026-03-09**: AgentCoreポリシーなどがAWS Weekly Roundupで発表。
* **2026-02-27**: 住信 SBI ネット銀行における、AgentCoreを活用したAI銀行サービス「NEOBANK ai」の導入事例が公開。
...
(出典: [AWS Machine Learning Blog](https://aws.amazon.com/blogs/machine-learning/) など)
...
| **拡張機能** | 自律決済 (AgentCore Payments) | ◎<br><small>x402プロトコル対応</small> | × | × | × |
