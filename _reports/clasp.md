...
quick_summary:
  ...
  target_users:
    - 開発者
    - Google Workspace管理者
  latest_highlight: 2026年3月にv3.3.0をリリース。Claude Code CLIやGemini CLIのサポートを追加
  update_frequency: 中
evaluation:
  score: 83
  base_score: 70
...
* **2025-10-15**: v3.1.0リリース。`delete`コマンドの追加や、listコマンドにスクリプトIDを指定するオプションの追加などを実施。
* **2025-05-30**: v3.0.5-alphaリリース。サーバー側で削除されたファイルをローカルからも削除する `-d` オプションの追加や、AIエージェント向けにMCPサーバーとして動作可能な `mcp` コマンドを追加。

(出典: [CHANGELOG](https://github.com/google/clasp/blob/master/CHANGELOG.md) など)
