...
quick_summary:
  ...
  target_users:
    - AIエージェント開発者
    - 自動化エンジニア
  latest_highlight: MCPサーバー連携やタスク録画機能（Browser Use Videos）を追加
  update_frequency: 高
evaluation:
  score: 83
  base_score: 70
...
* **アーキテクチャ**: LLMエージェントとブラウザをCDP（Chrome DevTools Protocol）を用いて直接接続する軽量な仲介アーキテクチャ。
* **主要コンポーネントとデータフロー**:
  * **LLMエージェント (Claude Code等)**: 自然言語のプロンプトを受け取り、必要な操作（クリック、タイピング、スクロール等）を推論する。
  * **Browser Harness (Pythonスクリプト)**: 中間の抽象化レイヤーを持たず、エージェントからの命令をCDPコマンドに変換してブラウザに送信する。
  * **ブラウザ (Local / Remote)**: CDPを介して直接操作され、画面の状態やDOM情報をHarness経由でエージェントに返す。
* **特筆すべき要素技術**:
  * **Chrome DevTools Protocol (CDP)**: ブラウザのネイティブAPIを直接叩くことで、正確な状態把握と操作を実現。
  * **自己修復（Self-Healing）機能**: エージェントが操作に必要な関数が不足していると判断した場合、`helpers.py`を自ら書き換えて新しい機能を追加する。

```mermaid
graph TD
    A["LLMエージェント<br>(Claude Codeなど)"] -->|操作命令/推論| B("Browser Harness")
    B -->|CDPコマンド| C{"ブラウザ<br>(Local / Cloud)"}
    C -->|DOM/画面状態| B
    B -->|コンテキスト情報| A
    B -.->|必要に応じて自己改修| D["helpers.py<br>(自己修復機能)"]
    A -.->|新スキルの登録| E["domain-skills/<br>(スキルファイル)"]
```
...
* **2026-09-04**: (v0.1.13) MCP extraのピン留めや、macOS/Chrome権限まわりの信頼性向上アップデート。
* **2026-09-03**: (v0.1.11) MCPサーバーのラッパー機能が追加され、エージェントからの利用が容易になった。
* **2026-07-26**: (v0.1.8) 事前にChromeを開かなくても、自動的に起動してリモートデバッグを有効化する機能が追加。
* **2026-07-17**: (v0.1.6) **Browser Use Videos**の追加。実際のブラウザフレームやタイピング等の操作を動画化し、検証可能な結果として出力する機能。
* **2026-04-17**: Hacker Newsにてプロジェクトの公開およびコンセプト（自己修復型CDPハーネス）の発表。

(出典: [GitHub Releases](https://github.com/browser-use/browser-harness/releases))

## **17. 類似ツールとの比較**

### **17.1 機能比較表 (星取表)**

| 機能カテゴリ | 機能項目 | Browser Harness | Browserbase | Playwright | Selenium |
|:---:|:---|:---:|:---:|:---:|:---:|
| **基本機能** | ブラウザ制御 | ◎<br><small>CDP直接</small> | ◯<br><small>API・SDK経由</small> | ◯<br><small>フレームワーク経由</small> | ◯<br><small>WebDriver経由</small> |
| **AI親和性** | 自己修復 | ◎<br><small>AIがコード改修</small> | ◯<br><small>AI用抽象化やSession管理</small> | △<br><small>別途構築が必要</small> | ×<br><small>なし</small> |
| **アーキテクチャ** | 抽象化レイヤー | ◎<br><small>極めて薄い</small> | △<br><small>フルマネージド（厚い）</small> | △<br><small>厚い</small> | △<br><small>厚い</small> |
| **リモート** | クラウドブラウザ | ◯<br><small>標準連携</small> | ◎<br><small>クラウド特化</small> | △<br><small>サードパーティ要</small> | △<br><small>サードパーティ要</small> |

### **17.2 詳細比較**

| ツール名 | 特徴 | 強み | 弱み | 選択肢となるケース |
|---------|------|------|------|------------------|
| **Browser Harness** | AIが直接CDPを叩く自己修復型ハーネス | 抽象化による文脈喪失がない。未知のタスクに適応可能。動画録画（Videos）機能による監査が容易。 | 非常に優秀なLLMが必要。制御が難しくセキュリティリスクへの考慮が不可欠。 | AIエージェントに自律的にブラウザ上の未知のタスクを解決させたい場合。 |
| **Browserbase** | 開発者向けのフルマネージドサーバーレスブラウザ | APIを叩くだけで複雑なクラウドブラウザ環境（プロキシ、CAPTCHA等）を利用可能。 | ツール自体の費用がかかる。独自のバックエンドに依存。 | スケーラブルなAIエージェントのブラウザ操作環境を手軽に構築したい場合。 |
| **Playwright** | モダンなWebテスト・自動化フレームワーク | 堅牢で高速。充実したエコシステムや公式の強力な機能（Codegen等）が豊富。 | スクリプトの保守コストが高い。UI変更に弱くAI向けの適応にはMCP等の連携が必要。 | プログラムによる決定論的なテストやスクレイピングを行う場合。 |
