...
quick_summary:
  ...
  target_users:
    - 開発者
    - インフラエンジニア
  latest_highlight: '2026年6月: ネイティブなLinuxコンテナをサポートするWSL Containers (WSLC) のパブリックプレビューが開始'
  update_frequency: 高
evaluation:
  score: 88
  base_score: 70
...
* **WSL Containers (WSLC)**: Docker Desktopに依存せず、ネイティブにLinuxコンテナを構築、実行、管理できる機能（パブリックプレビュー）。

## **4. 動作原理・システム構成**

* **アーキテクチャ**: Windowsと統合された軽量ユーティリティVM(仮想マシン)ベース
* **主要コンポーネントとデータフロー**:
  * **Windows OS (Host)**: WSLを起動・管理するホスト環境。
  * **Hyper-V アーキテクチャ**: WSL 2は、MicrosoftのHyper-Vハイパーバイザー技術を利用して、軽量なユーティリティVM(Utility VM)を起動する。
  * **Utility VM**: このVM内で、Microsoftが提供するカスタムの完全なLinuxカーネルが実行される。
  * **Plan 9 (9P) プロトコル**: Windowsのファイルシステム（NTFSなど）とLinuxのファイルシステム（ext4など）間のファイルアクセスは、9Pプロトコルサーバーを介してシームレスに行われる。
  * **WSLg**: Waylandサーバー（Weston）、XWayland、PulseAudioなどを含み、LinuxのGUIアプリや音声出力をWindowsデスクトップと統合する。
  * **WSL Containers (WSLC)**: 新しいコンテナランタイム機能。Docker Desktopに依存せず、WSL内でコンテナを直接管理・実行する。

```mermaid
flowchart TD
    subgraph Windows["Windows OS"]
        WindowsApps["Windowsアプリケーション"]
        VSCode["VS Code"]
        DockerDesktop["Docker Desktop"]
    end

    subgraph HyperV["Hyper-V Hypervisor"]
        subgraph UtilityVM["WSL 2 Utility VM"]
            LinuxKernel["Linuxカーネル"]
            subgraph WSLDistro["WSL ディストリビューション (Ubuntu等)"]
                Bash["Bash / コマンド"]
                LinuxApps["Linux アプリケーション"]
                WSLC["WSLC コンテナ"]
            end
            WSLg["WSLg (GUI/Audio)"]
        end
    ...
    WindowsApps <-->|9P Protocol| WSLDistro
    VSCode <-->|Remote - WSL| WSLDistro
    DockerDesktop <-->|統合| UtilityVM
    WSLg -->|RDP/GUI統合| Windows
```

* **特筆すべき要素技術**:
  * **軽量ユーティリティVM (Utility VM)**: 従来の重い仮想マシンとは異なり、1秒未満で起動し、動的にメモリを割り当て/解放するよう最適化されている。
  * **完全なLinuxカーネル**: WSL 1のシステムコール変換とは異なり、WSL 2は本物のLinuxカーネルを使用するため、完全な互換性を持つ。
  * **仮想ファイルシステム**: VHD（仮想ハードディスク）にLinux側のファイルを格納し、高パフォーマンスを実現。
...
* **2026-08-25**: バージョン 2.9.9 リリース（プレリリース）。バグ修正およびハイブリッドCRTリンクの追加など。
* **2026-08-24**: バージョン 2.9.8 リリース（プレリリース）。WSLc APIドキュメントの追加、ネットワークオプションの拡充、WSLgログ収集機能の強化など多数の機能追加と修正。
* **2026-08-18**: バージョン 2.7.12 リリース。ディストリビューション移動時のVHD所有権リストアなどのバグ修正をバックポート。
* **2026-07-24**: バージョン 2.7.11 リリース。セキュリティ修正のバックポート。
* **2026-07-13**: バージョン 2.9.4 リリース（プレリリース）。WSLcおよびネットワーク関連の機能追加、各種バグ修正。
* **2026-06-29**: バージョン 2.9.3 リリース（プレリリース）。**WSL Containers (WSLC)** がパブリックプレビューに。Docker DesktopなしでネイティブなLinuxコンテナを管理・実行できる機能を提供。
...
| 機能カテゴリ | 機能項目 | 本ツール(WSL) | dockur/windows | Wine |
|:---:|:---|:---:|:---:|:---:|
| **基本機能** | 異種OS実行 | ◎<br><small>Windows上でLinuxをシームレス実行</small> | ◯<br><small>Docker(Linux等)内でWindowsを実行</small> | ◯<br><small>Linux/Mac上でWindowsアプリを実行</small> |
| **利便性** | 起動時間 | ◎<br><small>1秒未満で起動</small> | ◯<br><small>コンテナ起動で環境用意</small> | ◎<br><small>即座に実行</small> |
| **統合性** | ホストOSとの統合 | ◎<br><small>ファイルシステムやコマンドの相互呼び出しが可能</small> | △<br><small>RDPやVNC経由のアクセスが基本</small> | ◯<br><small>シームレスな統合（ファイルやクリップボード等）</small> |
| **パフォーマンス** | ネイティブ性能 | ◯<br><small>軽量VM(WSL2)による高パフォーマンス</small> | ◯<br><small>KVM利用でネイティブに近い性能(環境依存)</small> | ◎<br><small>API変換のみでエミュレートしないため高速</small> |

### **17.2 詳細比較**

| ツール名 | 特徴 | 強み | 弱み | 選択肢となるケース |
|---------|------|------|------|------------------|
| **本ツール (WSL)** | Windows上でLinux環境を統合的に実行。 | 起動が爆速、Windowsアプリと同時に使える、ホストとのファイル共有が容易。 | グラフィックや特殊なハードウェア制御はネイティブ環境に劣る場合がある。 | WindowsをメインOSとしつつ、Linux向けの開発やツールを利用したい大半の開発シナリオ。 |
| **dockur/windows** | Dockerコンテナ内でWindowsを実行。 | インストール自動化、使い捨て環境の構築が容易。 | KVM環境がないと非常に遅い、ホストとの密接な連携は苦手。 | Linux環境等で一時的なWindows環境やテスト環境が欲しい場合、CI/CDでWindowsが必要な場合。 |
| **Wine** | POSIX準拠OS上でWindowsアプリを直接実行。 | 無料、OSのインストール不要、軽量で高速。 | アプリによっては動かない、設定や調整に専門知識が必要。 | LinuxやmacOSをメインOSとし、仮想マシンを立てずに特定のWindowsアプリのみを使用したい場合。 |
