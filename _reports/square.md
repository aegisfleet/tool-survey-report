...
* **アーキテクチャ**: クラウド完結型SaaSを中心とし、多様なクライアント（自社製専用ハードウェア、モバイルアプリ、Web）からAPIを通じてデータ同期を行う統合プラットフォーム。
* **主要コンポーネントとデータフロー**:
  * 店舗やモバイル端末の「Square POSレジ（クライアント）」で入力された売上・在庫・顧客データは、クラウド上のSquareバックエンドサーバーへリアルタイム（またはオフラインからの復旧時に一括）で送信される。
  * 「Square オンラインビジネス（eコマース）」のトランザクションも同様にバックエンドサーバーと同期し、実店舗とオンラインのデータを一元管理する。
  * 決済データは、PCI DSSに準拠したセキュアな決済処理システム（Payment Processing）へ送られ、銀行ネットワーク（アクワイアラ）を通じて処理される。
* **特筆すべき要素技術**:
  * **APIとSDKの提供**: Terminal API、Mobile Payments SDK、Web Payments SDKなど豊富なエンドポイントを備え、サードパーティのWebアプリやモバイルアプリから決済・カタログ・顧客情報などを操作可能。
  * **エンドツーエンド暗号化**: ハードウェアリーダーによるカード読み取り時点から決済サーバー到達まで情報を暗号化し、デバイスに決済データを残さない。
  * **オフライン決済**: ネットワーク切断時でも一時的にデバイスに暗号化データを保持し、接続回復後にバックグラウンドで決済を処理する仕組み。

```mermaid
flowchart TD
    subgraph クライアント
        A[店舗用 POSアプリ / 専用ハードウェア]
        B[Square オンラインビジネス]
        C[サードパーティ製アプリ / カスタムWeb]
    end

    subgraph Square バックエンドクラウド
        D[API ゲートウェイ]
        E[POS・在庫・顧客・スタッフ 管理システム]
        F[決済処理システム]
    end
