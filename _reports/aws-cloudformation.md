...
quick_summary:
  ...
  target_users:
    ...
    - インフラ管理者
    - 開発者
  latest_highlight: 2026年9月にCloudFormation CLIおよびcfn-lintのアップデートがリリース
  update_frequency: 高
evaluation:
  score: 81
  base_score: 70
...
* **アーキテクチャ**: クラウド完結型のフルマネージドサービス
* **主要コンポーネントとデータフロー**:
  * ユーザーはJSONまたはYAML形式のテンプレートを作成し、AWS CLIやマネジメントコンソールを通じてCloudFormationに送信します。
  * CloudFormationはテンプレートを解析し、AWSリソース間の依存関係を特定して、リソースの作成、更新、削除を正しい順序で実行します。
  * AWSの各サービスのAPIを呼び出して実際のインフラをプロビジョニングし、スタックの状態（ステート）をAWS側で管理します。
* **特筆すべき要素技術**:
  * 宣言型プログラミングモデル、APIの冪等性、リソース間の依存関係解決のためのグラフ理論など。

```mermaid
graph TD
    A[ユーザー / CI/CD] -->|1. テンプレート送信| B(CloudFormation)
    B -->|2. 依存関係の解析| B
    B -->|3. APIコール| C{AWS サービス}
    C -->|EC2作成| D[Amazon EC2]
    C -->|S3バケット作成| E[Amazon S3]
    C -->|RDS構築| F[Amazon RDS]
    B -.->|4. 状態管理| G[(CloudFormation State)]
```
...
* **2026-09-10**: **cfn-lint v1.56.3 リリース**
  * AWS CloudFormation Linter (cfn-lint) がアップデートされ、新ルールの追加やバグ修正が行われました。特にSAMのGlobalsプロパティでの組み込み関数のサポートやECS関連の機能強化（Fargateの32 vCPUサポートなど）が含まれています。
* **2026-09-03**: **CloudFormation CLI v0.2.41 リリース**
  * `jsonschema` のアップデートやリリース修正を含む `aws-cloudformation/cloudformation-cli` の新バージョンがリリースされました。サードパーティリソースやフック開発における堅牢性が向上しています。
* **2026-08-31**: **カバレッジの継続的強化とDrift Detectionの追加**
  * `AWS::Glue::Job` などの多数のAWSリソースタイプでプロパティ拡張が行われ、`AWS::EC2::VPCEndpoint` や `AWS::SSM::MaintenanceWindow` などで新しくドリフト検出 (Drift Detection) がサポートされるなど、IaCカバレッジが順次拡大しています。
...
(出典: [What's New with AWS?](https://aws.amazon.com/new/), [CloudFormation CLI Releases](https://github.com/aws-cloudformation/cloudformation-cli/releases), [cfn-lint Releases](https://github.com/aws-cloudformation/cfn-lint/releases))

## **17. 類似ツールとの比較**

### **17.1 機能比較表 (星取表)**

| 機能カテゴリ | 機能項目 | 本ツール | Terraform | Pulumi | OpenTofu |
|:---:|:---|:---:|:---:|:---:|:---:|
| **基本機能** | マルチクラウド対応 | ×<br><small>AWSに特化</small> | ◎<br><small>プロバイダーが非常に豊富</small> | ◎<br><small>170+のプロバイダー対応</small> | ◎<br><small>数千のプロバイダー対応</small> |
| **カテゴリ特定** | プログラミング言語での記述 | ×<br><small>YAML/JSONのみ</small> | △<br><small>CDKTFを使用すれば可能</small> | ◎<br><small>ネイティブに複数言語対応</small> | △<br><small>HCLのみ (言語非対応)</small> |
| **運用・管理** | 状態(ステート)管理 | ◯<br><small>AWS側でフルマネージド管理</small> | △<br><small>ステートファイルの管理が自己責任 (HCP等)</small> | ◯<br><small>Pulumi Cloud推奨/管理可能</small> | ◎<br><small>クライアントサイド暗号化対応</small> |
| **非機能要件** | AWS新機能への追従速度 | ◎<br><small>AWSネイティブのため最速</small> | ◯<br><small>公式プロバイダーの更新頻度が高い</small> | ◯<br><small>対応待ちになる場合あり</small> | ◯<br><small>Terraformと同等の追従性</small> |
...
| **Terraform** | HCL言語を使用するマルチクラウド対応のIaCツール。 | 圧倒的な実績、ドキュメント、エコシステム。IBM/HashiCorpのサポート。 | BUSLライセンスによる制限。RUM課金によるコスト増の懸念。 | マルチクラウド戦略を採用している、エンタープライズでの標準化やサポートを重視する場合。 |
| **Pulumi** | 汎用プログラミング言語 (TypeScript, Python等) を使用するIaCツール。 | 高い表現力、テスト容易性、AIアシスタント(Copilot)による生産性向上。 | HCLに比べるとコミュニティは小さい。IaC独自概念の学習は必要。 | 開発者が使い慣れた言語でインフラを管理し、複雑なロジックやテストを重視する場合。 |
| **OpenTofu** | Terraformのオープンソースフォーク。コミュニティ主導。 | MPL-2.0ライセンス(完全OSS)、Terraformと高い互換性、暗号化等の独自進化。 | エコシステムの成熟度やサポート体制が発展途上。 | ライセンス料やベンダーロックインを回避したい場合、OSSを重視する組織。 |
