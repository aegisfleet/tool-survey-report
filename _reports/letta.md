...
* **大規模コンテキスト対応**: デフォルトで128kのコンテキストウィンドウをサポートし、安定したCompaction（圧縮・要約）メカニズムにより大規模な会話や情報を扱える。
...
* **アーキテクチャ**: ローカルファーストまたはクラウドで動作するクライアント・サーバー構成（マネージドなLetta Cloudも提供）。
* **主要コンポーネントとデータフロー**:
  * LLMプロバイダーへのリクエストをラップし、MemGPTの仕組みに基づいた「Core Memory」と「Archival Memory」間で文脈を管理・抽出・更新する。
  * エージェントはツール（Python関数のサンドボックス実行など）を介して状態の更新を行う。
* **特筆すべき要素技術**:
  * **OSアプローチ**: ユーザーメッセージを割り込み（Interrupt）として扱い、エージェント（プロセス）がメモリのページング（Archival Memoryからの検索とCore Memoryへの読み込み）を行う。

```mermaid
graph TD
    A[ユーザー/クライアント] -->|メッセージ/イベント| B[Letta Server]
    B --> C{Agent OS}
    C --> D[Core Memory<br>常にLLMコンテキストに存在]
    C -->|検索/保存| E[(Archival Memory<br>ベクトルDB等)]
    C -->|推論リクエスト| F[LLM Provider<br>OpenAI/Anthropic等]
    C -->|ツール実行| G[Sandbox Environment]
```
...
* **ポジティブな評価**:
  * ついに長期記憶を持つエージェントが簡単に作れるようになった。
    * （Redditより要約）
  * ADEの可視化機能が素晴らしい。エージェントの思考やメモリ状態のデバッグが容易。
    * （GitHub Discussionsより要約）
  * モデルに依存しないため、多様なLLMを切り替えて使える点が柔軟で良い。
    * （AI関連フォーラムより要約）
* **ネガティブな評価 / 改善要望**:
  * セットアップが少し複雑（特にDocker周りの設定）。
    * （Redditより要約）
  * 簡単なチャットボットを作るにはオーバースペックで学習コストが高い。
    * （GitHub Discussionsより要約）
  * Python版と比べて他の言語対応の充実が期待される。
    * （AI関連フォーラムより要約）
* **特徴的なユースケース**:
...
* **2026-05-14 (v0.16.8)**: セキュリティ修正（Sandbox・Server間のツール結果転送でPickleからJSONを使用）およびワークフローの更新。
* **2026-03-31 (v0.16.7)**: デフォルトのグローバルコンテキスト上限を32kから128kへ拡大し、Compaction（圧縮）ロジックのバグやオーバーフローを大幅に修正。ブロックサイズの制限撤廃。
* **2026-03-04 (v0.16.6)**: デフォルトカンバセーション（Agent-direct mode）のAPIサポート拡張。新規カンバセーション開始時にシステムメッセージを即座にコンパイル・永続化するよう変更。
* **2026-02-24 (v0.16.5)**: パフォーマンス改善とメモリ管理ロジックの調整。
* **2026-01-29 (v0.16.4)**: 新しいモデル対応の強化とツール連携機能の最適化。
...
| 機能カテゴリ | 機能項目 | Letta | AutoGPT | LangChain |
|:---:|:---|:---:|:---:|:---:|
| **基本機能** | 永続的メモリ管理 | ◎<br><small>OSレベルで統合</small> | ◯<br><small>ベクターDB利用</small> | △<br><small>実装が必要</small> |
| **カテゴリ特定** | 自律タスク実行 | ◎<br><small>高度な自律動作</small> | ◎<br><small>完全自律型</small> | ◎<br><small>LangGraphが強力</small> |
| **エンタープライズ** | 開発環境/GUIデバッガ | ◎<br><small>ADEが強力</small> | ◎<br><small>ノードベースのGUI</small> | ◯<br><small>LangSmith</small> |
| **非機能要件** | モデル非依存 | ◎<br><small>柔軟なプロバイダー対応</small> | ◎<br><small>多モデル対応</small> | ◎<br><small>60以上のLLM連携</small> |
