...
quick_summary:
  ...
  target_users:
    - 開発者
  latest_highlight: 2026年7月リリースのGrok 4.5を統合し推論能力がさらに向上
  update_frequency: 高
evaluation:
  score: 80
  base_score: 70
...
* **Grokモデル搭載**: 推論能力に優れた最新のGrokモデル（Grok 4.5 Reasoningなど）を採用。
* **サンドボックスランタイム**: ローカルで安全にコードを実行、テスト、検証可能。

## **4. 動作原理・システム構成**

* **アーキテクチャ**: ローカルクライアントとクラウド（xAIのインフラ）が連携するハイブリッド型アーキテクチャ。
* **主要コンポーネントとデータフロー**:
  * ローカルのCLIツールがファイルシステムやGitコンテキストを読み取り、GrokのAPIと通信する。
  * Web Workspaceはクラウド上で動作し、GitHub等の外部リポジトリと直接連携してコンテキストを構築する。
  * 推論エンジンにはGrok 4.5 Reasoningモデルが使用され、コード生成やバグ修正の提案を行う。
* **特筆すべき要素技術**:
  * **サンドボックスランタイム**: 生成されたコードを安全な分離環境でテスト実行し、動作検証を行う。
  * **Context Compaction**: 大規模なリポジトリのコンテキストを効率的に圧縮し、Grokモデルに送信する最適化技術（xAI独自の技術を活用）。

```mermaid
graph TD
    User[開発者] -->|コマンド実行| CLI[ローカルCLI]
    User -->|ブラウザ操作| WebUI[Web Workspace]
    CLI -->|リポジトリ情報| GrokAPI[Grok API]
    WebUI -->|リポジトリ情報| GrokAPI
    GrokAPI --> GrokEngine[Grok 4.5 Reasoning Engine]
    GrokEngine --> Sandbox[サンドボックスランタイム]
    Sandbox -->|検証結果| GrokEngine
    GrokEngine -->|コード提案・修正| CLI
    GrokEngine -->|コード提案・修正| WebUI
    CLI --> LocalRepo[ローカルリポジトリ]
    WebUI --> GitHub[GitHub連携]
```
...
* **2026-07-08**: Grok 4.5モデルのリリースに伴い、推論エンジンをGrok 4.5にアップグレード。コーディングおよびエージェントタスクの性能が大幅に向上。
* **2026-05-14**: Grok Buildのベータ版を提供開始。エージェント型コーディングワークフロー向けTUIを提供。
* **2026-04-08**: Grok 4.20 Reasoningモデルを搭載し、Live betaとして提供開始。

(出典: [Grok Release Notes](https://docs.x.ai/docs/release-notes))
...
|:---:|:---|:---:|:---:|:---:|
| **基本機能** | 自律的コーディング | ◎<br><small>Grok 4.5を活用</small> | ◎<br><small>完全自律型</small> | ◯<br><small>アシスタント型</small> |
| **環境** | ローカル連携 | ◎<br><small>CLIから直接操作</small> | △<br><small>クラウド完結</small> | ◎<br><small>IDE統合</small> |
| **機能拡張** | Web Workspace | ◯<br><small>専用UI提供</small> | ◎<br><small>MCP連携・独自UI</small> | ×<br><small>IDE依存</small> |
