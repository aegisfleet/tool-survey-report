...
quick_summary:
  ...
  target_users:
    ...
    - IT運用チーム
    - 開発者
  latest_highlight: エージェントの継続的アップデート（v2.1.46リリース等）およびAIインシデント調査の強化
  update_frequency: 高
evaluation:
  score: 85
  base_score: 70
...
* **アーキテクチャ**: エージェント主導型のクラウド/オンプレミスハイブリッド構成（SaaS版またはセルフホスト版）
* **主要コンポーネントとデータフロー**:
  * 各監視対象ホスト（VM、Kubernetesノード等）にデプロイされたInstana Agentが、稼働中のコンテナやプロセスを自動検出し、必要なセンサーを動的にアタッチしてデータを収集します。
  * 収集されたメトリクスとトレースデータは、バックエンドのInstanaプラットフォームに送信され、AIエンジン（vStack）が依存関係をリアルタイムで解析します。
* **特筆すべき要素技術**:
  * **Dynamic Graph**: 収集データを元に、すべてのインフラとアプリケーションの相互関係をリアルタイムでマッピングする基盤技術。
  * **Auto-Instrumentation**: Java等のバイトコード・インスツルメンテーションを用いて、コード変更なしでトレースを実現。

```mermaid
graph TD
    subgraph 監視対象環境
        A[Host/Node] -->|自動検出| B(Instana Agent)
        B -.->|動的アタッチ| C[Java/Node.js/DB等]
        B -.->|動的アタッチ| D[Container/Pod]
    end
    B ==>|メトリクス・トレース送信| E((Instana Backend))
    subgraph Instanaプラットフォーム
        E --> F[Dynamic Graph]
        F --> G[AI/ML エンジン]
        G --> H[インシデント生成/UI表示]
    end
...
* **2026-09-10**: Agentアップデート (v2.1.46)。Goツールチェーンの更新と依存関係のセキュリティアップデートを実施。
...
(出典: [IBM Instana Release notes](https://www.ibm.com/docs/en/instana-observability?topic=release-notes) / [GitHub Releases](https://github.com/instana/instana-agent-operator/releases))

## **17. 類似ツールとの比較**

### **17.1 機能比較表 (星取表)**

| 機能カテゴリ | 機能項目 | IBM Instana | Datadog | New Relic |
|:---:|:---|:---:|:---:|:---:|
| **基本機能** | メトリクス収集の粒度 | ◎<br><small>1秒単位の超高解像度</small> | ◯<br><small>標準は15秒〜</small> | ◯<br><small>標準は高め</small> |
| **可視化** | 分散トレース | ◎<br><small>サンプリングなし全件取得</small> | ◯<br><small>サンプリングベースでの取得が主体</small> | ◎<br><small>OpenTelemetryに強い</small> |
| **運用性** | 自動検出・設定 | ◎<br><small>シングルエージェントで完全自動</small> | ◯<br><small>広範な統合だが設定の手間はあり</small> | ◯<br><small>柔軟だが手動設定が必要な場面も</small> |
| **エンタープライズ**| 課金体系のシンプルさ | ◎<br><small>ホスト(MVS)単位のみでわかりやすい</small> | △<br><small>機能ごとの従量課金で予測が難しい</small> | ◯<br><small>ユーザー数＋データ量の従量課金（100GB無料）</small> |
| **非機能要件** | セルフホスト対応 | ◎<br><small>SaaSと完全同等のセルフホスト版</small> | ×<br><small>SaaSのみ</small> | ×<br><small>SaaS中心</small> |
...
| **New Relic** | 開発者向けに強いフルスタックAPM | 無料枠(100GB/月)の寛大さ、OpenTelemetryへの強力な対応 | 大規模なデータ量ではコスト管理が必須 | 開発者主体でコストを抑えつつ強力なAPMを利用したい場合 |
