...
* **Grouped Security Updates**: 複数の脆弱性修正やバージョン更新を1つのプルリクエストにまとめて、ノイズを減らす機能。複数ディレクトリにまたがる同名依存関係のグループ化にも対応。
...
|:---|:---:|:---|:---|
| **JavaScript (Node.js/Deno/Bun)** | ◎ | npm/yarn/pnpmに加え、DenoやJSRエコシステムにも対応。更新頻度が高いため効果大。 | `node_modules`の更新頻度が高すぎてノイズになりがち。 |
| **Python** | ◎ | pip/poetry/pipenv、さらにuv workspacesによる依存関係グラフもサポート。 | 依存関係の解決に時間がかかる場合がある。 |
| **Java / Scala** | ◯ | Maven/Gradleに加え、SBTエコシステムにも対応。 | ビルド設定が複雑な場合、Dependabotが解析できないことがある。 |
| **Docker** | ◯ | Dockerfile内のベースイメージ更新に対応。 | マルチステージビルドなどで一部制限がある場合も。 |
| **IaC (Terraform / OpenTofu)** | ◎ | Terraformモジュールやプロバイダーの他、OpenTofuエコシステムにも対応。 | モジュールの互換性による破壊的変更に注意。 |

## **11. セキュリティとコンプライアンス**

* **認証**: GitHubの認証基盤を利用。プライベートレジストリ向けにAWS CodeArtifact、Azure DevOps、JFrog、Google Artifact Registry、Cloudsmith等のOIDC認証をサポート。
* **データ管理**: 解析はGitHubのインフラ上で行われ、ソースコードが外部に送信されることはない（SaaS版の場合）。内部リポジトリへのクロスオーガニゼーションアクセスもサポート。
...
* **効果的な活用法 (Modern Practices)**:
  * **Grouped Updatesの活用**: 関連するライブラリやエコシステムごとにPRをグループ化し、マージの手間を減らす。ディレクトリをまたいだ同名依存関係のグループ化も有効。
...
* **2026-05-19**: Python 3.9の非推奨化およびプライベートレジストリ向けOIDC認証サポートの拡大（Cloudsmith、Google Artifact Registry）。
* **2026-04-23**: Python（pip、uv、Poetry）向けのDependabotベースの依存関係グラフサポートを追加。
* **2026-02-24**: 複数ディレクトリにまたがる同名依存関係のグループ化機能を追加。
* **2026-02-03**: Dependabot Proxyのオープンソース化（MITライセンス）およびOIDC認証の初期サポート。
