...
tags:
  ...
  - "VS Code"
  - "コーディング支援"
description: "VS Code内で動作するオープンソースのAIコーディングエージェント。クラウド上の自律型エージェントチームと連携し、複雑なタスクを自動化します。"
...
evaluation:
  ...
  plus_points:
    - point: 5
      reason: "完全なモデル非依存（BYOK）で最新モデルを即座に利用可能"
    - point: 5
      reason: "クラウドエージェント（Planner, Coder等）による高度な自律タスク実行"
    - point: 3
      reason: "VS Code拡張として既存環境に導入しやすい"
    - point: 2
      reason: "Slack連携などチーム開発向け機能が充実"
  minus_points:
    - point: 0
      reason: "基本機能は無料だが、クラウドエージェントは従量課金が必要"
...
* **データ管理**: VS Code拡張版ではコードはローカルに留まり、指定したAPIプロバイダーにのみ送信されます。`.rooignore` で送信除外設定が可能。
