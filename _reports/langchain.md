...
* **Agents**: LLMを推論エンジンとして使用し、与えられたツール（計算機、検索、API）を自律的に選択・実行してタスクを解決する機能。v1.3.0より高度なイベントストリーミング（`v3`）にも対応。
...
* **総合評価**: 4.5/5.0 (G2)
* **ポジティブな評価**:
  * 「LangGraphのおかげで、以前はスパゲッティコードになっていた複雑なエージェントロジックが綺麗に整理できた。」（G2より引用）
  * 「LangSmithのデバッグ機能なしでは、もうLLMアプリ開発には戻れない。コスト分析も便利。」（G2より引用）
  * 「エコシステムが広大で、新しいモデルが出ても即座に対応できる。」（G2より引用）
* **ネガティブな評価 / 改善要望**:
  * 「ドキュメントの構成が頻繁に変わり、探している情報に辿り着けないことがある。」（G2より引用）
  * 「JS版とPython版で微妙に挙動や引数が異なり、移植時にハマる。」（G2より引用）
  * 「バージョンアップで動かなくなることが（v1.0で減ったとはいえ）まだある。」（G2より引用）
* **特徴的なユースケース**:
...
* **2026-05-15**: **LangChain 1.3.1**: summarization token checkの修正など、バグ修正と安定性向上リリース。
...
(出典: [GitHub Releases](https://github.com/langchain-ai/langchain/releases) / [LangChain Changelog](https://changelog.langchain.com/) / [LangChain Blog](https://blog.langchain.dev/))
