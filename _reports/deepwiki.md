...
quick_summary:
  ...
  target_users:
    - 開発者
    - OSSコントリビューター
  latest_highlight: 2026年4月に「Wiki v2」がリリースされ、ドキュメントの品質向上とACUコストの可視化が実現
  update_frequency: 中
evaluation:
  score: 80
  base_score: 70
...
* **Wiki v2と高度な推論**: 強力な推論能力とサブエージェント（Agentic page writers）を活用し、より高品質で詳細なドキュメントを生成します。
...
* **労力（Effort Level）の調整**: ドキュメント生成時の深さとコスト（ACU消費量）のトレードオフをユーザーが選択・調整可能です。
...
* プライベートリポジトリの解析にはDevinのアカウントや権限設定が必要であり、生成に伴いACU（Agent Compute Unit）を消費する。
...
| **Devin Users** | Devinの料金に含まれる | 任意のリポジトリのインデックスや高度な連携機能が提供される（ACUを消費する） |

* **課金体系**: 閲覧は無料。Wikiの生成や更新にはDevinのACU（Agent Compute Unit）を消費する（ユーザーがEffort Levelを選択することでコストをコントロール可能）。
* **無料トライアル**: 公開情報の閲覧は無料
...
* **2026-07-24**: DeepWiki MCPを利用したQ&A機能の応答速度が向上。
* **2026-04-10**: Wiki生成時の労力（Effort Level）に応じた予想ACU消費量の表示機能が追加され、コスト内訳が確認可能に。
* **2026-04-08**: より強力な推論とサブエージェントを活用した「Wiki v2」がリリースされ、ドキュメントの品質が向上。
...
| 機能カテゴリ | 機能項目 | 本ツール | Code Wiki | DeepWiki-Open |
|:---:|:---|:---:|:---:|:---:|
| **基本機能** | ドキュメント生成 | ◎<br><small>詳細なWiki自動生成</small> | ◎<br><small>変更追従型</small> | ◯<br><small>Wiki生成可能</small> |
| **基本機能** | 対話機能 | ◎<br><small>Devin・Gemini連携</small> | ◎<br><small>Gemini搭載</small> | ◯<br><small>チャット可能</small> |
| **提供形態** | 提供方式 | SaaS<br><small>クラウド</small> | SaaS<br><small>クラウド・CLI連携</small> | OSS<br><small>セルフホスト</small> |
| **連携** | エージェント連携 | ◎<br><small>Devinと完全統合</small> | △<br><small>CLIベース(Waitlist)</small> | △<br><small>対応弱</small> |

### **17.2 詳細比較**

| ツール名 | 特徴 | 強み | 弱み | 選択肢となるケース |
|---------|------|------|------|------------------|
| **本ツール** | Devin統合型のGitHubドキュメント生成ツール | 生成されるWikiの品質が高く、ソースコードへのリンクが充実。Devinとの連携が強力。 | プライベート利用にはDevinとACUが必要。 | 既にDevinを利用している、または公開OSSの詳細な構造を知りたい場合。 |
| **Code Wiki** | Googleが提供する変更追従型のWiki生成ツール | Googleエコシステム（Gemini）を活用し、コード変更に自動追従する点が強力。 | プライベートリポジトリ対応（CLI）がウェイトリスト制で限定的。 | 公開OSSの調査や、Google環境を中心とした開発チームでの利用。 |
