...
quick_summary:
  ...
  target_users:
    ...
    - 開発チーム
    - OSSメンテナー
  latest_highlight: 2025年12月にSAST/SCA/IaC等の包括的セキュリティスキャンを追加、2025年2月に最新AIモデル対応やIDEチャットを改善
  update_frequency: 高
evaluation:
  score: 83
  base_score: 70
...
* **プルリクエストの自動レビュー**: GitHubやGitLabで作成されたプルリクエストに対して、即座に変更のサマリー、概要レベルのフィードバック、行単位の提案・コメントを提供。多様なAIモデルの特性を活かし多角的なレビューを行う。
* **包括的なセキュリティスキャン**: プロジェクトのライフサイクル全体で、SAST（静的アプリケーションセキュリティテスト）、SCA（ソフトウェアコンポジション解析）、シークレット検出、IaC構成のチェック、オープンソースライセンス検出を実施し、Jiraと連携したIssue自動作成も可能。
...
* **カスタムルール設定**: チーム独自のコーディング標準やスタイルガイドに合わせて、Sourceryのレビューにカスタムルールを追加可能。IDEチャットにおけるカスタムプロンプトも設定可能。
* **Sentryとの深い連携**: 本番環境のエラー監視ツールであるSentryと連携し、AIによる問題の調査から根本原因の特定、さらに修正PRの生成までをシームレスに自動化。
...
| **Team** | $24/月 | リポジトリ分析、無制限のセキュリティ問題修正、毎日スキャン、200以上のリポジトリでのセキュリティスキャン、独自のLLMの持ち込み(Bring your own LLM) |
...
* **外部サービス連携**:
  ...
  * プロジェクト管理: GitHub Issues, Jira (セキュリティ問題からのIssue自動作成等)
  * エラー監視: Sentry (エラーの自動調査・修正提案)
  * コミュニケーション: Slack (Sentry連携時の通知等)
...
| **VS Code / JetBrains / Cursor** | ◎ | 公式プラグインが提供されており、リアルタイムフィードバックやチャット履歴が利用可能。 | 特になし |
| **その他の言語** | ◯ | AIによる一般的なコードレビューやセキュリティスキャンは動作する。 | ルールベースの高度なリファクタリングは限定的 |

## **11. セキュリティとコンプライアンス**

* **認証**: GitHubやGitLabを通じたOAuth認証。
* **データ管理**: サードパーティのLLM（OpenAI, Anthropic等）にコードの差分を送信するが、コード自体を保存せず、LLMの学習データとしても使用されない。データの保持期間は最大30日と制限されている。
...
* **2025-02-27**: より強力な新しいAIモデル群への対応、カスタムプロンプトの追加、IDEチャット履歴機能の追加などの改善。Sentry連携（本番エラーの自動調査・修正）のクローズドベータも開始。
* **2025-01-23**: IDEでの新スタイルのチャット機能追加、1クリックでの変更適用。
* **2025-01-08**: IDEチャット体験の主要な改善、セルフホスト版GitLab・GitHubのサポート。
* **2025-12-23**: 包括的なセキュリティスキャン（SAST、SCA、シークレット検出、IaC構成のチェック、オープンソースライセンス検出など）の追加。Jira連携によるIssue自動作成にも対応。
* **2024-12-10**: 新しいIDEレビュー体験の導入。
* **2024-11-18**: IDEレビューの改善とボット操作の向上。

(出典: [Changelog](https://www.sourcery.ai/changelog))
...
|---------|------|------|------|------------------|
| **Sourcery** | セキュリティスキャンとAIレビューを兼ね備えたツール。 | Python等に特化した高度な静的解析とAIの融合に加え、SAST/SCA等の包括的セキュリティスキャン機能を搭載。IDEでのリアルタイム自動修正やSentry連携も強力。OSS無料。 | UIやドキュメントが英語中心。 | オープンソースプロジェクトや、Pythonを中心とした開発チームでセキュリティ（SAST等）とコード品質を同時に高め、IDE上での体験も重視したい場合。 |
| **CodeRabbit** | AIを用いたコードレビュー専門エージェント。 | 40以上の解析ツールとAIを組み合わせた精度の高いノイズフィルタリング。リポジトリ全体の文脈を理解し、人間のようなレビューを行う。日本語対応が強力。 | IDEでのリアルタイムな自動リファクタリング機能は持たない。 | PRレビューの負荷軽減と日本語でのやり取りを重視するチーム。 |
