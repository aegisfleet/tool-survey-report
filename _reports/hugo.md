...
quick_summary:
  ...
  target_users:
    ...
    - コンテンツ制作者
    - 技術ライター
  latest_highlight: 2026年6月にv0.163.3をリリース、AVIF画像対応の改善やセキュリティ強化を実施
  update_frequency: 高
evaluation:
  score: 83
  base_score: 70
...
* **アセットパイプライン (Hugo Pipes)**: Sass/SCSSのCSS変換、JavaScriptのバンドルと最小化（ESBuild利用）、画像処理（リサイズ、WebP/AVIF変換等）をビルドプロセスに統合できます。
...
* **2026-06-18 (v0.163.3)**: バグ修正と依存ライブラリの更新を含むパッチリリース。
* **2026-06-11 (v0.163.1)**: セキュリティの強化およびバグ修正。
* **2026-06-08 (v0.163.0)**: AVIF画像の処理改善（デフォルトの品質設定調整、ヒント機能の追加）。
* **2026-05-26 (v0.162.0)**: AVIF画像のエンコード・デコードを新たにサポート。
* **2026-02-25 (v0.157.0)**: Hugo ModulesのGitInfoサポート追加、AVIF, HEIF, HEICのパーシャルサポート追加などの改善。
* **2026-02-18 (v0.156.0)**: `collections.Where`と`collections.Sort`のパフォーマンスが大幅に向上し、APIのクリーンアップが行われた。
* **2026-02-08 (v0.155.3)**: isHTML出力フォーマットのデフォルトサイトリダイレクトの修正、サーバーのグローバルエラーログの修正等のバグ修正。
...
| 機能カテゴリ | 機能項目 | 本ツール (Hugo) | Astro | Jekyll | WordPress |
|:---:|:---|:---:|:---:|:---:|:---:|
| **基本機能** | 静的サイト生成(SSG) | ◎<br><small>世界最速級</small> | ◎<br><small>高速</small> | ◯<br><small>標準的</small> | △<br><small>要プラグイン</small> |
| **レンダリング** | SSR (サーバーサイド) | ×<br><small>非対応</small> | ◯<br><small>アダプター利用</small> | ×<br><small>非対応</small> | -<br><small>動的生成</small> |
| **開発体験** | 複数FW混在 | △<br><small>マウントが必要</small> | ◎<br><small>JSX等そのまま利用可</small> | -<br><small>テンプレートのみ</small> | -<br><small>PHPベース</small> |
| **パフォーマンス** | ゼロJS | ◎<br><small>デフォルト</small> | ◎<br><small>デフォルト</small> | ◎<br><small>デフォルト</small> | ×<br><small>スクリプトが多い</small> |
| **コンテンツ管理**| 使いやすさ | ◯<br><small>MarkdownとCLI</small> | ◯<br><small>Markdownとコンポーネント</small> | ◯<br><small>Markdownベース</small> | ◎<br><small>直感的なCMS</small> |

### **16.2 詳細比較**

| ツール名 | 特徴 | 強み | 弱み | 選択肢となるケース |
|---------|------|------|------|------------------|
| **Hugo (本ツール)** | Go製。超高速ビルドと単一バイナリが特徴。 | 圧倒的なビルド速度、依存関係なし、豊富な組込機能（AVIF画像対応等）。 | 独自のテンプレート構文、プラグインエコシステムの不在。 | 速度と安定性を最優先する大規模サイト、ドキュメントサイト。 |
| **Astro** | コンポーネント指向。アイランドアーキテクチャ。 | デフォルトでJSを送らず高速、主要なUIフレームワーク(React, Vue等)と連携可能。 | 高度に動的なWebアプリには不向き。 | パフォーマンスを重視しつつ、React等のコンポーネントを部分的に利用したい場合。 |
...
| **WordPress** | オープンソースのCMS。圧倒的なシェアとエコシステム。 | 非常に高い拡張性とカスタマイズ性。直感的なコンテンツ管理。 | 動的生成によるパフォーマンスとセキュリティ管理に知識が必要。 | 非エンジニアを含むチームで、豊富なプラグインと直感的なCMSを活用したい場合。 |
