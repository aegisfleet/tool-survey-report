...
* **調査対象**: Google検索 (G2, Capterraのレビュー情報), Twitter (X)
* **総合評価**: 4.8/5.0 (G2より引用)
* **ポジティブな評価**:
...
* **2026-03-13 (v15.12.0)**: Studioパネルのコードワードラップオプションの追加や各種バグ修正。
* **2026-02-25 (v15.11.0)**: `--pass-with-no-tests`コマンドラインフラグの追加、プロキシでのBrotli圧縮サポート。
* **2026-02-03 (v15.10.0)**: 非推奨の`Cypress.env()`に代わる`cy.env()`コマンドと`allowCypressEnv`オプションの導入。
...
(出典: [公式Changelog](https://github.com/cypress-io/cypress/releases))
