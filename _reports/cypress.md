...
* **Cypress 15.7.1 (2025年12月02日)**:
  * **パフォーマンス**: コマンドログのスナップショット表示パフォーマンスを改善し、特に複雑なページでの要素ハイライトが大幅に高速化された。
* **Cypress 15.7.0 (2025年11月19日)**:
  * **機能**: Next.js 16のコンポーネントテストをサポート（ただし、バンドラはwebpackのみ）。
  * **バグ修正**: `cy.wrap()` が循環参照を持つオブジェクトで呼び出された際に無限再帰を引き起こす問題を修正。
* **Cypress 15.6.0 (2025年11月04日)**:
  * **機能**: AI支援テスト機能である `cy.prompt()` が要素を見つけられずに自己修復した場合に、コマンドログに「Self-healed」バッジを表示するようになった。
