...
quick_summary:
  ...
  target_users:
    ...
    - スピーカー
    - 技術カンファレンス
  latest_highlight: 2025年12月にスライド内リンクのクリックに対応（最新機能）
  update_frequency: 低
evaluation:
  score: 76
  base_score: 70
...
* **コレクション機能**: アカウント内の複数のスライドを整理し、コレクションとしてまとめてナビゲーションしやすくする機能。
* **Pro機能（有料）**:
  * パスワード保護
  * 限定公開URL (Private URLs) / カスタムURL
  * スライドのダウンロード禁止設定
  * 予約投稿 (Scheduled publishing)
  * 広告・ブランディングの削除
  * スライドノート（各スライドへの補足テキスト追加）
  * 指定ドメインでのみ埋め込みを許可する制限機能 (Domain-level privacy)
  * 埋め込みプレイヤーのプログレスバーの色変更
...
| プラン名 | 料金 | 主な特徴 |
|---|---|---|
| **Free** | 無料 | 1日10ファイルまでアップロード可。生涯100ファイル制限あり。1ファイル最大150MB。広告表示あり。 |
| **Pro** | $8/月<br>($80/年) | 無制限アップロード。1ファイル最大250MB。プライベートURL、パスワード保護、ダウンロード禁止、広告非表示、予約投稿、スライドノート、埋め込みドメイン制限。 |
...
* **2025-12-28**: **2025’s Most Viewed Speaker Deck Presentations** - 年間ランキング（Most Viewed Decks）の発表。
* **2025-12-27**: **Introducing Clickable Links** - スライド内のリンクをクリック可能にする機能を追加。これまでPDF内のリンクは無効化されていたが、閲覧者が直接リソースにアクセス可能になった。
* **2025-12-26**: **Trending Storyboards of 2025** - ストーリーボード機能のトレンド公開。
...
| **Docswell** | 日本発の共有サービス | 日本語検索に強い、スライド・動画共有やフォルダ公開対応、アクセス解析が詳細 | ユーザー数はSlideShareに劣る | 日本国内向けのマーケティング、詳細な分析や動画共有が必要な場合 |
