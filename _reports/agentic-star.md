...
quick_summary:
  ...
  target_users:
    ...
    - DX推進担当者
    - 企画・マーケティング部門
  latest_highlight: モック開発の30分への短縮やIaC構築の工数95%削減などの実績公開
  update_frequency: 中
evaluation:
  score: 77
  base_score: 70
...
* **長期記憶とパーソナライズ**: ユーザーとの過去のやり取りやドキュメントを学習・保持し、個々のユーザーや組織の背景知識（コンテキスト）を踏まえた支援を行います。
* **サンドボックス環境**: 本番環境や業務データから切り離された安全な実行環境を内蔵し、生成したコードをその場で実行・検証し、エラーがあれば自己修復します。
...
* **導入事例**:
  * **アプリ開発・要件定義**: API仕様の自律的な解釈や設計・実装の並行処理により、モックアプリ開発の時間を約30時間から約30分へ（約98%）短縮。議事録から要件定義書等のドラフトを自動生成する業務でも活用されています。
  * **インフラ構築（IaC）**: 長期記憶を活用して設計の背景を保持し、Terraform実行からエラー修正までを自律的に行うことで、インフラ構築工数を約10人日から約0.5人日へ（約95%）削減。
  * 提案資料作成などの業務でも活用されています。
...
* **調査対象**: 公式発表、プレスリリース、SNS（X等）、公式ビジネスブログ
* **総合評価**: リリース直後のため、数値的な総合評価はまだ確立されていません。
* **ポジティブな評価**:
  * 「API仕様の調査や環境構築などの準備作業から解放され、モック開発が約30時間から約30分に短縮された」（社内事例）
  * 「長期記憶があるため、設計の背景が引き継がれ、ブラックボックス化や属人化を防ぐ強力なパートナーになる」（社内事例）
  * 「資料作成の時間が大幅に短縮できた」（社内事例）
  * 「要件定義のたたき台作成が楽になった」（社内事例）
* **ネガティブな評価 / 改善要望**:
  * 「料金が不明確で導入検討が進めにくい」（一般的な反応）
* **特徴的なユースケース**:
  * モックアップの作成からテストまでの自律的な実行。
  * IaC（Infrastructure as Code）のコード生成・実行からエラー特定・修正までの自律ループ。
  * 提案シナリオからスライド作成までの一気通貫などの「クリエイティブ×ロジカル」な業務。

## **15. 直近半年のアップデート情報**

* **2026-03-24**: アプリケーション開発（モック開発）における活用事例を公開。API仕様の調査から設計、実装、テストまでを自律的に行い、開発時間を大幅に短縮。
* **2026-03-16**: 情報システム部門におけるインフラ構築（IaC）への活用事例を公開。「長期記憶」を活用してシステムの属人化・ブラックボックス化を防ぎつつ、構築工数を削減する取り組みを紹介。
* **2025-12-11**: AGENTIC STAR の提供開始。SaaSモデルおよびカスタマイズモデルをローンチ。
* **2025-11-19**: 開発者向けブログにてAIエージェント開発の裏側を公開。

(出典: [ソフトバンク ビジネスブログ](https://www.softbank.jp/biz/blog/business/))
...
|:---:|:---|:---:|:---:|:---:|:---:|
| **基本機能** | 自律タスク実行 | ◎<br><small>ゴール設定のみで実行</small> | ◯<br><small>Agent Mode機能はあるが支援寄り</small> | ◎<br><small>計画から実行まで完全自律(開発特化)</small> | ◎<br><small>CRMフロー内で完全自律可能</small> |
| **カテゴリ特定** | 汎用性 | ◎<br><small>業務全般に対応</small> | ◎<br><small>Office連携に強み</small> | △<br><small>開発に特化</small> | △<br><small>CRM/顧客対応に特化</small> |
| **エンタープライズ** | セキュリティ | ◎<br><small>独立仮想環境</small> | ◎<br><small>Ent.グレード、Purview対応</small> | ◯<br><small>EnterpriseはVPC/SSO対応</small> | ◎<br><small>最高水準のSalesforce基準</small> |
