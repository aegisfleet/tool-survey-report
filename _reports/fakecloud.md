...
quick_summary:
  ...
  target_users:
    - 開発者
    - QAエンジニア
  latest_highlight: v1.1.0リリース（EventBridgeの強化など）
  update_frequency: 高
evaluation:
  score: 90
  base_score: 70
...
* **アーキテクチャ**: ローカルファーストの単一バイナリによるエミュレーター構成。
* **主要コンポーネントとデータフロー**:
  * AWS SDKやCLIからのリクエストをローカルのHTTPサーバー（デフォルト `localhost:4566`）で受信し、Go言語で実装された各サービスのエミュレーターが処理を行う。
  * 状態（ステート）はデフォルトでオンメモリに保持される。
  ```mermaid
  graph TD
      A[AWS SDK / CLI] -->|HTTP Request| B(fakecloud HTTP Server)
      B --> C{サービスルーター}
      C --> D[S3 エミュレーター]
      C --> E[SQS エミュレーター]
      C --> F[Lambda エミュレーター]
  ...
  ```
* **特筆すべき要素技術**:
  * **Go言語による単一バイナリ**: 依存関係なしで高速に起動。
  * **Docker統合**: LambdaやRDSなど、ステートフルや実行環境が必要なサービスのみオンデマンドでDockerコンテナを起動するハイブリッドな構成。
...
* **ドキュメント**: 公式ドキュメントサイト（[https://fakecloud.dev/docs](https://fakecloud.dev/docs)）で Getting Started、サービスリファレンス、SDKガイドが網羅されている。
...
* **2026-04-15**: v1.1.0リリース（EventBridgeの強化など）。
