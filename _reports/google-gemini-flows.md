...
* **前提条件**:
  ...
  * 現在はLimited Previewのため、ホワイトリストへの登録や対象ユーザーとしての招待が必要な場合があります。
* **インストール/導入**:
  * ソフトウェアのインストールは不要です。ブラウザベースで動作し、Google Workspace環境にアドオンとして統合されます。
* **初期設定**:
  * 管理コンソールで対象ユーザーへのアプリケーションの有効化が必要になる場合があります。
* **クイックスタート**:
  1. Google Workspaceのアカウントでログインし、Workspace Studioにアクセスします。
  2. チャットUIで「〇〇の処理を自動化したい」とプロンプトを入力します。
  3. Geminiが提案するフロー（Flows）を確認し、必要に応じてステップ（Steps）や変数を調整します。
  4. テスト実行後、フローをデプロイして稼働を開始します。
...
* **日本語対応**: UIや機能の詳細な日本語サポート状況については公式からの発表が少なく、英語環境が前提となる場合があります。
...
* **2026-02-15**: Google Workspace全体のGemini統合がさらに進化し、Flowsへのフィードバック機構が向上したとの報告あり。
* **2025-12-11**: ドキュメント更新。Workspace StudioのガイドラインやAPIリファレンスが整備された。
* **2025-08-01**: Google Workspace Studio (Flows) のLimited Previewプログラムが拡大され、多くの機能がテスト可能になった。

(出典: [Google Workspace for Developers Documentation](https://developers.google.com/workspace/add-ons/studio))
