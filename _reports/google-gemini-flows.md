...
* **前提条件**:
  ...
  * 現在はLimited Previewのため、利用申し込みが必要。
* **インストール/導入**:
  * Google Workspace のアドオンとして提供され、個別のインストールコマンドは不要。
* **初期設定**:
  * Gmail、Drive、Chat などの各アプリとの連携許可を初回実行時に承認する。
* **クイックスタート**:
  * Chatやサイドパネルから、自然言語で「〜したら〜するフローを作って」とGeminiに指示を出すことで構築を開始する。
