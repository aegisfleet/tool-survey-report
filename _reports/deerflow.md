...
quick_summary:
  ...
  target_users:
    ...
    - データサイエンティスト
    - リサーチャー
  latest_highlight: 2026年6月にv2.0.0をリリースし、自己更新エージェントやIMチャネル連携などを追加
  update_frequency: 高
evaluation:
  score: 83
  base_score: 70
...
* **自己更新エージェント**: `SOUL.md` や `config.yaml` への編集を通常のチャットから永続化できる。
* **ユーザー所有のIMチャネル接続**: Slack, Telegram, Discord, Feishu/Lark, DingTalk, WeChat, WeCom のアカウントをバインド可能。
* **高度なコミュニティ検索**: Brave Search, SearXNG, Browserless, Serper Google Images などの検索ツール連携。

## **4. 動作原理・システム構成**

* **アーキテクチャ**: ローカルファーストのクライアント・サーバー構成。
* **主要コンポーネントとデータフロー**:
  * ユーザーの指示やスキル（Markdown）を読み込み、サブエージェントがタスクを分割・計画する。タスク実行は安全なDockerサンドボックス内で行われ、結果はリードエージェントに統合される。長期・短期メモリシステムを利用し、セッション間の文脈も維持する。
* **特筆すべき要素技術**:
  * Dockerコンテナによる分離環境
  * Agent Skills（Markdownによる動的ロード）
  * 長期・短期メモリシステムによるContext Engineering
...
* **外部サービス連携**: OpenAI、Anthropic (Claude Code)、Gemini などのLLM群。InfoQuest、Brave Search、SearXNG、Browserless、Serper Google Images などの高度な検索ツール連携。Telegram、Slack、Discord、Feishu(Lark)、DingTalk、WeChat、WeCom などのチャット統合。
...
* **2026-06-25**: DeerFlow 2.0.0リリース（グランドアップ・リライトされ、自己更新エージェントやユーザー所有のIMチャネル接続、各種検索ツールなどが追加）。
...
(出典: [GitHub Releases](https://github.com/bytedance/deer-flow/releases))
...
|:---:|:---|:---:|:---:|:---:|:---:|
| **基本機能** | 自律タスク実行 | ◎<br><small>サブエージェントで複雑なタスクに対応</small> | ◎<br><small>高度な推論とツール操作。2026年3月にGemini 3.1 Pro搭載</small> | ◎<br><small>計画から実行まで完全自律。2026年6月にMCP MarketplaceやGitLab連携を追加</small> | ◎<br><small>完全自律型。2026年3月にv1.6.0リリース</small> |
...
| **Google Jules** | Google製のエージェント型AI (2026年3月にGemini 3.1 Pro搭載) | 高度な推論能力とGoogle Workspace等のエコシステムとの強力な連携。 | オープンソースではなく、カスタマイズの自由度は限られる。 | Googleサービスを中心に業務を行っており、強力なAIアシスタントが必要な場合。 |
| **Devin** | 完全自律型AIソフトウェアエンジニア (2026年6月にMCP MarketplaceやGitLab連携を追加) | 計画からデプロイまで一貫して実行可能。独自の統合開発環境を持つ。 | コストが高めで、ローカル環境での完全なコントロールが難しい。 | ソフトウェア開発の特定のプロセス（プロトタイプ作成やデバッグ等）を丸ごと委任したい場合。 |
| **OpenHands** | OSSの自律型エージェント (2026年3月にv1.6.0リリース) | オープンソースであり、ローカルで実行可能。ソフトウェア開発タスクに特化。 | セットアップや環境構築に手間がかかる場合がある。 | OSSで自律型のコーディングアシスタントをローカルで動かしたい場合。 |
