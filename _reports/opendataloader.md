...
quick_summary:
  ...
  target_users:
    - 開発者
    - AI・機械学習エンジニア
  latest_highlight: v2.5.0をリリースし、目次(TOC)検出や打ち消し線検出など解析精度が向上
  update_frequency: 高
evaluation:
  score: 83
  base_score: 70
...
* **ハイブリッドAIモード**: ローカルの決定論的処理とAIバックエンド（Hancom AIなど）を組み合わせ、複雑な境界線なしの表、LaTeX数式、チャートの説明生成、OCRなどを高精度に処理します。
...
* **アクセシビリティ自動化**: PDF AssociationおよびveraPDF開発元のDual Labとの協力により、タグなしPDFを自動的に解析し、タグ付きPDFを生成するパイプラインをオープンソース（Apache 2.0）で提供しています。
* **目次（TOC）と装飾の検出**: 目次構造の検出や、打ち消し線・下線といったテキスト装飾の抽出に対応。

## **4. 動作原理・システム構成**

* **アーキテクチャ**: ローカルでの決定論的処理と、AIバックエンドサーバーを組み合わせたハイブリッド構成。
* **主要コンポーネントとデータフロー**:
  * 通常モード: ローカル環境で高速にPDFを処理し、バウンディングボックスを含む構造化データ（JSON等）を出力する。
  * ハイブリッドAIモード: 複雑なページ（スキャン画像、表、数式等）を処理する場合、ローカルで起動したAIバックエンドサーバーへデータを送り、高精度な解析結果を受け取る。
* **特筆すべき要素技術**:
  * 読み取り順序の決定にXY-Cut++アルゴリズムを採用。
...
* **初期設定**:
  APIキーやクラウドの登録は不要で、ローカル環境ですぐに利用可能です。また、大量のページを処理する際は `--threads` オプションによる並列処理（マルチスレッド）が利用可能です。
* **クイックスタート**:
...
| プラン名 | 料金 | 主な特徴 |
|---------|------|---------|
| **オープンソース版** | 無料 | コアライブラリ（Apache 2.0ライセンス）。データ抽出、ハイブリッドモード（OCR、表解析など）、AI安全性フィルタ、自動タグ付けが利用可能。 |
...
* **2026-07-14**: v2.5.0リリース。目次(TOC)検出や打ち消し線の検出に対応し、JSONやHTML出力時のメタデータを強化。
* **2026-05-27**: v2.4.7リリース。ハイブリッドモードにおけるDLAオブジェクトの永続化、JSON出力でのAIスコア（ai_score）・PDF/UAタグ情報出力の追加。
* **2026-04-29**: v2.4.0リリース。Hancom AI連携の統合、PDF/UA-2対応の改善、`--threads`による並列処理をサポート。
* **2026-04-21**: v2.3.0リリース。自動タグ付け（auto-tagging）によるTagged PDF生成機能を搭載し、アクセシビリティ対応を強化。
