...
quick_summary:
  ...
  target_users:
    ...
    - システム管理者
    - 企業
  latest_highlight: 2026年4月にUbuntu 26.04 LTS (Resolute Raccoon) をリリースし、NVIDIA CUDAやAMD ROCmのネイティブサポート、Arm64でのLivepatch対応を追加
  update_frequency: 高
evaluation:
  score: 95
  base_score: 70
...
* **AI/MLツールキットのネイティブサポート**: NVIDIA CUDAやAMD ROCmといった業界標準のAI/MLツールキットを公式リポジトリからネイティブに提供し、AI開発環境の構築を簡素化。
* **GNOME Desktop**: 直感的でモダンなデスクトップ環境を提供し、WindowsやmacOSからの移行もスムーズ（Waylandデフォルト）。
...
* **前提条件**:
  * 仮想マシン、またはUSBブート可能なPC環境。
  * 少なくとも20GBのディスク容量と4GBのRAM（デスクトップ版の場合）。
  * インターネット接続環境。
* **インストール/導入**:
...
* **初期設定**:
  * インストールウィザードに従い、タイムゾーンやキーボードレイアウトを選択し、初期ユーザー（sudo権限あり）を作成。
* **クイックスタート**:
  * インストール完了後、ターミナルを開き、基本的なツールをインストールして環境構築を開始。
...
* **AIレディなプラットフォーム**: 公式リポジトリを通じたNVIDIA CUDAやAMD ROCmの提供により、エンタープライズレベルでのAIワークロードの展開と運用が容易。
...
* **高度なセキュリティ・メモリ安全性**: TPMを利用したフルディスク暗号化（FDE）や、Rust製システムツール（sudo-rs, uutils coreutilsなど）の導入により、OSの根本から安全性を向上させている。
...
* **認証**: Active DirectoryやLDAPとの統合に加え、`Authd` によりEntra IDやGoogle IAMなどのクラウドIDプロバイダ（OIDC）をネイティブにサポート。
* **データ管理**: インストール時のTPM連携フルディスク暗号化（TPM-backed FDE）、実験的なZFS暗号化などをサポート。ハードウェアチップによる強固な暗号化が可能。
* **準拠規格**: FIPS 140-3, FedRAMP, HIPAA, PCI-DSSなどの主要なセキュリティ基準に準拠。Ubuntu ProではCISベンチマークやDISA-STIGプロファイルの適用を自動化するツールも提供。
* **脆弱性対応**: Kernel Livepatchにより、再起動なしでカーネルのセキュリティパッチを適用可能（Arm64にも対応）。メインリポジトリだけでなく、ユニバースリポジトリもUbuntu Proで10年間のセキュリティサポート対象となる。
...
* **2026-04-23**: **Ubuntu 26.04 LTS (Resolute Raccoon) リリース**。NVIDIA CUDA/AMD ROCmの公式リポジトリでの提供、Waylandのデフォルト化、TPMバックのFDE、Rustによるメモリ安全性向上などが特徴。
...
(出典: [Ubuntu Blog - Releases](https://ubuntu.com/blog/category/releases))
...
| **先進機能** | AI/ML対応 | ◎<br><small>CUDA等ネイティブ</small> | △<br><small>手動設定</small> | ◯<br><small>比較的新しい</small> | △<br><small>エンタープライズ</small> |
