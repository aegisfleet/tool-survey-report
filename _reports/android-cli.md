...
quick_summary:
  ...
  target_users:
    ...
    - AIエージェント
    - CI/CDエンジニア
  latest_highlight: 2026年9月のアップデートでレイアウトダンプの最適化（--no-idle, --fullフラグ）および自動補完生成コマンド等が追加された
  update_frequency: 中
evaluation:
  score: 85
  base_score: 70
...
* **アプリのデプロイメント機能**: 指定したAPKファイルを接続されたデバイスまたはエミュレータにデプロイし、特定のアクティビティを起動（`android run`）。増分APKプッシュによる高速なインストール機能（`android install`）も提供される。
* **UIレイアウト取得機能**: 接続されたデバイスで実行中のアクティブなアプリのUIレイアウトをJSON形式で取得（`android layout`）。待機なし（`--no-idle`）や全ツリー取得（`--full`）も可能。
...
* **シェル自動補完機能**: BashやZsh向けのシェル自動補完スクリプトの生成・インストール（`android completion`）。
* **Android Studio連携機能**: IDEのインスペクションエンジンによるファイル分析や、Composeプレビューのレンダリング、シンボル宣言の検索等、AIエージェントからAndroid Studioの機能に直接アクセス（`android studio`）。

## **4. 動作原理・システム構成**

* **アーキテクチャ**: ローカル環境で実行されるコマンドラインツール。ユーザーのマシン（Windows / macOS / Linux等）にインストールされ、動作する。
* **主要コンポーネントとデータフロー**:
  * ユーザーまたはAIエージェントがターミナルから `android` コマンドを実行する。
  * コマンドラインからの入力は、CLI内部で解釈され、必要に応じて背後にあるAndroid SDK、ADB（Android Debug Bridge）、エミュレータなどのツール群へと渡される。
  * 処理結果（JSONメタデータ、ログ、画像データ等）は標準出力（stdout）やファイルとしてローカル環境内に返され保存される。
* **特筆すべき要素技術**:
  * 内部的にAndroid SDKやADBなどをラップし、複雑な操作を単一のコマンドラインインターフェースに統合している。
  * AIエージェントとの統合を前提としており、JSON形式での出力（`android layout`等）やエージェントのスキル設定の追加（`android skills`）を標準でサポートしている。

```mermaid
graph TD
    User["ユーザー / AIエージェント"] -->|コマンド実行| AndroidCLI["Android CLI"]
    AndroidCLI -->|内包・ラップ| ADB["ADB (Android Debug Bridge)"]
    AndroidCLI -->|内包・ラップ| SDKManager["Android SDK Manager"]
    AndroidCLI -->|内包・ラップ| Emulator["Android Emulator"]

    ADB -->|操作・情報取得| Device["物理デバイス / 仮想デバイス"]
    Emulator -->|起動・管理| Device

    Device -->|出力・データ| AndroidCLI
    AndroidCLI -->|標準出力 / JSON / 画像| User
```
...
* **2026-09**: レイアウトダンプの最適化（`--no-idle`および`--full`フラグの追加）、Bash/Zsh向けのシェル自動補完生成コマンド（`android completion`）の追加。
* **2026-07**: 高速な増分APKインストールに特化した `android install` コマンドの追加、マルチパッケージ削除のサポート、Claude Code、Cursor、Cline、OpenHands、Windsurf、Zed、Devin等を含む幅広いエージェントスキルのサポート追加。
* **2026-05**: Antigravity 2.0 への統合、`android studio` コマンド（コード分析やComposeレンダリング等）の追加、Journeysによるクリティカルなユーザー体験テストのサポート追加。
* **2026-04-16**: Android公式ドキュメントにおけるAndroid CLIページが更新され、エージェント向けのスキル機能やスクリーン解決機能などの最新仕様が記載された。

(出典: [Android Developers - Android CLI Release notes](https://developer.android.com/tools/agents/android-cli/release-notes))
