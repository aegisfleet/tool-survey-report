...
* **Tableau Pulse**: 生成AIを活用し、ユーザーに合わせてパーソナライズされたデータのインサイト（洞察）や高度な自動インサイトを配信する機能。
* **Tableau Agent (旧 Einstein Copilot for Tableau)**: 自然言語で質問するだけで、計算フィールドの作成やデータの説明、ダッシュボードの自動生成を行ってくれるAIアシスタント。
* **データコネクタ**: Excel, CSVから、Salesforce, Google BigQuery, AWS Redshift, Snowflakeなど80種類以上のデータソースに接続可能。

## **4. 動作原理・システム構成**

* **アーキテクチャ**: クライアント・サーバー構成（Tableau Server）およびクラウド完結型SaaS（Tableau Cloud）
* **主要コンポーネントとデータフロー**:
  * データソース（DB、ファイルなど）からデータを抽出し、インメモリデータエンジン（Hyper）に保存、またはライブ接続で直接クエリを実行。
  * VizQLエンジンがユーザーのドラッグ＆ドロップ操作をデータベースクエリ（SQL等）に変換し、結果を視覚的にレンダリングする。
* **特筆すべき要素技術**:
  * **VizQL**: 独自のビジュアルクエリ言語
  * **Hyper**: 高速インメモリデータエンジン
...
* **2026-07-21**: Tableau 2026.2.1 マイナーリリース。不具合修正および安定性の向上。
* **2026-06-09**: Tableau 2026.2 リリース。Tableau Agentの機能拡張による自然言語ダッシュボード生成や、Tableau Pulseでのより高度な自動インサイト配信機能が強化された。
...
| **AI機能** | 分析アシスト | ◯<br><small>Tableau Agent</small> | ◎<br><small>Copilot</small> | ◎<br><small>Amazon Q統合</small> | △<br><small>限定的</small> |
