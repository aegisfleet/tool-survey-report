...
quick_summary:
  ...
  target_users:
    - 開発者
    - 開発チーム
  latest_highlight: Agent KanbanやClaude Code対応を含む包括的なAIエージェント統合とコンフリクトファイルのパネル追加
  update_frequency: 高
evaluation:
  score: 83
  base_score: 70
...
* **GitKraken AI**: AIを活用して、コミットメッセージの生成、変更の要約、PRの説明作成、変更履歴(Changelog)の生成などを自動化します。また、Claude Code Hooksなどのエージェント統合（MCPサーバー登録含む）により高度な自動化をサポートします。
* **Cloud Workspaces & Worktrees**: 複数のリポジトリをグループ化して管理するCloud Workspacesや、現在のブランチから離れることなく別ブランチで作業できるWorktreesを提供します。
* **Interactive Rebase Editor**: ターミナルから独立した直感的なリベースエディタを提供し、コンフリクトの解決をサポートするパネルを搭載しています。

## **4. 動作原理・システム構成**

* **アーキテクチャ**: VS Code拡張機能アーキテクチャに準拠した、ローカルファーストのクライアントアプリケーション。
* **主要コンポーネントとデータフロー**:
  * ユーザーのローカルGitリポジトリを直接読み込み、VS CodeのUI上にBlameやGraphを描画。
  * GitKraken AIやLaunchpadなどの高度な機能を利用する際は、GitKrakenのクラウドAPIおよび外部LLMプロバイダーと通信を行う。
* **特筆すべき要素技術**:
  * Model Context Protocol (MCP): 外部のAIエージェントと連携するための標準プロトコル。
  * Git Worktrees: 別ブランチでの作業をローカルで効率的に行うためのGitネイティブ機能。
...
* **包括的なAIエージェント統合**: Claude Code統合やModel Context Protocol (MCP) を通じた外部エージェントとの連携により、IDE内で高度なAI支援とタスク自動化を実現します。
...
* **2026-04-22 (v17.12.2)**: ターミナル統合の問題修正、GitLens MCP（Model Context Protocol）のインストール改善、インタラクティブリベースエディタの挙動改善等の修正と最適化。
* **2026-04-14 (v17.12.0)**: Claude Code Hooksを通じたAgent Statusフレームワークの実装、Agent Kanbanモードの追加、インタラクティブリベース時のコンフリクトファイルパネルの追加など、AI連携と利便性を大幅に向上。
* **2026-03-04 (v17.11.0)**: Claude Sonnet 4.6 / Opus 4.6、Gemini 3.1 Pro プレビューなどの最新AIモデルに対応。Gitサブモジュールの基本サポートを追加。
...
| **AI連携** | コミット生成等 | ◎<br><small>Claude Code/MCP連携等、広範なAI統合</small> | ◎<br><small>GitKraken AI搭載</small> | ×<br><small>サードパーティ頼り</small> | ×<br><small>非対応</small> |
