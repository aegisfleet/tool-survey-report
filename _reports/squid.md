...
quick_summary:
  ...
  target_users:
    ...
    - システム管理者
    - ネットワーク管理者
  latest_highlight: v7.6のリリース（2026年6月）
  update_frequency: 中
evaluation:
  score: 85
  base_score: 70
...
* **アーキテクチャ**: ローカルファーストのクライアント・サーバー構成（主にプロキシ・キャッシュサーバーとして動作）。
* **主要コンポーネントとデータフロー**:
  * クライアントからのHTTP/HTTPS/FTPリクエストをSquidが受け取る。
  * ACL（アクセス制御リスト）によるフィルタリングが行われる。
  * キャッシュヒットした場合は、ディスク（UFS, AUFS等）またはメモリ（RAM）から直接応答を返す。
  * キャッシュミスした場合は、Squidがバックエンドサーバーへリクエストを転送し、取得したデータをクライアントに返しつつ、キャッシュストレージに保存する。
* **特筆すべき要素技術**:
  * **SSL Bumping**: HTTPS通信を動的に復号し、中身を検査・キャッシュした後、再暗号化してクライアントに送る中間者（MITM）的プロキシ機能。
  * **ICAP (Internet Content Adaptation Protocol)**: ウイルススキャンやコンテンツフィルタリングを外部サーバーへ委譲するためのプロトコル。
  * **イベント駆動型I/O**: epoll（Linux）やkqueue（BSD）などを活用し、単一プロセスまたは少数のワーカープロセスで多数の同時接続を処理する設計。
...
* **2026-06-08**: v7.6 リリース（HTTP/1.1 Transfer-Encoding:identityの禁止、Nettle 4.0 md5_digest APIサポートなど）
