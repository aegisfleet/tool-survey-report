...
quick_summary:
  ...
  target_users:
    ...
    - "スタートアップ"
    - "デザイナー"
  latest_highlight: "2025年12月にFlutter 3.38のマイナーアップデートを実施"
  update_frequency: "高"
...
* **表現力豊かで一貫したUI**: 独自のレンダリングエンジン（Skia/Impeller）を使用し、プラットフォームのUIに依存しないピクセルパーフェクトなUIを構築。これにより、OSのバージョンやデバイスに関わらず、一貫したデザインを実現。
...
* **ポジティブな評価**:
  ...
  * 「AIコーディングツール（Cursor等）との相性が良く、Widgetの生成がスムーズに行える」
* **ネガティブな評価 / 改善要望**:
  * 「サードパーティライブラリの更新が遅れることがあり、OSの最新バージョンへの対応に時間がかかる場合がある」
  * 「ネイティブアプリと比較すると、特に複雑な処理でパフォーマンスが低下することがある」
  * 「Impellerエンジンの導入で改善されたが、古いiOSデバイスでのシェーダーコンパイルによるカクつきが以前は問題だった」
* **特徴的なユースケース**:
...
* **2026-01-08**: **メンテナンスアップデート**
  * 安定性とパフォーマンスの向上を含むマイナーパッチがリリース。
* **2025-12-12**: **Flutter 3.38.5**
  * 複数のバグ修正とレンダリングエンジンの安定性向上。iOS 18以降での特定のクラッシュ問題に対処。
* **2025-11-12**: **Flutter 3.38 & Dart 3.10**
  * **AI支援開発**: Gemini CLI拡張機能が正式発表され、AIによるコード生成とリファクタリング支援が強化。
  * **Web**: Jasprフレームワークによる公式サイトの刷新と、Web設定ファイルのサポート追加。
  * **開発ツール**: Widget PreviewerのIDE統合が強化され、VS Codeでのプレビュー体験が向上。
* **2025-08**: **Flutter 3.35 & Dart 3.9**
  * **Web**: Stateful hot reloadがデフォルトで有効化。
  * **開発ツール**: Widget Previews機能が実験的に導入。
  * **AI連携**: MCPサーバーの安定版リリースによるAIアシスタントとの連携強化。

(出典: [Flutter official blog](https://medium.com/flutter), [Flutter Release Notes](https://docs.flutter.dev/release/whats-new))
...
* **総合的な評価**:
  Flutterは、クロスプラットフォーム開発において依然としてトップクラスの選択肢であり、特に開発速度とUIの一貫性・表現力において卓越しています。2025年後半のアップデートでAI支援機能やWeb対応がさらに強化され、モダンな開発フローへの適応が進んでいます。
* **推奨されるチームやプロジェクト**:
...
* **選択時のポイント**:
  プロジェクトが「UI/UXの一貫性」と「開発速度」を最優先するならば、Flutterは非常に強力な候補です。一方で、Kotlin Multiplatformのように「ネイティブの完全なUXとロジック共有」を両立するアプローチも成熟しており、プロジェクトの要件（UIのネイティブ忠実度 vs 開発効率）に応じた選択が重要です。
