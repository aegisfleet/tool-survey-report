...
quick_summary:
  ...
  target_users:
    ...
    - セキュリティエンジニア
    - DevOpsエンジニア
  latest_highlight: DevOps Agentの一般公開(GA)と無料トライアル、およびKiro Webの公開を発表
  update_frequency: 高
evaluation:
  score: 85
  base_score: 70
...
* **Kiro autonomous agent (開発)**:
  ...
  * ブラウザベースのKiro WebやClaude Opus 4.7モデルでのAdaptive Thinkingを用いた強力な自律実行。
...
* **アーキテクチャ**: 各ドメインに特化した複数のAIエージェントが、AWS環境および外部ツールと連携するクラウド完結型アーキテクチャ。
* **主要コンポーネントとデータフロー**:
  * **Kiro**: クラウド上のサンドボックス環境で動作し、GitHub等からコードを取得、実行、PR作成を行う。VS Code互換IDEやブラウザ（Kiro Web）からアクセス。
  * **Security Agent**: AWS環境の設計書やコードを継続的にスキャンし、サンドボックス内で攻撃シナリオをエミュレート。
  * **DevOps Agent**: CloudWatch等のテレメトリデータやログを収集・相関分析し、インフラ全体のトポロジーを推論して根本原因を特定する。
* **特筆すべき要素技術**:
  * **ネイティブMCP (Model Context Protocol) 統合**: Kiro等のエージェントはMCPをネイティブにサポートし、外部データソースやツールとシームレスに連携。
  * **Agent Hooks**: Kiro等におけるイベント駆動型のタスク実行。ファイル保存などをトリガーにテストやドキュメント更新を自動化。
...
| エージェント | 料金 | 主な特徴 |
|---|---|---|
| **Kiro autonomous agent** | 無料 (Freeプラン) | 月50クレジット付与。Pro ($20/月)、Pro+ ($40/月)などの上位プランあり。 |
| **AWS Security Agent** | 要確認 | プレビュー期間中の料金体系は公式発表を参照。 |
| **AWS DevOps Agent** | 従量課金 | 分析データ量等に基づく従量課金。新規向けに2ヶ月の無料トライアルあり。 |

* **課金体系**: Kiroは利用機能やモデルに応じたクレジット消費型。DevOps Agent等は分析データ量や処理量に基づく従量課金。
* **無料トライアル**: Kiroは初回サインアップ時にボーナスクレジットあり。DevOps Agentは2ヶ月間の無料トライアルが利用可能。
...
* **2026-05-26**: KiroがHIPAA準拠のサービスとして追加。
* **2026-05-07**: ブラウザベースのKiro Web (Preview) 公開。Claude Opus 4.7サポート。
* **2026-03-31**: AWS DevOps Agentが一般公開（GA）され、無料トライアル開始。
...
|:---:|:---|:---:|:---:|:---:|
| **基本機能** | 自律タスク実行 | ◎<br><small>3領域に特化</small> | ◎<br><small>完全自律</small> | ◎<br><small>Copilot Agents</small> |
| **領域** | カバー範囲 | ◎<br><small>開発・Sec・Ops</small> | ◯<br><small>開発中心</small> | ◯<br><small>開発中心</small> |
| **環境** | 実行環境 | ◎<br><small>AWS基盤</small> | ◎<br><small>クラウド上のIDE</small> | ◎<br><small>各種IDE/CLI等</small> |
| **信頼性** | エンタープライズ | ◎<br><small>AWS基準</small> | ◯<br><small>VPCデプロイ対応等</small> | ◎<br><small>Microsoft基準</small> |
