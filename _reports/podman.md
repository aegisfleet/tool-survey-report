...
* **Podman v5.7.1 (2025年12月10日)**:
    * **バグ修正**:
        * FreeBSDでのエミュレートされたLinuxコンテナへのデバイス追加が機能しない問題を修正。
        * 特定の状況下で`podman system migrate`コマンドがパニックを起こす可能性がある問題を修正。
        * `podman kube play`コマンドでファイルディスクリプタがリークする可能性がある問題を修正。
    * **依存関係の更新**:
        * Buildahをv1.42.2に更新。
        * containers/commonをv0.66.1に更新。

* **Podman v5.7.0 (2025年11月11日)**:
    * **セキュリティ**:
        * runcコンテナのエスケープとサービス拒否を許容する脆弱性（CVE-2025-52881）に対処。
    * **新機能**:
        * **TLS/mTLSサポート**: リモートクライアントとAPIサーバー間の接続をTLS/mTLSで暗号化する機能を追加。
        * **複数ファイル対応**: `podman kube play`および`podman kube down`で複数のYAMLファイルを一度に処理可能に。
        * **Quadletの強化**: OCIアーティファクトを管理するための`.artifact`ファイルをサポート。`.pod`ファイルで`StopTimeout`キーをサポートするなど、多数の機能拡張を実施。
        * **新しいWindowsインストーラ**: 管理者権限不要のユーザー単位インストールとマシン単位インストールをサポートする新しいMSIインストーラを導入。
    * **変更点**:
        * **BoltDBの廃止警告**: Podman 6.0で予定されているBoltDBデータベースの削除に向け、デフォルトで警告が表示されるように変更。
        * **Go 1.24が必須に**: PodmanのビルドにGo 1.24が必要になりました。
    * **APIの変更**:
        * Quadletをリストするための新しいAPIエンドポイントを追加 (`/libpod/quadlets/json`)。
        * Docker API v1.45との互換性のため、イメージの互換検査エンドポイントから`ContainerConfig`フィールドを削除。

* **アップデートの傾向**:
  * セキュリティ脆弱性への迅速な対応を継続。
  * リモートクライアントの機能強化（特にTLS暗号化）や、Kubernetes連携機能（`kube play`）の改善に注力。
  * systemdとの統合をより洗練させるQuadletの機能拡張が活発。
  * Windows対応の改善（新しいインストーラ）も進められており、クロスプラットフォームでの利便性向上を目指している。
