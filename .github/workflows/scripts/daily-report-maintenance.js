...
function buildJulesPrompt(toolName, reportPath, todayJST, templateContent) {
  const promptLines = [
    ...
    '- **代替手段の模索**: リンクチェック等のツール実行で403や404エラーに直面した場合や、情報が不足している場合は、自律的にGoogle検索（`google_search`）や外部ソースを活用し、検索キーワードを工夫して必要なエビデンスを自律的に収集せよ。',
    '- **自己レビューと自律的リトライ**: 更新されたレポートの記述に不備（リンク切れ、カテゴリやタグの不一致、テンプレート違反など）があると自己レビューで気づいた場合は、立ち止まらずに自律的に修正し、必要な条件を満たすまで検証を繰り返せ。',
