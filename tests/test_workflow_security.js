...
testCases.forEach((test) => {
  try {
    ...
    const reportCount = Number.parseInt(process.env.REPORT_COUNT, 10);
    ...
    if (Number.isNaN(test.expected)) {
      ...
      assert.strictEqual(
        reportCount,
        test.expected,
        `Expected ${test.expected} for input "${test.input}", got ${reportCount}`,
      );
    }
...
targetTestCases.forEach((test) => {
  try {
    ...
    assert.strictEqual(
      targetInput,
      test.expected.trim(),
      `Expected "${test.expected.trim()}" for input "${test.input}", got "${targetInput}"`,
    );
